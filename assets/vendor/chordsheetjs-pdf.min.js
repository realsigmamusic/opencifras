var ChordSheetJSPdf=(()=>{var Yt=Object.defineProperty;var DM=Object.getOwnPropertyDescriptor;var NM=Object.getOwnPropertyNames;var jM=Object.prototype.hasOwnProperty;var bM=(i,e)=>{for(var A in e)Yt(i,A,{get:e[A],enumerable:!0})},zM=(i,e,A,t)=>{if(e&&typeof e=="object"||typeof e=="function")for(let c of NM(e))!jM.call(i,c)&&c!==A&&Yt(i,c,{get:()=>e[c],enumerable:!(t=DM(e,c))||t.enumerable});return i};var TM=i=>zM(Yt({},"__esModule",{value:!0}),i);var io={};bM(io,{JsPdfMeasurer:()=>Vs,PdfFormatter:()=>co});var pc=window.jspdf.jsPDF;function yM(i){return i&&i.__esModule?i.default:i}var Rt=class{measureTextWidth(e,A){return this.measureText(e,A).width}measureTextHeight(e,A){return this.measureText(e,A).height}splitTextWithMeasure(e,A,t){return e?e.replace(/\r\n|\r/g,`
`).split(`
`).reduce((M,o)=>(M.push(...this.wrapParagraph(o,A,t)),M),[]):[]}wrapParagraph(e,A,t){if(e.length===0)return[""];let{lines:c,currentLine:M}=this.accumulateWords(e.split(" "),A,t);return M.length===0?c:[...c,M]}splitWord(e,A,t){let c=[...e].reduce((M,o)=>{let a=`${M.partial}${o}`;return t(a)<=A?M.partial=a:M.partial.length>0?(M.lines.push(M.partial),M.partial=o):(M.lines.push(o),M.partial=""),M},{lines:[],partial:""});return{lines:c.lines,remainder:c.partial}}accumulateWords(e,A,t){return e.reduce((c,M)=>{let o=c.currentLine.length===0?M:`${c.currentLine} ${M}`;if(t(o)<=A)c.currentLine=o;else if(c.currentLine.length>0)c.lines.push(c.currentLine),c.currentLine=M;else{let{lines:a,remainder:U}=this.splitWord(M,A,t);c.lines.push(...a),c.currentLine=U}return c},{lines:[],currentLine:""})}},Vs=class extends Rt{constructor(e){super(),this.doc=e}measureText(e,A){let t;return this.doc.withFontConfiguration(A,()=>{let c=this.doc.getTextDimensions(e);t={width:c.w,height:c.h}}),t}splitTextToSize(e,A,t){let c=[];return this.doc.withFontConfiguration(t,()=>{c=this.doc.splitTextToSize(e,A)}),c}},dt=class{constructor(e,A,t,c){this.pageWidth=e,this.pageHeight=A,this.layoutConfig=t,this.columns=c}get margins(){return this.layoutConfig.global.margins}get minX(){return this.margins.left}get maxX(){return this.pageWidth-this.margins.right}get minY(){return this.margins.top+this.layoutConfig.header.height}get maxY(){return this.pageHeight-this.margins.bottom}calculateOptimalColumnCount(e,A,t,c){return!t&&!c?1:c&&!t?this.fitMaxColumns(e,A,c):t&&!c?this.fitMaxColumnsWithMinWidth(e,A,t):t&&c?this.findMaxColumnsWithinBounds(e,A,t,c):1}findMaxColumnsWithinBounds(e,A,t,c){let M=Math.floor((e+A)/(t+A));for(let o=1;o<=M;o+=1){let a=this.evaluateColumnCount(o,e,A,t,c);if(a!==null)return a}return Math.max(1,M)}evaluateColumnCount(e,A,t,c,M){let o=(e-1)*t,a=(A-o)/e;if(a<c)return Math.max(1,e-1);if(a>=c&&a<=M){let U=e+1,Y=(U-1)*t;if((A-Y)/U<c)return e}return null}fitMaxColumnsWithMinWidth(e,A,t){let c=Math.floor((e+A)/(t+A)),M=Math.max(1,c),o=(M-1)*A;return M*t+o<=e?M:Math.max(1,M-1)}fitMaxColumns(e,A,t){return Math.max(1,Math.floor((e+A)/(t+A)))}calculateEffectiveColumnSpacing(e,A,t,c,M){return e<=1||M&&!c?t:c?this.calculateAdjustedSpacing(e,A,t,c):t}calculateAdjustedSpacing(e,A,t,c){let M=(e-1)*t,o=(A-M)/e;if(o>c){let Y=(o-c)*e/(e-1);return t+Y}return t}get effectiveColumnCount(){let{columnCount:e,columnSpacing:A,minColumnWidth:t,maxColumnWidth:c}=this.columns,{left:M,right:o}=this.margins,a=this.pageWidth-M-o;return t||c?this.calculateOptimalColumnCount(a,A,t,c):e||e||1}get effectiveColumnSpacing(){let e=this.effectiveColumnCount,{left:A,right:t}=this.margins,c=this.pageWidth-A-t,M=this.columns.columnSpacing,{maxColumnWidth:o,minColumnWidth:a}=this.columns;return this.calculateEffectiveColumnSpacing(e,c,M,o,a)}get availableSpace(){let{left:e,right:A}=this.margins;return this.pageWidth-e-A}get columnWidth(){let e=this.effectiveColumnCount,A=this.effectiveColumnSpacing,t=(e-1)*A,c=(this.availableSpace-t)/e,{maxColumnWidth:M,minColumnWidth:o}=this.columns;return e===1||o&&!M?c:M&&c>M&&e>1?M:c}},RM=dt,Gt=class{constructor(e=null){this.parentLine=null,this.line=null,this.column=null,this.offset=null,e&&(this.line=e.line||null,this.column=e.column||null,this.offset=e.offset||null)}},Ei=Gt,Ms="bridge",Fe="chorus",dB="grid",dM="indeterminate",ve="none",GB="tab",rs="verse",Bt="part",uB="ly",xB="abc",pB="svg",HB="textblock",aA="symbol",CA="numeric",zA="numeral",bA="solfege",Hc=["I","II","III","IV","V","VI","VII"],O="b",$="#",Ls="NM",sA=Ls,iA="m",nA="M",GM=["N","-1","x"],uM=0,Ke="start_tag",Oe="end_tag",ai="auto",Ps="german",Qi="album",xM="arranger",fi="artist",ns="capo",JB="comment",Ii="composer",wi="copyright",Ci="duration",li="image",pM="end_of_abc",SB="end_of_bridge",ct="end_of_chorus",hi="end_of_grid",HM="end_of_ly",Fi="end_of_tab",kB="end_of_verse",Ws="end_of_part",Ue="key",Js="_key",Ui="lyricist",JM="sorttitle",Yi="start_of_abc",it="start_of_bridge",os="start_of_chorus",VB="start_of_grid",mi="start_of_ly",LB="start_of_tab",Di="start_of_svg",SM="end_of_svg",Ni="start_of_textblock",kM="end_of_textblock",gt="start_of_verse",ts="start_of_part",PB="subtitle",ji="tempo",bi="time",WB="title",VM="transpose",zi="new_key",Ti="year",Zs="chordfont",Xs="chordsize",ut="chordcolour",vs="textfont",Ks="textsize",xt="textcolour";var ZB="chorus",yi="chord_style",Jc={[Yi]:xB,[it]:Ms,[os]:ZB,[VB]:dB,[mi]:uB,[Di]:pB,[LB]:GB,[Ni]:HB,[gt]:rs},Sc={[pM]:xB,[SB]:Ms,[ct]:ZB,[hi]:dB,[HM]:uB,[SM]:pB,[Fi]:GB,[kM]:HB,[kB]:rs},LM=/^start_of_(.+)$/,PM=/^end_of_(.+)$/,pt=class{static interpret(e,A){return new this(e,A).interpret()}constructor(e,A){this.tagName=e,this.tagValue=A}interpret(){return this.startOfPart()||this.endOfPart()||this.sectionStart()||this.sectionEnd()||this.startOfSection()||this.endOfSection()||[null,null]}startOfPart(){return this.tagName===ts&&this.tagValue?[Ke,this.tagValue.split(" ")[0].toLowerCase()]:null}endOfPart(){return this.tagName===Ws?[Oe,ai]:null}sectionStart(){return this.tagName in Jc?[Ke,Jc[this.tagName]]:null}sectionEnd(){return this.tagName in Sc?[Oe,Sc[this.tagName]]:null}startOfSection(){let e=LM.exec(this.tagName);return e?[Ke,e[1]]:null}endOfSection(){let e=PM.exec(this.tagName);return e?[Oe,e[1]]:null}},Ht=pt,Ri=Symbol.for("chordsheetjs.ChordLyricsPair"),di=Symbol.for("chordsheetjs.Comment"),Gi=Symbol.for("chordsheetjs.Literal"),ui=Symbol.for("chordsheetjs.SoftLineBreak"),xi=Symbol.for("chordsheetjs.Tag");function Es(i,e){Object.defineProperty(i,e,{value:!0,enumerable:!1,configurable:!1})}function as(i,e){return!!(i&&typeof i=="object"&&i[e]===!0)}var WM="cf",ZM="cs",pi="c",XM="eob",vM="eoc",KM="eog",OM="eot",$M="eov",qM="eop",_M="nk",Ar="sob",er="soc",sr="sog",tr="sot",Br="sov",cr="sop",ir="st",gr="tf",Mr="ts",rr="t",nr="p",or="ep",Er=[JB,li],Jt=[WB,PB,fi,Qi,Ti,Ii,Ui,Ue,ns,yi,ji,bi,xM,wi,Ci,JM],ar=[Js],Qr=[Zs,Xs,ut,vs,Ks,xt],fr=[ZB,Yi,it,os,VB,mi,Di,LB,Ni,gt,ts],kc={[WM]:Zs,[ZM]:Xs,[pi]:JB,[XM]:SB,[vM]:ct,[KM]:hi,[OM]:Fi,[$M]:kB,[qM]:Ws,[or]:Ws,[_M]:zi,[Ar]:it,[er]:os,[sr]:VB,[tr]:LB,[Br]:gt,[cr]:ts,[nr]:ts,[ir]:PB,[gr]:vs,[Mr]:Ks,[rr]:WB},Ir=/^([^:\s]+)(:?\s*(.+))?$/,Vc=/^x_(.+)$/;function Lc(i){return ar.includes(i)}var wr=i=>{if(!i)return i;let e=i.trim();return e in kc?kc[e]:e},Os=class i extends Ei{static[Symbol.hasInstance](e){return as(e,xi)}constructor(e,A=null,t=null,c={},M=null,o=!1){super(t),this._isMetaTag=!1,this._originalName="",this._name="",this._value="",this.selector=null,this.isNegated=!1,this.attributes={},this.parseNameValue(e,A),this.attributes=c,this.selector=M,this.isNegated=o}parseNameValue(e,A){e==="meta"?this.parseMetaTag(A):(this.name=e,this.value=A||"")}parseMetaTag(e){if(!e)throw new Error("Expected value");let[A,t]=e.split(/\s(.+)/);this.name=A,this.value=t||"",this._isMetaTag=!0}static parse(e){return e instanceof i?e:this.parseWithRegex(e,Ir)}static parseWithRegex(e,A){let t=e.match(A);return t!==null?new i(t[1],t[3]||null):null}static parseOrFail(e){let A=this.parse(e);if(!A)throw new Error(`Failed to parse ${e}`);return A}get label(){let e=this.attributes.label;return e&&e.length>0?e:this.value||""}isSectionDelimiter(){return this.isSectionStart()||this.isSectionEnd()}isSectionStart(){let[e]=Ht.interpret(this.name,this.value);return e===Ke}isSectionEnd(){let[e]=Ht.interpret(this.name,this.value);return e===Oe}isInlineFontTag(){return Qr.includes(this.name)}isComment(){return this.name===JB||this.name===pi}isImage(){return this.name===li}set name(e){this._name=wr(e),this._originalName=e}get name(){return this._name.trim()}get originalName(){return this._originalName.trim()}set value(e){this._value=e||""}get value(){return`${this._value}`.trim()}hasValue(){return this.value.length>0}hasAttributes(){return Object.keys(this.attributes).length>0}hasLabel(){return this.label.length>0}isRenderable(){return Er.includes(this.name)||this.hasRenderableLabel()}hasRenderableLabel(){return(fr.includes(this.name)||this.isSectionStart())&&this.hasLabel()}isMetaTag(){return this._isMetaTag||Vc.test(this.name)||Jt.indexOf(this.name)!==-1}isStandardOrCustomMetaTag(){return Jt.indexOf(this.name)!==-1||Vc.test(this.name)}clone(){return new i(this._originalName,this.value,null,this.attributes)}toString(){return`Tag(name=${this.name}, value=${this.value})`}set({value:e}){return new i(this._originalName,e,null,this.attributes)}setAttribute(e,A){return new i(this._originalName,this.value,null,{...this.attributes,[e]:A})}};Es(Os.prototype,xi);var J=Os,he=i=>i,Cr={separator:",",order:[...Jt,{match:/^x_/,sortMethod:"alphabetical"}]},lr={abc:he,ly:he,svg:he,tab:he,grid:he},hr={decapo:!1,delegates:lr,evaluate:!1,expandChorusDirective:!1,instrument:null,key:null,metadata:Cr,normalizeChords:!0,normalizeChordSuffix:!0,useUnicodeModifiers:!1,user:null};var Fr={cssClasses:{annotation:"annotation",chord:"chord",chordSheet:"chord-sheet",column:"column",comment:"comment",emptyLine:"empty-line",image:"image",label:"label",labelWrapper:"label-wrapper",line:"line",literal:"literal",literalContents:"contents",lyrics:"lyrics",paragraph:"paragraph",rhythmSymbol:"rhythm-symbol",row:"row",subtitle:"subtitle",title:"title"}},Hi={title:{name:"Helvetica",style:"bold",size:18,color:"#000000"},subtitle:{name:"Helvetica",style:"normal",size:14,color:"#000000"},chord:{name:"Helvetica",style:"bold",size:13,color:"#000000"},comment:{name:"Helvetica",style:"italic",size:13,color:"#666666"},metadata:{name:"Helvetica",style:"normal",size:11,color:"#555555"},text:{name:"Helvetica",style:"normal",size:12,color:"#000000"},annotation:{name:"Helvetica",style:"normal",size:10,color:"#000000"},sectionLabel:{name:"Helvetica",style:"bold",size:14,color:"#000000"}},Ur={global:{margins:{top:40,right:40,bottom:40,left:40}},header:{height:0,content:[]},footer:{height:0,content:[]}},Yr={fonts:Hi,measurer:"canvas",layout:Ur},mr={pageSize:{width:0,height:0},fonts:{title:{name:"Arial",style:"bold",size:22,color:"#151515"},subtitle:{name:"Arial",style:"normal",size:11,color:"#6f6f6f"},metadata:{name:"Arial",style:"normal",size:10,color:"#8b8b8b"},text:{name:"Arial",style:"normal",size:10,color:"#232323"},chord:{name:"Arial",style:"bold",size:9,color:"#232323"},comment:{name:"Arial",style:"bold",size:10,color:"#232323"},sectionLabel:{name:"Arial",style:"bold",size:10,color:"#a1312d"},annotation:{name:"Arial",style:"normal",size:10,color:"#232323"}},layout:{global:{margins:{top:14,bottom:12,left:45,right:45}},header:{height:72,content:[{type:"text",template:"%{title}",style:{name:"Arial",style:"bold",size:19,color:"#151515",weight:700},position:{x:"left",y:0,clip:!0,ellipsis:!0},condition:{page:{first:!0}}},{type:"text",template:"%{artist}",style:{name:"Arial",style:"normal",size:11,color:"#6f6f6f"},position:{x:"left",y:25,clip:!0,ellipsis:!0},condition:{page:{first:!0}}},{type:"text",template:"%{tempo|%{} BPM}%{time| \xB7 %{}}%{capo| \xB7 Capo %{}}",style:{name:"Arial",style:"normal",size:11,color:"#6f6f6f"},position:{x:"left",y:40,width:240,clip:!0,ellipsis:!0},condition:{page:{first:!0}}},{type:"line",style:{width:1,color:"#d7d7d7"},position:{x:0,y:60,width:"auto",height:0},condition:{page:{first:!0}}},{type:"text",template:"%{key}",cssClass:"measured-html-key-badge",elementStyle:{width:"28px",height:"28px",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:"999px",backgroundColor:"#a1312d",textAlign:"center",boxSizing:"border-box"},style:{name:"Arial",style:"bold",size:12,color:"#ffffff",weight:700},position:{x:"right",y:12,width:28,offsetX:-2},condition:{and:[{page:{first:!0}},{key:{exists:!0}}]}},{type:"text",template:"%{title}",style:{name:"Arial",style:"bold",size:12,color:"#151515",weight:700},position:{x:"left",y:0,clip:!0,ellipsis:!0},condition:{page:{greater_than:1}}},{type:"text",template:"%{page}/%{pages}",style:{name:"Arial",style:"normal",size:10,color:"#9a9a9a"},position:{x:"right",y:4,offsetX:-38},condition:{page:{greater_than:1}}},{type:"text",template:"%{key}",cssClass:"measured-html-key-badge",elementStyle:{width:"28px",height:"28px",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:"999px",backgroundColor:"#a1312d",textAlign:"center",boxSizing:"border-box"},style:{name:"Arial",style:"bold",size:12,color:"#ffffff",weight:700},position:{x:"right",y:2,width:28,offsetX:-2},condition:{and:[{page:{greater_than:1}},{key:{exists:!0}}]}},{type:"line",style:{width:1,color:"#d7d7d7"},position:{x:0,y:32,width:"auto",height:0},condition:{page:{greater_than:1}}}]},footer:{height:0,content:[]},sections:{global:{paragraphSpacing:10,linePadding:4,chordLyricSpacing:2,chordSpacing:2,columnWidth:0,columnSpacing:25,minColumnWidth:0,maxColumnWidth:0},base:{display:{lyricsOnly:!1,repeatedSections:"full"}}},chordDiagrams:{enabled:!0,renderingConfig:{titleY:28,neckWidth:120,neckHeight:160,nutThickness:10,fretThickness:4,nutColor:0,fretColor:"#929292",stringIndicatorSize:14,fingerIndicatorSize:16,stringColor:0,fingerIndicatorOffset:0,stringThickness:3,fretLineThickness:4,openStringIndicatorThickness:2,unusedStringIndicatorThickness:2,markerThickness:2,barreThickness:2,titleFontSize:48,baseFretFontSize:8,fingerNumberFontSize:28,showFingerNumbers:!0,diagramSpacing:7},fonts:{title:{name:"Arial",style:"bold",size:9,color:"#232323"},fingerings:{name:"Arial",style:"bold",size:6,color:"#232323"},baseFret:{name:"Arial",style:"bold",size:6,color:"#232323"}}}}},Dr={fonts:{title:{name:"NimbusSansL-Bol",style:"bold",size:24,color:"black"},subtitle:{name:"NimbusSansL-Reg",style:"normal",size:10,color:100},metadata:{name:"NimbusSansL-Reg",style:"normal",size:10,color:100},text:{name:"NimbusSansL-Reg",style:"normal",size:10,color:"black"},chord:{name:"NimbusSansL-Bol",style:"bold",size:9,color:"black"},comment:{name:"NimbusSansL-Bol",style:"bold",size:10,color:"black"},sectionLabel:{name:"NimbusSansL-Bol",style:"bold",size:10,color:"black"},annotation:{name:"NimbusSansL-Reg",style:"normal",size:10,color:"black"}},layout:{global:{margins:{top:35,bottom:10,left:45,right:45}},header:{height:60,content:[{type:"text",template:"%{title}",style:{name:"NimbusSansL-Bol",style:"bold",size:24,color:"black"},position:{x:"left",y:15}},{type:"text",template:"Key of %{key} - BPM %{tempo} - Time %{time}",style:{name:"NimbusSansL-Reg",style:"normal",size:12,color:100},position:{x:"left",y:28}},{type:"text",template:"By %{artist} %{subtitle}",style:{name:"NimbusSansL-Reg",style:"normal",size:10,color:100},position:{x:"left",y:38}}]},footer:{height:30,content:[{type:"text",value:"\xA92024 My Music Publishing",style:{name:"NimbusSansL-Reg",style:"normal",size:10,color:"black"},position:{x:"left",y:0}}]},sections:{global:{paragraphSpacing:10,linePadding:4,chordLyricSpacing:2,chordSpacing:2,columnCount:2,columnWidth:0,columnSpacing:25},base:{display:{lyricsOnly:!1,repeatedSections:"title_only"}}},chordDiagrams:{enabled:!0,renderingConfig:{titleY:28,neckWidth:120,neckHeight:160,nutThickness:10,fretThickness:4,nutColor:0,fretColor:"#929292",stringIndicatorSize:14,fingerIndicatorSize:16,stringColor:0,fingerIndicatorOffset:0,stringThickness:3,fretLineThickness:4,openStringIndicatorThickness:2,unusedStringIndicatorThickness:2,markerThickness:2,barreThickness:2,titleFontSize:48,baseFretFontSize:8,fingerNumberFontSize:28,showFingerNumbers:!0,diagramSpacing:7},fonts:{title:{name:"NimbusSansL-Bol",style:"bold",size:9,color:"black"},fingerings:{name:"NimbusSansL-Bol",style:"bold",size:6,color:"black"},baseFret:{name:"NimbusSansL-Bol",style:"bold",size:6,color:"black"}}}}},St=class extends Error{},Nr={Ab:{B:"Cb"},Bb:{B:"Cb"},Cb:{B:"Cb","A#":"Bb",E:"Fb"},C:{"C#":"Db","D#":"Eb","F#":"Gb","G#":"Ab","A#":"Bb"},"C#":{Eb:"D#",Bb:"A#"},Db:{Cb:"B","F#":"Gb","G#":"Ab","A#":"Bb"},D:{"D#":"Eb","A#":"Bb",Gb:"F#"},Eb:{"D#":"Eb","F#":"Gb","G#":"Ab","A#":"Bb"},E:{Ab:"G#","A#":"Bb","D#":"Eb",Db:"C#",Eb:"D#"},F:{"A#":"Bb","F#":"Gb","D#":"Eb","G#":"Ab"},"F#":{Bb:"A#",Eb:"D#"},Gb:{"A#":"Bb","D#":"Eb","G#":"Ab",B:"Cb",E:"Fb"},G:{"A#":"Bb","D#":"Eb","G#":"Ab","C#":"Db"},"G#":{"A#":"Bb","D#":"Eb",Cb:"B#"},Am:{"G#":"Ab","F#":"Gb","C#":"Db","D#":"Eb","A#":"Bb"},Bbm:{Cb:"B",Gb:"F#"},Bm:{"A#":"Bb","D#":"Eb"},"C#m":{"A#":"Bb","D#":"Eb",Gb:"F#"},Cm:{"G#":"Ab","A#":"Bb","D#":"Eb","F#":"Gb","C#":"Db"},Dm:{"A#":"Bb","D#":"Eb","F#":"Gb","G#":"Ab","C#":"Db"},Em:{"A#":"Bb","D#":"Eb","C#":"Db"},"F#m":{"A#":"Bb","D#":"Eb",Gb:"F#",Ab:"G#",Db:"C#"},Fm:{"G#":"Ab","A#":"Bb","D#":"Eb","F#":"Gb","C#":"Db"},Gm:{"G#":"Ab","A#":"Bb","D#":"Eb","C#":"Db","F#":"Gb"},"G#m":{Bb:"A#",Eb:"D#"},B:{Eb:"D#"},Lab:{Si:"Dob"},Dob:{Si:"Dob","La#":"Sib",Mi:"Fab"},Do:{"Do#":"Reb","Re#":"Mib","Fa#":"Solb","Sol#":"Lab","La#":"Sib"},"Do#":{Mib:"Re#",Sib:"La#"},Reb:{Si:"Dob","Fa#":"Solb"},Re:{"Re#":"Mib","La#":"Sib",Solb:"Fa#"},Mib:{"Re#":"Mib","Fa#":"Solb","Sol#":"Lab","La#":"Sib"},Mi:{Lab:"Sol#","La#":"Sib","Re#":"Mib",Reb:"Do#",Mib:"Re#"},Fa:{"La#":"Sib","Fa#":"Solb","Re#":"Mib","Sol#":"Lab"},"Fa#":{Sib:"La#",Mib:"Re#"},Solb:{"La#":"Sib","Re#":"Mib","Sol#":"Lab",Si:"Dob",Mi:"Fab"},Sol:{"La#":"Sib","Re#":"Mib","Sol#":"Lab","Do#":"Reb"},"Sol#":{"La#":"Sib","Re#":"Mib",Dob:"Si#"},Lam:{"Sol#":"Lab","Fa#":"Solb","Do#":"Reb","Re#":"Mib","La#":"Sib"},Sibm:{Dob:"Si",Solb:"Fa#"},Sim:{"La#":"Sib","Re#":"Mib"},"Do#m":{"La#":"Sib","Re#":"Mib",Solb:"Fa#"},Dom:{"Sol#":"Lab","La#":"Sib","Re#":"Mib","Fa#":"Solb","Do#":"Reb"},Rem:{"La#":"Sib","Re#":"Mib","Fa#":"Solb","Sol#":"Lab","Do#":"Reb"},Mim:{"La#":"Sib","Re#":"Mib","Do#":"Reb"},"Fa#m":{"La#":"Sib","Re#":"Mib",Solb:"Fa#",Lab:"Sol#",Reb:"Do#"},Fam:{"Sol#":"Lab","La#":"Sib","Re#":"Mib","Fa#":"Solb","Do#":"Reb"},Solm:{"Sol#":"Lab","La#":"Sib","Re#":"Mib","Do#":"Reb","Fa#":"Solb"},"Sol#m":{Sib:"La#",Mib:"Re#"},Si:{Mib:"Re#"}},jr=Nr,br={symbol:/^(?<key>((?<note>[A-Ha-h])(?<accidental>#|b)?))(?<minor>m)?$/,solfege:/^(?<key>((?<note>Do|Re|Mi|Fa|Sol|La|Si|do|re|mi|fa|sol|la|si)(?<accidental>#|b)?))(?<minor>m)?$/,numeric:/^(?<key>(?<accidental>#|b)?(?<note>[1-7]))(?<minor>m)?$/,numeral:/^(?<key>(?<accidental>#|b)?(?<note>I{1,3}|IV|VI{0,2}|i{1,3}|iv|vi{0,2}))$/},Pc=[aA,bA,CA,zA],zr=[1,2,3,4,5,8,9,10],Tr=[4,11],yr=[1,4],Rr=[5,0],dr=[3,7],Wc=[2,3,10],kt=class i{get unicodeAccidental(){switch(this.accidental){case O:return"\u266D";case $:return"\u266F";default:return null}}get unicodeModifier(){return HA("unicodeModifier is deprecated, use unicodeAccidental instead"),this.unicodeAccidental}get modifier(){return HA("modifier is deprecated, use accidental instead"),this.accidental}get preferredModifier(){return HA("preferredModifier is deprecated, use preferredAccidental instead"),this.preferredAccidental}static parse(e){if(!e)return null;let A=e.trim();if(!A)return null;for(let t=0,c=Pc.length;t<c;t+=1){let M=this.parseAsType(A,Pc[t]);if(M)return M}return null}static parseAsType(e,A){let t=e.match(br[A]);if(!t)return null;let{minor:c,note:M,accidental:o}=t.groups;return this.resolve({key:M,keyType:A,minor:c||!1,accidental:o||null,preferredNotation:A===aA&&Ji(M)?Ps:null})}static resolve({key:e,keyType:A,minor:t,accidental:c,preferredNotation:M}){let o=`${e}`,a=this.isMinor(o,A,t),U=Vr(o,M),Y=kr(o,c,U)??c,N=A===aA||A===bA?vc(o,Y||Ls,A,a):null;return N!==null?new i({grade:0,minor:a,type:A,accidental:c||null,preferredAccidental:c||null,referenceKeyGrade:N,originalKeyString:o,preferredNotation:U}):new i({number:this.getNumberFromKey(o,A),minor:a,type:A,accidental:c||null,preferredAccidental:c||null,originalKeyString:o})}static getNumberFromKey(e,A){if(A===CA)return parseInt(e,10);let t=e.toUpperCase();return Hc.findIndex(c=>t===c)+1}static keyWithAccidental(e,A,t){let c=e.toUpperCase(),M=A||"";return t===bA?`${e.charAt(0).toUpperCase()+e.slice(1).toLowerCase()}${M}`:t===aA?`${c}${M}`:`${M}${c}`}static keyWithModifier(e,A,t){return HA("keyWithModifier is deprecated, use keyWithAccidental instead"),this.keyWithAccidental(e,A,t)}static isMinor(e,A,t){switch(A){case"numeral":return e.toLowerCase()===e;default:switch(typeof t){case"string":return t==="m"||t.toLowerCase()==="min";case"boolean":return t;default:return!1}}}static parseOrFail(e){let A=this.parse(e);if(!A)throw new Error(`Failed to parse ${e}`);return A}static wrap(e){return e instanceof i?e:e===null?null:this.parse(e)}static wrapOrFail(e=null){if(e===null)throw new Error("Unexpected null key");let A=this.wrap(e);if(A===null)throw new Error(`Failed: invalid key ${e}`);return A}static toString(e){return`${i.wrapOrFail(e)}`}static distance(e,A){return this.wrapOrFail(e).distanceTo(A)}constructor({grade:e=null,number:A=null,minor:t,type:c,accidental:M,referenceKeyGrade:o=null,referenceKeyMode:a=null,originalKeyString:U=null,preferredAccidental:Y=null,explicitAccidental:N=!1,preferredNotation:R=null}){this.number=null,this.minor=!1,this.referenceKeyGrade=null,this.referenceKeyMode=null,this.originalKeyString=null,this.explicitAccidental=!1,this.preferredNotation=null,this.grade=e,this.number=A,this.minor=t,this.type=c,this.accidental=M,this.preferredAccidental=Y,this.referenceKeyGrade=o,this.referenceKeyMode=a,this.originalKeyString=U,this.explicitAccidental=N,this.preferredNotation=R}distanceTo(e){let A=i.wrapOrFail(e);return i.shiftGrade(A.effectiveGrade-this.effectiveGrade)}get effectiveGrade(){if(this.grade===null)throw new Error("Cannot calculate effectiveGrade without a grade");return i.shiftGrade(this.grade+(this.referenceKeyGrade||0))}isMinor(){return this.minor}makeMinor(){return this.set({minor:!0})}get relativeMajor(){return this.changeGrade(3).set({minor:!1})}get relativeMinor(){return this.changeGrade(-3).set({minor:!0})}toMajor(){return this.isMinor()?this.transpose(3).set({minor:!1}):this.clone()}clone(){return this.set({})}ensureGrade(){this.grade===null&&this.calculateGradeFromNumber()}calculateGradeFromNumber(){if(this.number===null)throw new Error("Cannot calculate grade, number is null");this.grade=vc(this.number.toString(),this.accidental||Ls,CA,this.isMinor()),this.number=null}toChordSymbol(e,A=!1){return this.isChordSymbol()?this.clone():this.convertToChordType(e,aA,A)}toChordSolfege(e,A=!1){return this.isChordSolfege()?this.clone():this.convertToChordType(e,bA,A)}convertToChordType(e,A,t){let{accidental:c}=this;this.ensureGrade();let M=i.wrapOrFail(e),o=this.handleMinorKeyConversion(M,t);if(o)return o;let U=this.set({referenceKeyGrade:i.shiftGrade(this.effectiveGrade+M.effectiveGrade),grade:0,type:A,accidental:null,preferredAccidental:c||M.accidental}).normalizeEnharmonics(M);return c?U.set({preferredAccidental:c,accidental:null}):U}handleMinorKeyConversion(e,A){return!(this.isNumeral()||this.isNumeric())||!A?null:this.grade===8?e.relativeMinor:this.grade===7?e.relativeMinor.changeGrade(-1):this.grade===9?e.relativeMinor.changeGrade(1):null}toChordSymbolString(e){return this.toChordSymbol(e).toString()}toChordSolfegeString(e){return this.toChordSolfege(e).toString()}is(e){return this.type===e}isNumeric(){return this.is(CA)}isChordSymbol(){return this.is(aA)}isChordSolfege(){return this.is(bA)}isNumeral(){return this.is(zA)}equals(e){return this.grade===e.grade&&this.number===e.number&&this.accidental===e.accidental&&this.preferredAccidental===e.preferredAccidental&&this.type===e.type&&this.minor===e.minor}static equals(e,A){return e===null?A===null:A===null?!1:e.equals(A)}toNumeric(e=null){if(this.isNumeric())return this.clone();if(this.isNumeral())return this.set({type:CA});let A=i.wrapOrFail(e),t=A.effectiveGrade,c=A.minor?iA:nA,M=i.shiftGrade(this.effectiveGrade-t),o=A.accidental;return Wc.includes(M)&&(o=O),this.set({type:CA,grade:i.shiftGrade(this.effectiveGrade-t),referenceKeyGrade:0,accidental:null,preferredAccidental:o,referenceKeyMode:c})}toNumericString(e=null){return this.toNumeric(e).toString()}toNumeral(e=null){if(this.isNumeral())return this.clone();if(this.isNumeric())return this.set({type:zA});let A=i.wrapOrFail(e),t=A.effectiveGrade,c=A.minor?iA:nA,M=i.shiftGrade(this.effectiveGrade-t),o=A.accidental;return Wc.includes(M)&&(o=O),this.set({type:zA,grade:i.shiftGrade(this.effectiveGrade-t),referenceKeyGrade:0,accidental:null,preferredAccidental:o,referenceKeyMode:c})}toNumeralString(e=null){return this.toNumeral(e).toString()}toString({showMinor:e=!0,useUnicodeModifier:A=!1}={}){let{note:t}=this;return A&&(t=t.replace("#","\u266F").replace("b","\u266D")),`${t}${e?this.minorSign:""}`}get note(){if(this.grade===null)return this.getNoteForNumber();if((this.isChordSymbol()||this.isChordSolfege())&&this.referenceKeyGrade===null)throw new Error("Not possible, reference key grade is null");let{minor:e}=this;this.referenceKeyMode&&(e=this.referenceKeyMode===iA);let A=vr({type:this.type,accidental:this.accidental,preferredAccidental:this.preferredAccidental,grade:this.effectiveGrade,minor:e});return this.applyGermanRendering(A)}applyGermanRendering(e){return this.preferredNotation!==Ps||!this.isChordSymbol()?e:this.accidental===null&&this.effectiveGrade===10?"B":e==="B"?"H":e}getNoteForNumber(){if(this.number===null)throw new Error("Not possible, grade and number are null");if(this.isNumeric())return`${this.accidental||""}${this.number}`;let e=Hc[this.number-1];return`${this.accidental||""}${this.isMinor()?e.toLowerCase():e}`}get minorSign(){if(!this.minor)return"";switch(this.type){case aA:return"m";case bA:return"m";case CA:return this.isNaturalMinor()?"":"m";default:return""}}isNaturalMinor(){if(this.ensureGrade(),!this.grade)throw new Error("Expected grade to be set, but it is is still empty.");return zr.includes(this.grade)}transpose(e){if(e===0)return this;let A=this.accidental,t=this.clone(),c=e<0?"transposeDown":"transposeUp";for(let M=0,o=Math.abs(e);M<o;M+=1)t=t[c]();return t.useAccidental(A)}changeGrade(e){return this.referenceKeyGrade?this.set({referenceKeyGrade:i.shiftGrade(this.referenceKeyGrade+e)}):(this.ensureGrade(),this.set({grade:i.shiftGrade(this.grade+e)}))}transposeUp(){let A=this.normalize().changeGrade(1);return this.accidental||!A.canBeSharp()?A=A.useAccidental(null):A.canBeSharp()&&(A=A.useAccidental($)),A=A.set({preferredAccidental:$}).normalize(),A}transposeDown(){let A=this.normalize().changeGrade(-1);return this.accidental||!A.canBeFlat()?A=A.useAccidental(null):A.canBeFlat()&&(A=A.useAccidental(O)),A.set({preferredAccidental:O})}canBeFlat(){let e=this.number!==null?yr:Tr,A=this.number??this.effectiveGrade;return!e.includes(A)}canBeSharp(){let e=this.number!==null?dr:Rr,A=this.number??this.effectiveGrade;return!e.includes(A)}setGrade(e){return this.set({grade:i.shiftGrade(e)})}static shiftGrade(e){return e<0?this.shiftGrade(e+12):e%12}useAccidental(e){return this.ensureGrade(),this.set({accidental:e,explicitAccidental:e!==null})}useModifier(e){return HA("useModifier is deprecated, use useAccidental instead"),this.useAccidental(e)}normalize(){return this.ensureGrade(),this.accidental===$&&!this.canBeSharp()?this.set({accidental:null}):this.accidental===O&&!this.canBeFlat()?this.set({accidental:null}):this.clone()}normalizeEnharmonics(e){if(e){if(this.explicitAccidental)return this.clone();let A=Xc(i.wrapOrFail(e).toString({showMinor:!0})),t=jr[A],c=Xc(this.toString({showMinor:!1}));if(t&&t[c])return i.parseOrFail(t[c]).set({minor:this.minor,preferredNotation:this.preferredNotation})}return this.clone()}set(e,A=!0){return new i({...A?{}:e,grade:this.grade,number:this.number,type:this.type,accidental:this.accidental,minor:this.minor,referenceKeyGrade:this.referenceKeyGrade,originalKeyString:this.originalKeyString,preferredAccidental:this.preferredAccidental,explicitAccidental:this.explicitAccidental,preferredNotation:this.preferredNotation,...A?e:{}})}},eA=kt;function mt(i,e,A){return A=A||" ",i.length>e?i:(e-=i.length,A+=A.repeat(e),i+A.slice(0,e))}var $e=class i extends Error{static buildMessage(e,A){function t(Y){return Y.charCodeAt(0).toString(16).toUpperCase()}function c(Y){return Y.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function M(Y){return Y.replace(/\\/g,"\\\\").replace(/\]/g,"\\]").replace(/\^/g,"\\^").replace(/-/g,"\\-").replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function o(Y){switch(Y.type){case"literal":return'"'+c(Y.text)+'"';case"class":let N=Y.parts.map(R=>Array.isArray(R)?M(R[0])+"-"+M(R[1]):M(R));return"["+(Y.inverted?"^":"")+N+"]";case"any":return"any character";case"end":return"end of input";case"other":return Y.description}}function a(Y){let N=Y.map(o),R,W;if(N.sort(),N.length>0){for(R=1,W=1;R<N.length;R++)N[R-1]!==N[R]&&(N[W]=N[R],W++);N.length=W}switch(N.length){case 1:return N[0];case 2:return N[0]+" or "+N[1];default:return N.slice(0,-1).join(", ")+", or "+N[N.length-1]}}function U(Y){return Y?'"'+c(Y)+'"':"end of input"}return"Expected "+a(e)+" but "+U(A)+" found."}constructor(e,A,t,c){super(),this.message=e,this.expected=A,this.found=t,this.location=c,this.name="PeggySyntaxError",typeof Object.setPrototypeOf=="function"?Object.setPrototypeOf(this,i.prototype):this.__proto__=i.prototype,typeof Error.captureStackTrace=="function"&&Error.captureStackTrace(this,i)}format(e){let A="Error: "+this.message;if(this.location){let t=null,c;for(c=0;c<e.length;c++)if(e[c].grammarSource===this.location.source){t=e[c].text.split(/\r\n|\n|\r/g);break}let M=this.location.start,o=this.location.source+":"+M.line+":"+M.column;if(t){let a=this.location.end,U=mt("",M.line.toString().length," "),Y=t[M.line-1],N=M.line===a.line?a.column:Y.length+1;A+=`
 --> `+o+`
`+U+` |
`+M.line+" | "+Y+`
`+U+" | "+mt("",M.column-1," ")+mt("",N-M.column,"^")}else A+=`
 at `+o}return A}};function Gr(i,e){e=e!==void 0?e:{};let A={},t=e.grammarSource,c={Chord:ls},M=ls,o="(",a=P("(",!1),U=")",Y=P(")",!1),N=function(E){return{type:"chord",...E,column:MA().start.column,optional:!0}},R=function(E){return{type:"chord",...E,column:MA().start.column}},W=/^[#b]/,qA=WA(["#","b"],!1,!1),me=function(E,C,m,T){let u=typeof m=="string"||m===null?{suffix:m}:{quality:m.quality,extensions:m.extensions};return{base:E,accidental:C,...u,...T,chordType:"symbol"}},_A=function(E){return{base:null,accidental:null,suffix:null,...E,chordType:"symbol"}},De=/^[A-Ha-h]/,Ne=WA([["A","H"],["a","h"]],!1,!1),lA="/",hA=P("/",!1),QA=function(E,C){return{bassBase:E,bassAccidental:C}},fA=function(E,C,m,T){let u=typeof m=="string"||m===null?{suffix:m}:{quality:m.quality,extensions:m.extensions};return{base:E,accidental:C,...u,...T,chordType:"solfege"}},Ae=function(E){return{base:null,accidental:null,suffix:null,...E,chordType:"solfege"}},JA="do",ee=P("Do",!0),je="re",se=P("Re",!0),be="mi",FA=P("Mi",!0),UA="sol",ze=P("Sol",!0),Te="la",YA=P("La",!0),SA="si",te=P("Si",!0),Z="Fa",K=P("Fa",!1),Be="dd",kA=P("dd",!0),TA="ug",ce=P("ug",!0),yA=function(){return"Fa"},mA="fa",ye=P("fa",!1),Re=function(){return"fa"},RA=function(E,C,m,T){let u=typeof m=="string"||m===null?{suffix:m}:{quality:m.quality,extensions:m.extensions};return{base:C,accidental:E,...u,...T,chordType:"numeral"}},dA=function(E){return{base:null,accidental:null,suffix:null,...E,chordType:"numeral"}},b="iii",q=P("III",!0),v="vii",k=P("VII",!0),GA="ii",L=P("II",!0),DA="iv",uA=P("IV",!0),Qs="vi",fs=P("VI",!0),ie=/^[IV]/i,tA=WA(["I","V"],!1,!0),BA=function(E,C){return{bassBase:C,bassAccidental:E}},Is=function(E,C,m,T){let u=typeof m=="string"||m===null?{suffix:m}:{quality:m.quality,extensions:m.extensions};return{base:C,accidental:E,...u,...T,chordType:"numeric"}},de=function(E){return{base:null,accidental:null,suffix:null,...E,chordType:"numeric"}},VA=/^[1-7]/,ge=WA([["1","7"]],!1,!1),IA=function(E,C){return{quality:E||null,extensions:C||null}},X="m",Me=P("m",!1),re="a",LA=P("a",!0),ne=function(){return"m"},Ge="dim",ue=P("dim",!0),oe="aug",xe=P("aug",!0),pe="sus4",He=P("sus4",!0),Ee="sus2",Je=P("sus2",!0),Se="sus",PA=P("sus",!0),ke="",NA=/^[a-zA-Z0-9#+\-o\u266D\u266F\u0394]/,xA=WA([["a","z"],["A","Z"],["0","9"],"#","+","-","o","\u266D","\u266F","\u0394"],!1,!1),Ve=function(E){return"("+E+")"},Q=0,V=0,oA=[{line:1,column:1}],I=0,F=[],j=0,d;if(e.startRule!==void 0){if(!(e.startRule in c))throw new Error(`Can't start parsing from rule "`+e.startRule+'".');M=c[e.startRule]}function _(){return i.substring(V,Q)}function MA(){return ZA(V,Q)}function cA(E,C){throw C=C!==void 0?C:ZA(V,Q),Cs([nt(E)],i.substring(V,Q),C)}function qB(E,C){throw C=C!==void 0?C:ZA(V,Q),ot(E,C)}function P(E,C){return{type:"literal",text:E,ignoreCase:C}}function WA(E,C,m){return{type:"class",parts:E,inverted:C,ignoreCase:m}}function Mt(){return{type:"any"}}function rt(){return{type:"end"}}function nt(E){return{type:"other",description:E}}function ws(E){let C=oA[E],m;if(C)return C;for(m=E-1;!oA[m];)m--;for(C=oA[m],C={line:C.line,column:C.column};m<E;)i.charCodeAt(m)===10?(C.line++,C.column=1):C.column++,m++;return oA[E]=C,C}function ZA(E,C){let m=ws(E),T=ws(C);return{source:t,start:{offset:E,line:m.line,column:m.column},end:{offset:C,line:T.line,column:T.column}}}function p(E){Q<I||(Q>I&&(I=Q,F=[]),F.push(E))}function ot(E,C){return new $e(E,[],"",C)}function Cs(E,C,m){return new $e($e.buildMessage(E,C),E,C,m)}function ls(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===40?(C=o,Q++):(C=A,j===0&&p(a)),C!==A?(m=We(),m===A&&(m=Ze(),m===A&&(m=Le(),m===A&&(m=hs()))),m!==A?(i.charCodeAt(Q)===41?(T=U,Q++):(T=A,j===0&&p(Y)),T!==A?(V=E,C=N(m),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,C=We(),C===A&&(C=Ze(),C===A&&(C=Le(),C===A&&(C=hs()))),C!==A&&(V=E,C=R(C)),E=C),E}function EA(){let E;return W.test(i.charAt(Q))?(E=i.charAt(Q),Q++):(E=A,j===0&&p(qA)),E}function hs(){let E,C,m,T,u;return E=Q,C=Fs(),C!==A?(m=EA(),m===A&&(m=null),m!==A?(T=ae(),T!==A?(u=Us(),u===A&&(u=null),u!==A?(V=E,C=me(C,m,T,u),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,C=Us(),C!==A&&(V=E,C=_A(C)),E=C),E}function Fs(){let E;return De.test(i.charAt(Q))?(E=i.charAt(Q),Q++):(E=A,j===0&&p(Ne)),E}function Us(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===47?(C=lA,Q++):(C=A,j===0&&p(hA)),C!==A?(m=Fs(),m!==A?(T=EA(),T===A&&(T=null),T!==A?(V=E,C=QA(m,T),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E}function Le(){let E,C,m,T,u;return E=Q,C=Ys(),C!==A?(m=EA(),m===A&&(m=null),m!==A?(T=ae(),T!==A?(u=Pe(),u===A&&(u=null),u!==A?(V=E,C=fA(C,m,T,u),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,C=Pe(),C!==A&&(V=E,C=Ae(C)),E=C),E}function Ys(){let E;return i.substr(Q,2).toLowerCase()===JA?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(ee)),E===A&&(i.substr(Q,2).toLowerCase()===je?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(se)),E===A&&(i.substr(Q,2).toLowerCase()===be?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(FA)),E===A&&(E=ms(),E===A&&(i.substr(Q,3).toLowerCase()===UA?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(ze)),E===A&&(i.substr(Q,2).toLowerCase()===Te?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(YA)),E===A&&(i.substr(Q,2).toLowerCase()===SA?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(te)))))))),E}function ms(){let E,C,m,T;return E=Q,i.substr(Q,2)===Z?(C=Z,Q+=2):(C=A,j===0&&p(K)),C!==A?(m=Q,j++,i.substr(Q,2).toLowerCase()===Be?(T=i.substr(Q,2),Q+=2):(T=A,j===0&&p(kA)),T===A&&(i.substr(Q,2).toLowerCase()===TA?(T=i.substr(Q,2),Q+=2):(T=A,j===0&&p(ce))),j--,T===A?m=void 0:(Q=m,m=A),m!==A?(V=E,C=yA(),E=C):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,i.substr(Q,2)===mA?(C=mA,Q+=2):(C=A,j===0&&p(ye)),C!==A?(m=Q,j++,i.substr(Q,2).toLowerCase()===Be?(T=i.substr(Q,2),Q+=2):(T=A,j===0&&p(kA)),T===A&&(i.substr(Q,2).toLowerCase()===TA?(T=i.substr(Q,2),Q+=2):(T=A,j===0&&p(ce))),j--,T===A?m=void 0:(Q=m,m=A),m!==A?(V=E,C=Re(),E=C):(Q=E,E=A)):(Q=E,E=A)),E}function Pe(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===47?(C=lA,Q++):(C=A,j===0&&p(hA)),C!==A?(m=Ys(),m!==A?(T=EA(),T===A&&(T=null),T!==A?(V=E,C=QA(m,T),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E}function We(){let E,C,m,T,u;return E=Q,C=EA(),C===A&&(C=null),C!==A?(m=Ds(),m!==A?(T=ae(),T!==A?(u=Ns(),u===A&&(u=null),u!==A?(V=E,C=RA(C,m,T,u),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,C=Ns(),C!==A&&(V=E,C=dA(C)),E=C),E}function Ds(){let E;return i.substr(Q,3).toLowerCase()===b?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(q)),E===A&&(i.substr(Q,3).toLowerCase()===v?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(k)),E===A&&(i.substr(Q,2).toLowerCase()===GA?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(L)),E===A&&(i.substr(Q,2).toLowerCase()===DA?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(uA)),E===A&&(i.substr(Q,2).toLowerCase()===Qs?(E=i.substr(Q,2),Q+=2):(E=A,j===0&&p(fs)),E===A&&(ie.test(i.charAt(Q))?(E=i.charAt(Q),Q++):(E=A,j===0&&p(tA))))))),E}function Ns(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===47?(C=lA,Q++):(C=A,j===0&&p(hA)),C!==A?(m=EA(),m===A&&(m=null),m!==A?(T=Ds(),T!==A?(V=E,C=BA(m,T),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E}function Ze(){let E,C,m,T,u;return E=Q,C=EA(),C===A&&(C=null),C!==A?(m=js(),m!==A?(T=ae(),T!==A?(u=bs(),u===A&&(u=null),u!==A?(V=E,C=Is(C,m,T,u),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E===A&&(E=Q,C=bs(),C!==A&&(V=E,C=de(C)),E=C),E}function js(){let E;return VA.test(i.charAt(Q))?(E=i.charAt(Q),Q++):(E=A,j===0&&p(ge)),E}function bs(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===47?(C=lA,Q++):(C=A,j===0&&p(hA)),C!==A?(m=EA(),m===A&&(m=null),m!==A?(T=js(),T!==A?(V=E,C=BA(m,T),E=C):(Q=E,E=A)):(Q=E,E=A)):(Q=E,E=A),E}function ae(){let E,C,m,T,u;if(E=Q,C=Et(),C!==A){for(m=Q,T=[],u=zs();u!==A;)T.push(u),u=zs();T!==A?m=i.substring(m,Q):m=T,m!==A?(V=E,C=IA(C,m),E=C):(Q=E,E=A)}else Q=E,E=A;return E}function Et(){let E,C,m,T;return E=Q,i.charCodeAt(Q)===109?(C=X,Q++):(C=A,j===0&&p(Me)),C!==A?(m=Q,j++,i.substr(Q,1).toLowerCase()===re?(T=i.charAt(Q),Q++):(T=A,j===0&&p(LA)),j--,T===A?m=void 0:(Q=m,m=A),m!==A?(V=E,C=ne(),E=C):(Q=E,E=A)):(Q=E,E=A),E===A&&(i.substr(Q,3).toLowerCase()===Ge?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(ue)),E===A&&(i.substr(Q,3).toLowerCase()===oe?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(xe)),E===A&&(i.substr(Q,4).toLowerCase()===pe?(E=i.substr(Q,4),Q+=4):(E=A,j===0&&p(He)),E===A&&(i.substr(Q,4).toLowerCase()===Ee?(E=i.substr(Q,4),Q+=4):(E=A,j===0&&p(Je)),E===A&&(i.substr(Q,3).toLowerCase()===Se?(E=i.substr(Q,3),Q+=3):(E=A,j===0&&p(PA)),E===A&&(E=ke)))))),E}function zs(){let E,C,m,T,u;if(E=Q,i.charCodeAt(Q)===40?(C=o,Q++):(C=A,j===0&&p(a)),C!==A){if(m=Q,T=[],NA.test(i.charAt(Q))?(u=i.charAt(Q),Q++):(u=A,j===0&&p(xA)),u!==A)for(;u!==A;)T.push(u),NA.test(i.charAt(Q))?(u=i.charAt(Q),Q++):(u=A,j===0&&p(xA));else T=A;T!==A?m=i.substring(m,Q):m=T,m!==A?(i.charCodeAt(Q)===41?(T=U,Q++):(T=A,j===0&&p(Y)),T!==A?(V=E,C=Ve(m),E=C):(Q=E,E=A)):(Q=E,E=A)}else Q=E,E=A;if(E===A){if(E=Q,C=[],NA.test(i.charAt(Q))?(m=i.charAt(Q),Q++):(m=A,j===0&&p(xA)),m!==A)for(;m!==A;)C.push(m),NA.test(i.charAt(Q))?(m=i.charAt(Q),Q++):(m=A,j===0&&p(xA));else C=A;C!==A?E=i.substring(E,Q):E=C}return E}if(d=M(),d!==A&&Q===i.length)return d;throw d!==A&&Q<i.length&&p(rt()),Cs(F,I<i.length?i.charAt(I):null,I<i.length?ZA(I,I+1):ZA(I,I))}var ur=Gr,Vt=class i{get suffix(){return this._quality!==null||this._extensions!==null?(this._quality||"")+(this._extensions||""):this._suffix}get quality(){return this._quality}get extensions(){return this._extensions}static parse(e,A={}){try{return this.parseOrFail(e,A)}catch{return null}}static parseOrFail(e,A={}){let t=e.trim();try{let c=ur(t);return new i({...c,notation:A.notation??null})}catch(c){let M=c;throw new St(`Failed parsing '${t}': ${M.message}`)}}clone(){return this.set({})}toChordSymbol(e=null){if(this.isChordSymbol())return this.clone();let{keyObj:A,referenceIsMinor:t}=this.prepareKeyForConversion(e),c=new i({suffix:this.normalizedSuffix,root:this.root?.toChordSymbol(A,t)||null,bass:this.bass?.toChordSymbol(A,t)||null,optional:this.optional});return this.finalizeConvertedChord(c,A)}toChordSymbolString(e=null){return this.toChordSymbol(e).toString()}isChordSymbol(){return this.is(aA)}toChordSolfege(e=null){if(this.isChordSolfege())return this.clone();let{keyObj:A}=this.prepareKeyForConversion(e),t=new i({suffix:this.normalizedSuffix,root:this.root?.toChordSolfege(A)||null,bass:this.bass?.toChordSolfege(A)||null,optional:this.optional});return this.finalizeConvertedChord(t,e)}toChordSolfegeString(e=null){return this.toChordSolfege(e).toString()}isChordSolfege(){return this.is(bA)}toNumeric(e=null){if(this.isNumeric())return this.clone();if(this.isNumeral())return this.transform(t=>t.toNumeric());let A=eA.wrap(e);return A&&A.isMinor()&&(A=A.relativeMajor),new i({suffix:xs(this.suffix),root:this.root?.toNumeric(A)||null,bass:this.bass?.toNumeric(A)||null,...this.optional?{optional:!0}:{}})}toNumeral(e=null){if(this.isNumeral())return this.clone();if(this.isNumeric())return this.transform(t=>t.toNumeral());let A=eA.wrap(e);return A&&A.isMinor()&&(A=A.relativeMajor),new i({suffix:xs(this.suffix),root:A&&this.root?this.root.toNumeral(A):null,bass:this.bass?.toNumeral(A)||null,...this.optional?{optional:!0}:{}})}toNumeralString(e=null){return this.toNumeral(e).toString()}isNumeric(){return this.is(CA)}toNumericString(e=null){return this.toNumeric(e).toString()}isNumeral(){return this.is(zA)}toString({useUnicodeModifier:e=!1}={}){let A="",t=this.suffix||"",c=t[0]!=="m";return e&&(t=t.replace(/#(?=\d)/g,"\u266F").replace(/b(?=\d)/g,"\u266D")),this.root&&(A=this.root.toString({showMinor:c,useUnicodeModifier:e})+t),this.bass&&(A=`${A}/${this.bass.toString({useUnicodeModifier:e})}`),this.optional&&(A=`(${A})`),A}normalize(e=null,{normalizeSuffix:A=!0}={}){let t=A?xs(this.suffix):this.suffix,c=this.root;return this.root&&(c=this.root.normalize(),e&&(c=c.normalizeEnharmonics(e))),this.set({suffix:t,root:c,bass:this.bass?this.bass.normalize().normalizeEnharmonics(c):null})}useAccidental(e){return this.transform(A=>A.useAccidental(e))}useModifier(e){return HA("useModifier is deprecated, use useAccidental instead"),this.useAccidental(e)}transposeUp(){return this.transform(e=>e.transposeUp())}transposeDown(){return this.transform(e=>e.transposeDown())}transpose(e){return this.transform(A=>A.transpose(e))}constructor(e){this._quality=e.quality??null,this._extensions=e.extensions??null,this._suffix=e.suffix??null,this.optional=e.optional??!1,this.root=i.determineRoot({...e,suffix:this.suffix}),this.bass=i.determineBass(e)}equals(e){return this.suffix===e.suffix&&this.optional===e.optional&&eA.equals(this.root,e.root)&&eA.equals(this.bass,e.bass)}static determineRoot(e){let{root:A,base:t,accidental:c,suffix:M,chordType:o,notation:a}=e;if(A)return A;if(!t)return null;if(!o)throw new Error("Can't resolve at this point without a chord type");return eA.resolve({key:t,keyType:o,minor:Wr(t,o,M??null),accidental:c??null,preferredNotation:a??null})}static determineBass(e){let{bass:A,bassBase:t,bassAccidental:c,chordType:M,notation:o}=e;if(A)return A;if(!t)return null;if(!M)throw new Error("Can't resolve at this point without a chord type");return eA.resolve({key:t,accidental:c??null,minor:!1,keyType:M,preferredNotation:o??null})}isMinor(){return this.root?.isMinor()||!1}makeMinor(){return this.set({root:this.root?.makeMinor()||null})}set(e){let A=this.determineSuffixProps(e);return new i({root:this.root?.clone()||null,...A,bass:this.bass?.clone()||null,...this.optional?{optional:!0}:{},...e})}determineSuffixProps(e){return"suffix"in e?{suffix:e.suffix,quality:null,extensions:null}:"quality"in e||"extensions"in e?{quality:e.quality??this._quality,extensions:e.extensions??this._extensions}:this._quality!==null||this._extensions!==null?{quality:this._quality,extensions:this._extensions}:{suffix:this._suffix}}is(e){return(!this.root||this.root.is(e))&&(!this.bass||this.bass.is(e))}transform(e){return this.set({root:this.root?e(this.root):null,bass:this.bass?e(this.bass):null})}get normalizedSuffix(){return this.suffix?xs(this.suffix):null}prepareKeyForConversion(e){let A=eA.wrap(e),t=A?.isMinor()||!1;return{keyObj:t?A?.relativeMajor||null:A,referenceIsMinor:t}}finalizeConvertedChord(e,A){let t=e;return this.root?.isMinor()&&(t=t.makeMinor()),t.normalize(A)}},$A=Vt,$s=class i{static[Symbol.hasInstance](e){return as(e,Ri)}constructor(e="",A=null,t=null,c=null,M=!1){this.parentLine=null,this._chordObj=null,this.chords=e||"",this.lyrics=A||"",this.annotation=t||"",this._chordObj=c,this.isRhythmSymbol=M}get chord(){return this._chordObj||$A.parse(this.chords.trim())}isRenderable(){return!0}hasLyrics(){return!!(this.lyrics&&this.lyrics.trim().length>0)}clone(){let e=this._chordObj?.clone()||null;return new i(this.chords,this.lyrics,this.annotation,e,this.isRhythmSymbol)}toString(){return`ChordLyricsPair(chords=${this.chords}, lyrics=${this.lyrics})`}set({chords:e,lyrics:A,annotation:t,chordObj:c,isRhythmSymbol:M}){return new i(e??this.chords,A??this.lyrics,t??this.annotation,c??null,M??this.isRhythmSymbol)}setLyrics(e){return this.set({lyrics:e})}setAnnotation(e){return this.set({annotation:e})}transpose(e,A=null,{normalizeChordSuffix:t}={normalizeChordSuffix:!1}){return this.changeChord(c=>{let M=c.transpose(e);return A?M.normalize(A,{normalizeSuffix:t}):M})}useAccidental(e){return this.changeChord(A=>A.useAccidental(e))}useModifier(e){return HA("useModifier is deprecated, use useAccidental instead"),this.useAccidental(e)}changeChord(e){let A=this.chord;if(A){let t=e(A);return this.set({chords:t.toString(),chordObj:t})}return this.clone()}};Es($s.prototype,Ri);var H=$s,xr={2:"2",4:"sus",5:"5",6:"6",7:"7",9:"9",11:"11",13:"13",42:"4(2)",69:"6(9)","(#4)":"(#4)","#4":"(#4)","+4":"(#4)","(+4)":"(#4)","(11)":"(11)",add11:"(11)","(add11)":"(11)","(13)":"(13)",add13:"(13)","(add13)":"(13)","(2)":"(2)",add2:"(2)","(add2)":"(2)","(4)":"(4)",add4:"(4)","(add4)":"(4)","sus(4)":"(4)","(6)":"(6)",add6:"(6)","(add6)":"(6)","(7)":"(7)","(9)":"(9)",add9:"(9)","(add9)":"(9)","(b5)":"(b5)","-5":"(b5)","(-5)":"(b5)",b5:"(b5)","2(6)":"2(6)","(b6)":"(b6)","(unis)":"(unis)",unis:"(unis)","[blank]":"[blank]",maj:"[blank]",major:"[blank]",M:"[blank]",ma:"[blank]",Ma:"[blank]",Majj:"[blank]","+":"+",aug:"+","(#5)":"+","#5":"+","+5":"+","(+5)":"+",x:"+",dom11:"11","11(#5)":"11(#5)","11#5":"11(#5)","11+5":"11(#5)","11(+5)":"11(#5)","11(#9)":"11(#9)","11#9":"11(#9)","11+9":"11(#9)","11(+9)":"11(#9)","11(b13)":"11(b13)","11b13":"11(b13)","11-13":"11(b13)","11(-13)":"11(b13)","11(b5)":"11(b5)","11b5":"11(b5)","11-5":"11(b5)","11(-5)":"11(b5)","11(b9)":"11(b9)","11b9":"11(b9)","11-9":"11(b9)","11(-9)":"11(b9)","11sus4":"11sus4","11sus":"11sus4",m11sus4:"11sus4",m11sus:"11sus4",dom13:"13","13(#11)":"13(#11)","13#11":"13(#11)","13+11":"13(#11)","13(+11)":"13(#11)","13(#5)":"13(#5)","13#5":"13(#5)","13+5":"13(#5)","13(+5)":"13(#5)","13(#9)":"13(#9)","13#9":"13(#9)","13(+9)":"13(#9)","13+9":"13(#9)","13(#9#5)":"13(#9#5)","13#9#5":"13(#9#5)","13(+9+5)":"13(#9#5)","13+9+5":"13(#9#5)","13(#9b5)":"13(#9b5)","13#9b5":"13(#9b5)","13(+9-5)":"13(#9b5)","13+9-5":"13(#9b5)","13(add2)":"13(add2)","13(add4)":"13(add4)","13(b5)":"13(b5)","13b5":"13(b5)","13-5":"13(b5)","13(-5)":"13(b5)","13(b9)":"13(b9)","13b9":"13(b9)","13-9":"13(b9)","13(-9)":"13(b9)","13(b9#5)":"13(b9#5)","13b9#5":"13(b9#5)","13-9+5":"13(b9#5)","13(-9+5)":"13(b9#5)","13(b9b5)":"13(b9b5)","13b9b5":"13(b9b5)","13-9-5":"13(b9b5)","13(-9-5)":"13(b9b5)","13sus4":"13sus4","13sus":"13sus4",m13sus4:"13sus4",m13sus:"13sus4",sus2:"2","add9(no3)":"2","2(#11)":"2(#11)","2(#4)":"2(#4)","2+4":"2(#4)","2#4":"2(#4)","2(+4)":"2(#4)","(#4)2":"2(#4)","2(#4)(#42)(2#4)":"2(#4)(#42)(2#4)","2(+7)":"2(+7)","2(4)":"2(4)","sus2(4)":"2(4)","2(ma7)":"2(ma7)","2(no3)":"2(no3)","4(2)":"4(2)",sus42:"4(2)",no3:"5","(no3)":"5","5(no3)":"5","6(2)":"6(2)","6(b9)":"6(b9)","6(no3)":"6(no3)","6(9)":"6(9)","6(add9)":"6(9)",dom7:"7","7(#11)":"7(#11)","7#11":"7(#11)","7+11":"7(#11)","7(+11)":"7(#11)","7(#5)":"7(#5)",aug7:"7(#5)","7#5":"7(#5)","7+5":"7(#5)","7(+5)":"7(#5)",x7:"7(#5)","7(#5#11)":"7(#5#11)","7#5#11":"7(#5#11)","7+5+11":"7(#5#11)","7(+5+11)":"7(#5#11)","7(#9)":"7(#9)","7#9":"7(#9)","7+9":"7(#9)","7(+9)":"7(#9)","7(#9#5)":"7(#9#5)","7(#5#9)":"7(#9#5)","7#5#9":"7(#9#5)","7+5+9":"7(#9#5)","7(+5+9)":"7(#9#5)","7(#9b13)":"7(#9b13)","7#9b13":"7(#9b13)","7(+9-13)":"7(#9b13)","7+9-13":"7(#9b13)","7(b13#9)":"7(#9b13)","7b13#9":"7(#9b13)","7-13+9":"7(#9b13)","7(-13+9)":"7(#9b13)","7(#9b5)":"7(#9b5)","7(6)":"7(6)","7(b13)":"7(b13)","7b13":"7(b13)","7-13":"7(b13)","7(-13)":"7(b13)","7(b5)":"7(b5)","7b5":"7(b5)","7-5":"7(b5)","7(-5)":"7(b5)","7(b5#11)":"7(b5#11)","7b5#11":"7(b5#11)","7-5+11":"7(b5#11)","7(-5+11)":"7(b5#11)","7(b5#9)":"7(b5#9)","7b5#9":"7(b5#9)","7-5+9":"7(b5#9)","7(-5+9)":"7(b5#9)","7(b5b9)":"7(b5b9)","7b5b9":"7(b5b9)","7-5-9":"7(b5b9)","7(-5-9)":"7(b5b9)","7(b9)":"7(b9)","7b9":"7(b9)","7-9":"7(b9)","7(-9)":"7(b9)","7(b9#5)":"7(b9#5)","7(#5b9)":"7(b9#5)","7#5b9":"7(b9#5)","7+5-9":"7(b9#5)","7(+5-9)":"7(b9#5)","7(b9b13)":"7(b9b13)","7b9b13":"7(b9b13)","7-9-13":"7(b9b13)","7(-9-13)":"7(b9b13)","7(b13b9)":"7(b9b13)","7b13b9":"7(b9b13)","7-13-9":"7(b9b13)","7(-13-9)":"7(b9b13)","7(b9b5)":"7(b9b5)","7(no3)":"7(no3)","7aug5":"7aug5","7b9sus4":"7b9sus4","7sus(6)":"7sus(6)","7sus(b9)":"7sus(b9)","7sus4":"7sus4","7sus":"7sus4",dom9:"9","9(#11)":"9(#11)","9#11":"9(#11)","9+11":"9(#11)","9(+11)":"9(#11)","9(#5)":"9(#5)","9#5":"9(#5)","9+5":"9(#5)","9(+5)":"9(#5)",aug9:"9(#5)",x9:"9(#5)","9(b13)":"9(b13)","9b13":"9(b13)","9-13":"9(b13)","9(-13)":"9(b13)","9(b5)":"9(b5)","9b5":"9(b5)","9-5":"9(b5)","9(-5)":"9(b5)","9aug":"9aug","9sus4":"9sus4","9sus":"9sus4",m9sus4:"9sus4",m9sus:"9sus4","b69(#11)":"b69(#11)",b69sus:"b69sus",b9sus:"b9sus",dim:"dim","m(b5)":"dim",mb5:"dim","m-5":"dim","m(-5)":"dim","-(b5)":"dim","-b5":"dim",dim7:"dim7",o7:"dim7",m:"m",mi:"m",min:"m",minor:"m","-":"m","m(11)":"m(11)","m(add11)":"m(11)","m(4)":"m(4)","m(add4)":"m(4)","m(9)":"m(9)","m(add9)":"m(9)","m(ma7)":"m(ma7)","m(M7)":"m(ma7)",mM7:"m(ma7)","mi(maj7)":"m(ma7)","min(maj7)":"m(ma7)","m(maj7)":"m(ma7)","m(+7)":"m(ma7)","m+7":"m(ma7)",mmaj7:"m(ma7)","m(ma9)":"m(ma9)","m(M9)":"m(ma9)",mM9:"m(ma9)","mi(maj9)":"m(ma9)","min(maj9)":"m(ma9)","m(maj9)":"m(ma9)","m(+9)":"m(ma9)","m+9":"m(ma9)","m(no5)":"m(no5)",m11:"m11",mi11:"m11",min11:"m11","-11":"m11","m11(#5)":"m11(#5)","m11#5":"m11(#5)","m11+5":"m11(#5)","m11(+5)":"m11(#5)","-11(#5)":"m11(#5)","-11#5":"m11(#5)","m9+5":"m9(#5)","-11(+5)":"m11(#5)","m11(#9)":"m11(#9)","m11#9":"m11(#9)","m11+9":"m11(#9)","m11(+9)":"m11(#9)","-11(#9)":"m11(#9)","-11#9":"m11(#9)","-11+9":"m11(#9)","-11(+9)":"m11(#9)","m11(#9#5)":"m11(#9#5)","m11(#5#9)":"m11(#9#5)","m11#5#9":"m11(#9#5)","m11+5+9":"m11(#9#5)","m11(+5+9)":"m11(#9#5)","-11(#5#9)":"m11(#9#5)","-11#5#9":"m11(#9#5)","m9+5+9":"m11(#9#5)","-11(+5+9)":"m11(#9#5)","m11(#9b13)":"m11(#9b13)","m11#9b13":"m11(#9b13)","m11+9-13":"m11(#9b13)","m11(+9-13)":"m11(#9b13)","-11(#9b13)":"m11(#9b13)","-11#9b13":"m11(#9b13)","-11+9-13":"m11(#9b13)","-11(+9-13)":"m11(#9b13)","m11(b13)":"m11(b13)",m11b13:"m11(b13)","m11-13":"m11(b13)","m11(-13)":"m11(b13)","-11(b13)":"m11(b13)","-11b13":"m11(b13)","-11-13":"m11(b13)","-11(-13)":"m11(b13)","m11(b13#5)":"m11(b13#5)","m11(#5b13)":"m11(b13#5)","m11#5b13":"m11(b13#5)","m11+5-13":"m11(b13#5)","m11(+5-13)":"m11(b13#5)","-11(#5b13)":"m11(b13#5)","-11#5b13":"m11(b13#5)","-11+5-13":"m11(b13#5)","-11(+5-13)":"m11(b13#5)","m11(b5)":"m11(b5)",m11b5:"m11(b5)","m11-5":"m11(b5)","m11(-5)":"m11(b5)","-11(b5)":"m11(b5)","-11b5":"m11(b5)","-9-5":"m9(b5)","-11(-5)":"m11(b5)","m11(b5#9)":"m11(b5#9)","m11b5#9":"m11(b5#9)","m11-5+9":"m11(b5#9)","m11(-5+9)":"m11(b5#9)","-11(b5#9)":"m11(b5#9)","-11b5#9":"m11(b5#9)","-11-5+9":"m11(b5#9)","-11(-5+9)":"m11(b5#9)","m11(b5b13)":"m11(b5b13)",m11b5b13:"m11(b5b13)","m11(-5-13)":"m11(b5b13)","m11-5-13":"m11(b5b13)","-11(b5b13)":"m11(b5b13)","-11b5b13":"m11(b5b13)","-11(-5-13)":"m11(b5b13)","-11-5-13":"m11(b5b13)","m11(b5b9)":"m11(b5b9)",m11b5b9:"m11(b5b9)","m11-5-9":"m11(b5b9)","m11(-5-9)":"m11(b5b9)","-11(b5b9)":"m11(b5b9)","-11b5b9":"m11(b5b9)","-11-5-9":"m11(b5b9)","-11(-5-9)":"m11(b5b9)","m11(b9)":"m11(b9)",m11b9:"m11(b9)","m11(-9)":"m11(b9)","m11-9":"m11(b9)","-11(b9)":"m11(b9)","-11b9":"m11(b9)","-11(-9)":"m11(b9)","-11-9":"m11(b9)","m11(b9#5)":"m11(b9#5)","m11(#5b9)":"m11(b9#5)","m11#5b9":"m11(b9#5)","m11+5-9":"m11(b9#5)","m11(+5-9)":"m11(b9#5)","-11(#5b9)":"m11(b9#5)","-11#5b9":"m11(b9#5)","m9+5-9":"m11(b9#5)","-11(+5-9)":"m11(b9#5)","m11(b9b13)":"m11(b9b13)",m11b9b13:"m11(b9b13)","m11(-9-13)":"m11(b9b13)","m11-9-13":"m11(b9b13)","-11(b9b13)":"m11(b9b13)","-11b9b13":"m11(b9b13)","-11(-9-13)":"m11(b9b13)","-11-9-13":"m11(b9b13)",m13:"m13",mi13:"m13",min13:"m13","-13":"m13","m13(#11)":"m13(#11)","m13#11":"m13(#11)","m13+11":"m13(#11)","m13(+11)":"m13(#11)","-13(#11)":"m13(#11)","-13#11":"m13(#11)","-13+11":"m13(#11)","-13(+11)":"m13(#11)","m13(#5)":"m13(#5)","m13#5":"m13(#5)","m13+5":"m13(#5)","m13(+5)":"m13(#5)","-13(#5)":"m13(#5)","-13#5":"m13(#5)","-13(+5)":"m13(#5)","m13(#9)":"m13(#9)","m13#9":"m13(#9)","m13(+9)":"m13(#9)","m13+9":"m13(#9)","-13(#9)":"m13(#9)","-13#9":"m13(#9)","-13(+9)":"m13(#9)","-13+9":"m13(#9)","m13(b5)":"m13(b5)",m13b5:"m13(b5)","m13-5":"m13(b5)","m13(-5)":"m13(b5)","-13(b5)":"m13(b5)","-13b5":"m13(b5)","-13-5":"m13(b5)","-13(-5)":"m13(b5)","m13(b9)":"m13(b9)",m13b9:"m13(b9)","m13-9":"m13(b9)","m13(-9)":"m13(b9)","-13(b13)":"m13(b9)","-13b13":"m13(b9)","-13-13":"m13(b9)","-13(-13)":"m13(b9)",m2:"m2",mi2:"m2",min2:"m2","m(add2)":"m2",madd2:"m2",m4:"m4",m6:"m6",mi6:"m6",min6:"m6","-6":"m6","m6(#5)":"m6(#5)","m6(9)":"m6(9)",m6add9:"m6(9)","m6(add9)":"m6(9)",m69:"m6(9)","m6(ma7)":"m6(ma7)","m6(M7)":"m6(ma7)","m6(+7)":"m6(ma7)",m6M7:"m6(ma7)","m6+7":"m6(ma7)",m7:"m7",mi7:"m7",min7:"m7","-7":"m7","m7(#11)":"m7(#11)","m7#11":"m7(#11)","m7+11":"m7(#11)","m7(+11)":"m7(#11)","-7(#11)":"m7(#11)","-7#11":"m7(#11)","-7+11":"m7(#11)","-7(+11)":"m7(#11)","m7(#5)":"m7(#5)","m7#5":"m7(#5)","m7+5":"m7(#5)","m7(+5)":"m7(#5)","-7(#5)":"m7(#5)","-7#5":"m7(#5)","-7(+5)":"m7(#5)","m7(#9)":"m7(#9)","m7#9":"m7(#9)","m7+9":"m7(#9)","m7(+9)":"m7(#9)","-7(#9)":"m7(#9)","-7#9":"m7(#9)","-7(+9)":"m7(#9)","m7(11)":"m7(11)","m7(4)":"m7(4)","m7(add4)":"m7(4)","m7(add11)":"m7(4)",m74:"m7(4)","m7(6)":"m7(6)","m7(add6)":"m7(add6)","m7(b13)":"m7(b13)",m7b13:"m7(b13)","m7-13":"m7(b13)","m7(-13)":"m7(b13)","-7(b13)":"m7(b13)","-7b13":"m7(b13)","-7-13":"m7(b13)","-7(-13)":"m7(b13)","m7(b5)":"m7(b5)",m7b5:"m7(b5)","m7-5":"m7(b5)","m7(-5)":"m7(b5)","-7(b5)":"m7(b5)","-7b5":"m7(b5)","-7(-5)":"m7(b5)","m7(b9)":"m7(b9)",m7b9:"m7(b9)","m7-9":"m7(b9)","m7(-9)":"m7(b9)","-7(b9)":"m7(b9)","-7b9":"m7(b9)","-7(-9)":"m7(b9)","m7(no3)":"m7(no3)","min7(no3)":"m7(no3)",m7sus4:"m7sus4",m7sus:"m7sus4",m9:"m9",mi9:"m9",min9:"m9","-9":"m9","m9(#11)":"m9(#11)","m9#11":"m9(#11)","m9+11":"m9(#11)","m9(+11)":"m9(#11)","-9(#11)":"m9(#11)","-9#11":"m9(#11)","-9+11":"m9(#11)","-9(+11)":"m9(#11)","m9(#5)":"m9(#5)","m9#5":"m9(#5)","m9(+5)":"m9(#5)","-9(#5)":"m9(#5)","-9#5":"m9(#5)","-9(+5)":"m9(#5)","m9(#7)":"m9(#7)","m9(b13)":"m9(b13)",m9b13:"m9(b13)","m9-13":"m9(b13)","m9(-13)":"m9(b13)","-9(b13)":"m9(b13)","-9b13":"m9(b13)","-9-13":"m9(b13)","-9(-13)":"m9(b13)","m9(b5)":"m9(b5)",m9b5:"m9(b5)","m9-5":"m9(b5)","m9(-5)":"m9(b5)","-9(b5)":"m9(b5)","-9b5":"m9(b5)","-9(-5)":"m9(b5)","m9(ma7)":"m9(ma7)","m9(maj7)":"m9(ma7)","m9(+7)":"m9(ma7)","m9(M7)":"m9(ma7)",m9M7:"m9(ma7)",ma9:"ma9",maj9:"ma9",maj9b11:"maj9b11",ma11:"ma11","11(#7)":"ma11","11#7":"ma11","11+7":"ma11","11(+7)":"ma11","+11":"ma11",M11:"ma11","ma11(#5)":"ma11(#5)","maj11#5":"ma11(#5)","maj11+5":"ma11(#5)","maj11(+5)":"ma11(#5)","+11(#5)":"ma11(#5)","+11#5":"ma11(#5)","M11+5":"ma11(#5)","+11(+5)":"ma11(#5)","maj11(#5)":"ma11(#5)","ma11#5":"ma11(#5)","ma11+5":"ma11(#5)","ma11(+5)":"ma11(#5)","ma11(#9)":"ma11(#9)","maj11#9":"ma11(#9)","maj11+9":"ma11(#9)","maj11(+9)":"ma11(#9)","+11(#9)":"ma11(#9)","+11#9":"ma11(#9)","M11+9":"ma11(#9)","+11(+9)":"ma11(#9)","maj11(#9)":"ma11(#9)","ma11#9":"ma11(#9)","ma11+9":"ma11(#9)","ma11(+9)":"ma11(#9)","ma11(b13)":"ma11(b13)",maj11b13:"ma11(b13)","maj11-13":"ma11(b13)","maj11(-13)":"ma11(b13)","+11(b13)":"ma11(b13)","+11b13":"ma11(b13)","M11-13":"ma11(b13)","+11(-13)":"ma11(b13)","maj11(b13)":"ma11(b13)",ma11b13:"ma11(b13)","ma11-13":"ma11(b13)","ma11(-13)":"ma11(b13)","ma11(b5)":"ma11(b5)",maj11b5:"ma11(b5)","maj11-5":"ma11(b5)","maj11(-5)":"ma11(b5)","+11(b5)":"ma11(b5)","+11b5":"ma11(b5)","M11-5":"ma11(b5)","+11(-5)":"ma11(b5)","maj11(b5)":"ma11(b5)",ma11b5:"ma11(b5)","ma11-5":"ma11(b5)","ma11(-5)":"ma11(b5)","ma11(b9)":"ma11(b9)",maj11b9:"ma11(b9)","maj11(-9)":"ma11(b9)","maj11-9":"ma11(b9)","+11(b9)":"ma11(b9)","+11b9":"ma11(b9)","M11(-9)":"ma11(b9)","+11-9":"ma11(b9)","maj11(b9)":"ma11(b9)",ma11b9:"ma11(b9)","ma11(-9)":"ma11(b9)","ma11-9":"ma11(b9)",ma13:"ma13","13(#7)":"ma13","+13":"ma13",M13:"ma13",maj13:"ma13",Maj13:"ma13","ma13(#11)":"ma13(#11)","maj13#11":"ma13(#11)","maj13+11":"ma13(#11)","maj13(+11)":"ma13(#11)","+13(#11)":"ma13(#11)","+13#11":"ma13(#11)","M13+11":"ma13(#11)","+13(+11)":"ma13(#11)","maj13(#11)":"ma13(#11)","ma7#11":"ma9(#11)","ma13+11":"ma13(#11)","ma13(+11)":"ma13(#11)","ma13(#11#5)":"ma13(#11#5)","maj13#11#5":"ma13(#11#5)","maj13+11+5":"ma13(#11#5)","maj13(+11+5)":"ma13(#11#5)","+13(#11#5)":"ma13(#11#5)","+13#11#5":"ma13(#11#5)","M13+11+5":"ma13(#11#5)","+13(+11+5)":"ma13(#11#5)","maj13(#11#5)":"ma13(#11#5)","ma7#11#5":"ma9(#11#5)","ma13+11+5":"ma13(#11#5)","ma13(+11+5)":"ma13(#11#5)","ma13(#5)":"ma13(#5)","maj13#5":"ma13(#5)","maj13+5":"ma13(#5)","maj13(+5)":"ma13(#5)","+13(#5)":"ma13(#5)","+13#5":"ma13(#5)","M13+5":"ma13(#5)","+13(+5)":"ma13(#5)","maj13(#5)":"ma13(#5)","ma13#5":"ma13(#5)","ma13+5":"ma13(#5)","ma13(+5)":"ma13(#5)","ma13(#9)":"ma13(#9)","maj13#9":"ma13(#9)","maj13(+9)":"ma13(#9)","maj13+9":"ma13(#9)","+13(#9)":"ma13(#9)","+13#9":"ma13(#9)","M13(+9)":"ma13(#9)","+13+9":"ma13(#9)","maj13(#9)":"ma13(#9)","ma13#9":"ma13(#9)","ma13(+9)":"ma13(#9)","ma13+9":"ma13(#9)","ma13(#9#5)":"ma13(#9#5)","maj13#9#5":"ma13(#9#5)","maj13(+9+5)":"ma13(#9#5)","maj13+9+5":"ma13(#9#5)","+13(#9#5)":"ma13(#9#5)","+13#9#5":"ma13(#9#5)","M13(+9+5)":"ma13(#9#5)","+13+9+5":"ma13(#9#5)","maj13(#9#5)":"ma13(#9#5)","ma7#9#5":"ma13(#9#5)","ma13(+9+5)":"ma13(#9#5)","ma13+9+5":"ma13(#9#5)","ma13(b5)":"ma13(b5)",maj13b5:"ma13(b5)","maj13-5":"ma13(b5)","maj13(-5)":"ma13(b5)","+13(b5)":"ma13(b5)","+13b5":"ma13(b5)","M13-5":"ma13(b5)","+13(-5)":"ma13(b5)","maj13(b5)":"ma13(b5)",ma13b5:"ma13(b5)","ma13-5":"ma13(b5)","ma13(-5)":"ma13(b5)","ma13(b9)":"ma13(b9)",maj13b9:"ma13(b9)","maj13-9":"ma13(b9)","maj13(-9)":"ma13(b9)","+13(b9)":"ma13(b9)","+13b9":"ma13(b9)","M13-9":"ma13(b9)","+13(-9)":"ma13(b9)","maj13(b9)":"ma13(b9)",ma13b9:"ma13(b9)","ma13-9":"ma13(b9)","ma13(-9)":"ma13(b9)","ma13(b9#5)":"ma13(b9#5)","maj13b9#5":"ma13(b9#5)","maj13-9+5":"ma13(b9#5)","maj13(-9+5)":"ma13(b9#5)","+13(b9#5)":"ma13(b9#5)","+13b9#5":"ma13(b9#5)","M13-9+5":"ma13(b9#5)","+13(-9+5)":"ma13(b9#5)","maj13(b9#5)":"ma13(b9#5)","ma7b9#5":"ma13(b9#5)","ma13-9+5":"ma13(b9#5)","ma13(-9+5)":"ma13(b9#5)",ma6:"ma6","ma6(9)":"ma6(9)",ma69:"ma6(9)",ma7:"ma7","+7":"ma7","#7":"ma7",M7:"ma7",Maj7:"ma7",maj7:"ma7","(triangle)":"ma7","ma7(#11)":"ma7(#11)","maj7#11":"ma7(#11)","maj7+11":"ma7(#11)","maj7(+11)":"ma7(#11)","+7(#11)":"ma7(#11)","+7#11":"ma7(#11)","M7+11":"ma7(#11)","+7(+11)":"ma7(#11)","maj7(#11)":"ma7(#11)","ma7+11":"ma7(#11)","ma7(+11)":"ma7(#11)","ma7(#4)":"ma7(#4)","ma7(#5)":"ma7(#5)","maj7#5":"ma7(#5)","maj7+5":"ma7(#5)","maj7(+5)":"ma7(#5)","+7(#5)":"ma7(#5)","+7#5":"ma7(#5)","M7+5":"ma7(#5)","+7(+5)":"ma7(#5)","maj7(#5)":"ma7(#5)","ma7#5":"ma7(#5)","ma7+5":"ma7(#5)","ma7(+5)":"ma7(#5)","aug(M7)":"ma7(#5)","aug(+7)":"ma7(#5)","aug(ma7)":"ma7(#5)","aug(maj7)":"ma7(#5)","ma7(#9)":"ma7(#9)","maj7#9":"ma7(#9)","maj7+9":"ma7(#9)","maj7(+9)":"ma7(#9)","+7(#9)":"ma7(#9)","+7#9":"ma7(#9)","M7+9":"ma7(#9)","+7(+9)":"ma7(#9)","maj7(#9)":"ma7(#9)","ma7#9":"ma7(#9)","ma7+9":"ma7(#9)","ma7(+9)":"ma7(#9)","ma7(b13)":"ma7(b13)",maj7b13:"ma7(b13)","maj7-13":"ma7(b13)","maj7(-13)":"ma7(b13)","+7(b13)":"ma7(b13)","+7b13":"ma7(b13)","M7-13":"ma7(b13)","+7(-13)":"ma7(b13)","maj7(b13)":"ma7(b13)",ma7b13:"ma9(b13)","ma7-13":"ma7(b13)","ma7(-13)":"ma7(b13)","ma7(b5)":"ma7(b5)",maj7b5:"ma7(b5)","maj7-5":"ma7(b5)","maj7(-5)":"ma7(b5)","+7(b5)":"ma7(b5)","+7b5":"ma7(b5)","M7-5":"ma7(b5)","+7(-5)":"ma7(b5)","maj7(b5)":"ma7(b5)",ma7b5:"ma7(b5)","ma7-5":"ma7(b5)","ma7(-5)":"ma7(b5)","ma7(b9)":"ma7(b9)",maj7b9:"ma7(b9)","maj7-9":"ma7(b9)","maj7(-9)":"ma7(b9)","+7(b9)":"ma7(b9)","+7b9":"ma7(b9)","M7-9":"ma7(b9)","+7(-9)":"ma7(b9)","maj7(b9)":"ma7(b9)",ma7b9:"ma7(b9)","ma7-9":"ma7(b9)","ma7(-9)":"ma7(b9)","ma7(no3)":"ma7(no3)","9(#7)":"ma9","+9":"ma9",M9:"ma9","ma9(#11)":"ma9(#11)","maj9#11":"ma9(#11)","maj9+11":"ma9(#11)","maj9(+11)":"ma9(#11)","+9(#11)":"ma9(#11)","+9#11":"ma9(#11)","M9+11":"ma9(#11)","+9(+11)":"ma9(#11)","maj9(#11)":"ma9(#11)","ma9+11":"ma9(#11)","ma9(+11)":"ma9(#11)","ma9(#11#5)":"ma9(#11#5)","maj9#11#5":"ma9(#11#5)","maj9+11+5":"ma9(#11#5)","maj9(+11+5)":"ma9(#11#5)","+9(#11#5)":"ma9(#11#5)","+9#11#5":"ma9(#11#5)","M9+11+5":"ma9(#11#5)","+9(+11+5)":"ma9(#11#5)","maj9(#11#5)":"ma9(#11#5)","ma9+11+5":"ma9(#11#5)","ma9(+11+5)":"ma9(#11#5)","ma9(#4)":"ma9(#4)","ma9(#5)":"ma9(#5)","maj9#5":"ma9(#5)","maj9+5":"ma9(#5)","maj9(+5)":"ma9(#5)","+9(#5)":"ma9(#5)","+9#5":"ma9(#5)","M9+5":"ma9(#5)","+9(+5)":"ma9(#5)","maj9(#5)":"ma9(#5)","ma9#5":"ma9(#5)","ma9+5":"ma9(#5)","ma9(+5)":"ma9(#5)","ma9(13)":"ma9(13)","ma9(b13)":"ma9(b13)",maj9b13:"ma9(b13)","maj9-13":"ma9(b13)","maj9(-13)":"ma9(b13)","+9(b13)":"ma9(b13)","+9b13":"ma9(b13)","M9-13":"ma9(b13)","+9(-13)":"ma9(b13)","maj9(b13)":"ma9(b13)","ma9-13":"ma9(b13)","ma9(-13)":"ma9(b13)","ma9(b13#5)":"ma9(b13#5)","maj9b13#5":"ma9(b13#5)","maj9-13+5":"ma9(b13#5)","maj9(-13+5)":"ma9(b13#5)","+9(b13#5)":"ma9(b13#5)","+9b13#5":"ma9(b13#5)","M9-13+5":"ma9(b13#5)","+9(-13+5)":"ma9(b13#5)","maj9(b13#5)":"ma9(b13#5)","ma7b13#5":"ma9(b13#5)","ma9-13+5":"ma9(b13#5)","ma9(-13+5)":"ma9(b13#5)","ma9(b5)":"ma9(b5)",maj9b5:"ma9(b5)","maj9-5":"ma9(b5)","maj9(-5)":"ma9(b5)","+9(b5)":"ma9(b5)","+9b5":"ma9(b5)","M9-5":"ma9(b5)","+9(-5)":"ma9(b5)","maj9(b5)":"ma9(b5)",ma9b5:"ma9(b5)","ma9-5":"ma9(b5)","ma9(-5)":"ma9(b5)",sus:"sus",sus4:"sus","sus(no5)":"sus(no5)","sus#42":"sus#42","":""},Zc=xr,pr={[aA]:{[nA]:{[sA]:{C:0,D:2,E:4,F:5,G:7,A:9,B:11},[$]:{B:0,C:1,D:3,E:5,F:6,G:8,A:10},[O]:{D:1,E:3,F:4,G:6,A:8,B:10,C:11}},[iA]:{[sA]:{C:0,D:2,E:4,F:5,G:7,A:9,B:11},[$]:{B:0,C:1,D:3,E:5,F:6,G:8,A:10},[O]:{D:1,E:3,F:4,G:6,A:8,B:10,C:11}}},[bA]:{[nA]:{[sA]:{Do:0,Re:2,Mi:4,Fa:5,Sol:7,La:9,Si:11},[$]:{Si:0,Do:1,Re:3,Mi:5,Fa:6,Sol:8,La:10},[O]:{Re:1,Mi:3,Fa:4,Sol:6,La:8,Si:10,Do:11}},[iA]:{[sA]:{Do:0,Re:2,Mi:4,Fa:5,Sol:7,La:9,Si:11},[$]:{Si:0,Do:1,Re:3,Mi:5,Fa:6,Sol:8,La:10},[O]:{Re:1,Mi:3,Fa:4,Sol:6,La:8,Si:10,Do:11}}},[CA]:{[nA]:{[sA]:{1:0,2:2,3:4,4:5,5:7,6:9,7:11},[$]:{7:0,1:1,2:3,3:5,4:6,5:8,6:10},[O]:{2:1,3:3,4:4,5:6,6:8,7:10,1:11}},[iA]:{[sA]:{1:0,2:2,3:3,4:5,5:7,6:8,7:10},[$]:{1:1,2:3,3:4,4:6,5:8,6:9,7:11},[O]:{2:1,3:2,4:4,5:6,6:7,7:9,1:11}}},[zA]:{[nA]:{[sA]:{I:0,II:2,III:4,IV:5,V:7,VI:9,VII:11},[$]:{VII:0,I:1,II:3,III:5,IV:6,V:8,VI:10},[O]:{II:1,III:3,IV:4,V:6,VI:8,VII:10,I:11}},[iA]:{[sA]:{I:0,II:2,III:3,IV:5,V:7,VI:8,VII:10},[$]:{I:1,II:3,III:4,IV:6,V:8,VI:9,VII:11},[O]:{II:1,III:2,IV:4,V:6,VI:7,VII:9,I:11}}}},Hr={[aA]:{[nA]:{[sA]:{0:"C",2:"D",4:"E",5:"F",7:"G",9:"A",11:"B"},[$]:{0:"B#",1:"C#",3:"D#",5:"E#",6:"F#",8:"G#",10:"A#"},[O]:{1:"Db",3:"Eb",4:"Fb",6:"Gb",8:"Ab",10:"Bb",11:"Cb"}},[iA]:{[sA]:{0:"C",2:"D",4:"E",5:"F",7:"G",9:"A",11:"B"},[$]:{0:"B#",1:"C#",3:"D#",5:"E#",6:"F#",8:"G#",10:"A#"},[O]:{1:"Db",3:"Eb",4:"Fb",6:"Gb",8:"Ab",10:"Bb",11:"Cb"}}},[bA]:{[nA]:{[sA]:{0:"Do",2:"Re",4:"Mi",5:"Fa",7:"Sol",9:"La",11:"Si"},[$]:{0:"Si#",1:"Do#",3:"Re#",5:"Mi#",6:"Fa#",8:"Sol#",10:"La#"},[O]:{1:"Reb",3:"Mib",4:"Fab",6:"Solb",8:"Lab",10:"Sib",11:"Dob"}},[iA]:{[sA]:{0:"Do",2:"Re",4:"Mi",5:"Fa",7:"Sol",9:"La",11:"Si"},[$]:{0:"Si#",1:"Do#",3:"Re#",5:"Mi#",6:"Fa#",8:"Sol#",10:"La#"},[O]:{1:"Reb",3:"Mib",4:"Fab",6:"Solb",8:"Lab",10:"Sib",11:"Dob"}}},[CA]:{[nA]:{[sA]:{0:"1",2:"2",4:"3",5:"4",7:"5",9:"6",11:"7"},[$]:{0:"#7",1:"#1",3:"#2",5:"#3",6:"#4",8:"#5",10:"#6"},[O]:{1:"b2",3:"b3",4:"b4",6:"b5",8:"b6",10:"b7",11:"b1"}},[iA]:{[sA]:{0:"1",2:"2",3:"3",5:"4",7:"5",8:"6",10:"7"},[$]:{1:"#1",3:"#2",4:"#3",6:"#4",8:"#5",9:"#6",11:"#7"},[O]:{1:"b2",2:"b3",4:"b4",6:"b5",7:"b6",9:"b7",11:"b1"}}},[zA]:{[nA]:{[sA]:{0:"I",2:"II",4:"III",5:"IV",7:"V",9:"VI",11:"VII"},[$]:{0:"#VII",1:"#I",3:"#II",5:"#III",6:"#IV",8:"#V",10:"#VI"},[O]:{1:"bII",3:"bIII",4:"bIV",6:"bV",8:"bVI",10:"bVII",11:"bI"}},[iA]:{[sA]:{0:"I",2:"II",3:"III",5:"IV",7:"V",8:"VI",10:"VII"},[$]:{1:"#I",3:"#II",4:"#III",6:"#IV",8:"#V",9:"#VI",11:"#VII"},[O]:{1:"bII",2:"bIII",4:"bIV",6:"bV",7:"bVI",9:"bVII",11:"bI"}}}};function Jr(i,e){return e.reduce((A,t)=>t(A),i)}function Ji(i){return i==="H"||i==="h"}function Sr(i){return i==="H"?"B":i==="h"?"b":i}function Xc(i){return i.startsWith("H")?`B${i.slice(1)}`:i}function kr(i,e,A){return A===Ps&&e===null&&(i==="B"||i==="b")?O:null}function Vr(i,e){return e||(Ji(i)?Ps:null)}function vc(i,e,A,t){let c=pr[A][t?iA:nA][e],M=A===aA?Sr(i):i;if(M in c)return c[M];let o=M.toUpperCase();return o in c?c[o]:null}function Lr(i){return i.items.some(e=>e instanceof H&&(e.chords||e.annotation))}function Pr(i){return"evaluate"in i&&typeof i.evaluate=="function"}function Kc(i,e){let A=i;for(let t=i.length;t<e;t+=1,A+=" ");return A}function Si(i){let e=globalThis.process;typeof e=="object"&&typeof e.emitWarning=="function"?e.emitWarning(i):console.warn(i)}function HA(i){try{throw new Error(`DEPRECATION: ${i}`)}catch(e){Si(`${i}
${e.stack}`)}}function XB(i){return i==null||i===""}function Wr(i,e,A){return e===zA?typeof i=="string"&&i.toLowerCase()===i:typeof A=="string"&&A[0]==="m"&&A.substring(0,2).toLowerCase()!=="ma"&&A.substring(0,3).toLowerCase()!=="maj"}function Zr(i){return i.replace(/\r\n?/g,`
`)}var Lt=class{constructor(e){this.grades=e}determineGrade(e,A,t){return this.getGradeForAccidental(e,t)||this.getGradeForAccidental(Ls,t)||this.getGradeForAccidental(A,t)||this.getGradeForAccidental($,t)}getGradeForAccidental(e,A){return e?this.grades[e][A]:null}};function Xr({type:i,accidental:e,preferredAccidental:A,grade:t,minor:c}){let M=c?iA:nA,o=Hr[i][M];return new Lt(o).determineGrade(e,A,t)}function vr(i){let{type:e,accidental:A,preferredAccidental:t,grade:c,minor:M}=i,o=Xr({type:e,accidental:A,preferredAccidental:t,grade:c,minor:M});if(!o)throw new Error(`Could not resolve ${i} to a key`);return M&&e===zA&&(o=o.toLowerCase()),o}function xs(i){return i===null||Zc[i]==="[blank]"?null:Zc[i]||i}var Bs;function Kr(i,e){let A={...i};return Object.keys(e).forEach(t=>{let c=i[t],M=e[t];c&&typeof c=="object"&&!Array.isArray(c)&&M&&typeof M=="object"&&!Array.isArray(M)?A[t]=Bs(c,M):A[t]=M}),A}Bs=(i,e)=>e==null?i:i==null||typeof i!="object"||typeof e!="object"||Array.isArray(e)?e:Kr(i,e);function Or(i,e){return Object.fromEntries(Object.entries(i).filter(([A,t])=>e(A,t)))}var $r={base:{},html:Fr,measurement:Yr,pdf:Dr,measured_html:mr},qr={base:["base"],html:["base","html"],measurement:["base","measurement"],pdf:["base","measurement","pdf"],measured_html:["base","measurement","measured_html"]},_r=i=>[...qr[i]||[]],vB=i=>{let e={...hr};return e=_r(i).reduce((t,c)=>Bs(t,$r[c]||{}),e),e},An=()=>vB("base");var en=()=>vB("measurement"),sn=()=>vB("pdf");var tn=`
`,Bn=`
AAABAAAAANpTmfAAAAAA0e+yRQAAAADR77JF
`,cn=`
`,gn=`
AjwCRQJ5AAAAAAABAAAAANpTmfAAAAAA0e+yRgAAAADR77JG
`,Mn={orientation:"p",unit:"pt",format:"letter",compress:!0,putOnlyUsedFonts:!0},Pt=class i{static setup(e,A=Mn){return new i(e,A)}constructor(e,A){this.currentPage=1,this.fontConfiguration=Hi.text,this.totalPages=1,this.doc=new e(A),this.doc.setLineWidth(0),this.doc.setDrawColor(0,0,0,0),this.addFonts()}get pageSize(){return this.doc.internal.pageSize}save(e){this.doc.save(e)}output(){return this.doc.output("blob")}eachPage(e){for(let A=1;A<=this.totalPages;A+=1)this.setPage(A),e()}newPage(){this.doc.addPage(),this.totalPages+=1,this.currentPage+=1}setPage(e){this.doc.setPage(e),this.currentPage=e}setFont(e,A,t){this.doc.setFont(e,A,t)}getFont(){return this.doc.getFont()}getFontSize(){return this.doc.getFontSize()}getFillColor(){return this.doc.getFillColor()}setFillColor(e){this.doc.setFillColor(e)}setFontStyle(e){this.doc.setFont(e.name,e.style,e.weight),this.doc.setFontSize(e.size),this.setTextColor(e.color),this.fontConfiguration=e}setFontSize(e){this.doc.setFontSize(e)}withFontConfiguration(e,A){let t=this.fontConfiguration;e&&this.setFontStyle(e);let c=A();return this.setFontStyle(t),c}setTextColor(e){switch(typeof e){case"string":this.doc.setTextColor(e);break;case"number":this.doc.setTextColor(e);break;default:break}}getTextDimensions(e,A){return!e||e.length===0?{w:0,h:0}:this.withFontConfiguration(A||null,()=>this.doc.getTextDimensions(e))}getTextWidth(e,A){return this.getTextDimensions(e,A).w}getTextHeight(e,A){return this.getTextDimensions(e,A).h}getSpaceWidth(){return this.getTextWidth(" ")}text(e,A,t,c){this.withFontConfiguration(c||null,()=>{if(this.doc.text(e,A,t),c?.underline){let{w:M}=this.doc.getTextDimensions(e);this.doc.setDrawColor(0),this.doc.setLineWidth(1.25),this.doc.line(A,t+3,A+M,t+3)}})}setDrawColor(e){this.doc.setDrawColor(e)}setLineWidth(e){this.doc.setLineWidth(e)}splitTextToSize(e,A,t){return this.withFontConfiguration(t||null,()=>this.doc.splitTextToSize(e,A))}addImage(e,A,t,c,M,o,a,U,Y){this.doc.addImage(e,A,t,c,M,o,a,U,Y)}setLineStyle(e){this.doc.setDrawColor(e.color),this.doc.setLineWidth(e.width),e.dash&&Array.isArray(e.dash)?(this.doc.setLineDash(e.dash),this.doc.setLineCap(1)):this.doc.setLineDash([])}resetDash(){this.doc.setLineDash([])}line(e,A,t,c){this.doc.line(e,A,t,c)}circle(e,A,t,c){this.doc.circle(e,A,t,c)}roundedRect(e,A,t,c,M,o,a){this.doc.roundedRect(e,A,t,c,M,o,a)}rect(e,A,t,c,M){this.doc.rect(e,A,t,c,M)}withDrawColor(e,A){let t=this.doc.getDrawColor();this.doc.setDrawColor(e),A(),this.doc.setDrawColor(t)}withFillColor(e,A){let t=this.doc.getFillColor();this.doc.setFillColor(e),A(),this.doc.setFillColor(t)}withFontSize(e,A){let t=this.doc.getFontSize();this.doc.setFontSize(e),A(),this.doc.setFontSize(t)}withLineWidth(e,A){let t=this.doc.getLineWidth();this.doc.setLineWidth(e),A(),this.doc.setLineWidth(t)}addFonts(){this.doc.addFileToVFS("NimbusSansL-Reg.ttf",tn),this.doc.addFont("NimbusSansL-Reg.ttf","NimbusSansL-Reg","normal"),this.doc.addFileToVFS("NimbusSansL-Bol.ttf",Bn),this.doc.addFont("NimbusSansL-Bol.ttf","NimbusSansL-Bol","bold"),this.doc.addFileToVFS("NimbusSansL-RegIta-italic.ttf",cn),this.doc.addFont("NimbusSansL-RegIta-italic.ttf","NimbusSansL-RegIta","italic"),this.doc.addFileToVFS("NimbusSanL-BolIta-bolditalic.ttf",gn),this.doc.addFont("NimbusSanL-BolIta-bolditalic.ttf","NimbusSansL-BolIta","bolditalic")}},rn=Pt,Oc=150,$c=270,Wt=class{#e;#s;#A;constructor(e,{x:A,y:t,width:c,fonts:M}){this.doc=e,this.#e=A,this.#s=t,this.#A=c/Oc,this.width=c,this.height=$c*this.#A,this.fonts=M}static calculateHeight(e){return $c*(e/Oc)}circle({x:e,y:A,size:t,fill:c,thickness:M}){this.withLineWidth(M,()=>{this.doc.withDrawColor(0,()=>{this.doc.circle(this.tx(e),this.ty(A),this.scale(t/2),c?"F":"S")})})}line({x1:e,y1:A,x2:t,y2:c,thickness:M,color:o=0}){this.withLineWidth(M,()=>{this.doc.withFillColor(o,()=>{this.doc.withDrawColor(o,()=>{this.doc.line(this.tx(e),this.ty(A),this.tx(t),this.ty(c))})})})}rect({x:e,y:A,width:t,height:c,fill:M,thickness:o,radius:a,color:U=0}){this.withLineWidth(o,()=>{this.doc.withDrawColor(U,()=>{this.doc.withFillColor(U,()=>{this.doc.roundedRect(this.tx(e),this.ty(A),this.scale(t),this.scale(c),this.scale(a),this.scale(a),M?"F":"S")})})})}text(e,{fontStyle:A,x:t,y:c}){let M=A??"title",o=this.fonts[M]??this.fonts.title;this.withFontConfiguration(o,()=>{let a=this.doc.getTextDimensions(e);this.doc.text(e,this.tx(t)-a.w/2,this.ty(c),o)})}scale(e){return e*this.#A}tx(e){return this.#e+this.scale(e)}ty(e){return this.#s+this.scale(e)}withFontConfiguration(e,A){this.doc.withFontConfiguration(e,A)}withLineWidth(e,A){this.doc.withLineWidth(this.scale(e),A)}},qc=Wt;function Dt(i,e,A){return A=A||" ",i.length>e?i:(e-=i.length,A+=A.repeat(e),i+A.slice(0,e))}var qe=class i extends Error{static buildMessage(e,A){function t(Y){return Y.charCodeAt(0).toString(16).toUpperCase()}function c(Y){return Y.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function M(Y){return Y.replace(/\\/g,"\\\\").replace(/\]/g,"\\]").replace(/\^/g,"\\^").replace(/-/g,"\\-").replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function o(Y){switch(Y.type){case"literal":return'"'+c(Y.text)+'"';case"class":let N=Y.parts.map(R=>Array.isArray(R)?M(R[0])+"-"+M(R[1]):M(R));return"["+(Y.inverted?"^":"")+N+"]";case"any":return"any character";case"end":return"end of input";case"other":return Y.description}}function a(Y){let N=Y.map(o),R,W;if(N.sort(),N.length>0){for(R=1,W=1;R<N.length;R++)N[R-1]!==N[R]&&(N[W]=N[R],W++);N.length=W}switch(N.length){case 1:return N[0];case 2:return N[0]+" or "+N[1];default:return N.slice(0,-1).join(", ")+", or "+N[N.length-1]}}function U(Y){return Y?'"'+c(Y)+'"':"end of input"}return"Expected "+a(e)+" but "+U(A)+" found."}constructor(e,A,t,c){super(),this.message=e,this.expected=A,this.found=t,this.location=c,this.name="PeggySyntaxError",typeof Object.setPrototypeOf=="function"?Object.setPrototypeOf(this,i.prototype):this.__proto__=i.prototype,typeof Error.captureStackTrace=="function"&&Error.captureStackTrace(this,i)}format(e){let A="Error: "+this.message;if(this.location){let t=null,c;for(c=0;c<e.length;c++)if(e[c].grammarSource===this.location.source){t=e[c].text.split(/\r\n|\n|\r/g);break}let M=this.location.start,o=this.location.source+":"+M.line+":"+M.column;if(t){let a=this.location.end,U=Dt("",M.line.toString().length," "),Y=t[M.line-1],N=M.line===a.line?a.column:Y.length+1;A+=`
 --> `+o+`
`+U+` |
`+M.line+" | "+Y+`
`+U+" | "+Dt("",M.column-1," ")+Dt("",N-M.column,"^")}else A+=`
 at `+o}return A}};function nn(i,e){e=e!==void 0?e:{};let A={},t=e.grammarSource,c={ChordDefinitionValue:LA},M=LA,o="frets",a=tA("frets",!1),U=function(I,F,j,d){return{name:I,baseFret:F||1,frets:j,fingers:d,text:uA()}},Y="/",N=tA("/",!1),R=/^[A-Ga-g]/,W=BA([["A","G"],["a","g"]],!1,!1),qA=/^[b#\u266D\u266F]/,me=BA(["b","#","\u266D","\u266F"],!1,!1),_A="es",De=tA("es",!1),Ne="s",lA=tA("s",!1),hA="is",QA=tA("is",!1),fA=/^[a-zA-Z0-9#\u266Fb\u266D()+\-\/\xF8\u0394\u2212]/,Ae=BA([["a","z"],["A","Z"],["0","9"],"#","\u266F","b","\u266D","(",")","+","-","/","\xF8","\u0394","\u2212"],!1,!1),JA="base-fret",ee=tA("base-fret",!1),je=function(I){return I},se="fingers",be=tA("fingers",!1),FA=function(I){return I},UA=function(I){return I},ze=/^[\-A-Za-z]/,Te=BA(["-",["A","Z"],["a","z"]],!1,!1),YA=/^[0-9]/,SA=BA([["0","9"]],!1,!1),te=function(I){return parseInt(I,10)},Z=function(I){return I},K="0",Be=tA("0",!1),kA=function(){return 0},TA="-1",ce=tA("-1",!1),yA=/^[NXnx]/,mA=BA(["N","X","n","x"],!1,!1),ye=VA("whitespace"),Re=VA("optional whitespace"),RA=/^[ \t\n\r]/,dA=BA([" ","	",`
`,"\r"],!1,!1),b=0,q=0,v=[{line:1,column:1}],k=0,GA=[],L=0,DA;if(e.startRule!==void 0){if(!(e.startRule in c))throw new Error(`Can't start parsing from rule "`+e.startRule+'".');M=c[e.startRule]}function uA(){return i.substring(q,b)}function Qs(){return IA(q,b)}function fs(I,F){throw F=F!==void 0?F:IA(q,b),re([VA(I)],i.substring(q,b),F)}function ie(I,F){throw F=F!==void 0?F:IA(q,b),Me(I,F)}function tA(I,F){return{type:"literal",text:I,ignoreCase:F}}function BA(I,F,j){return{type:"class",parts:I,inverted:F,ignoreCase:j}}function Is(){return{type:"any"}}function de(){return{type:"end"}}function VA(I){return{type:"other",description:I}}function ge(I){let F=v[I],j;if(F)return F;for(j=I-1;!v[j];)j--;for(F=v[j],F={line:F.line,column:F.column};j<I;)i.charCodeAt(j)===10?(F.line++,F.column=1):F.column++,j++;return v[I]=F,F}function IA(I,F){let j=ge(I),d=ge(F);return{source:t,start:{offset:I,line:j.line,column:j.column},end:{offset:F,line:d.line,column:d.column}}}function X(I){b<k||(b>k&&(k=b,GA=[]),GA.push(I))}function Me(I,F){return new qe(I,[],"",F)}function re(I,F,j){return new qe(qe.buildMessage(I,F),I,F,j)}function LA(){let I,F,j,d,_,MA,cA;if(I=b,F=ne(),F!==A)if(j=V(),j!==A)if(d=pe(),d===A&&(d=null),d!==A)if(i.substr(b,5)===o?(_=o,b+=5):(_=A,L===0&&X(a)),_!==A){if(MA=[],cA=PA(),cA!==A)for(;cA!==A;)MA.push(cA),cA=PA();else MA=A;MA!==A?(cA=He(),cA===A&&(cA=null),cA!==A?(q=I,F=U(F,d,MA,cA),I=F):(b=I,I=A)):(b=I,I=A)}else b=I,I=A;else b=I,I=A;else b=I,I=A;else b=I,I=A;return I}function ne(){let I,F,j,d;return I=b,F=b,j=Ge(),j!==A?(d=ue(),d===A&&(d=null),d!==A?(j=[j,d],F=j):(b=F,F=A)):(b=F,F=A),F!==A?I=i.substring(I,b):I=F,I}function Ge(){let I,F,j,d;return I=b,F=b,j=oe(),j!==A?(d=xe(),d===A&&(d=null),d!==A?(j=[j,d],F=j):(b=F,F=A)):(b=F,F=A),F!==A?I=i.substring(I,b):I=F,I}function ue(){let I,F,j,d;return I=b,F=b,i.charCodeAt(b)===47?(j=Y,b++):(j=A,L===0&&X(N)),j!==A?(d=oe(),d!==A?(j=[j,d],F=j):(b=F,F=A)):(b=F,F=A),F!==A?I=i.substring(I,b):I=F,I}function oe(){let I,F,j,d;return I=b,F=b,R.test(i.charAt(b))?(j=i.charAt(b),b++):(j=A,L===0&&X(W)),j!==A?(qA.test(i.charAt(b))?(d=i.charAt(b),b++):(d=A,L===0&&X(me)),d===A&&(i.substr(b,2)===_A?(d=_A,b+=2):(d=A,L===0&&X(De)),d===A&&(i.charCodeAt(b)===115?(d=Ne,b++):(d=A,L===0&&X(lA)),d===A&&(i.substr(b,2)===hA?(d=hA,b+=2):(d=A,L===0&&X(QA))))),d===A&&(d=null),d!==A?(j=[j,d],F=j):(b=F,F=A)):(b=F,F=A),F!==A?I=i.substring(I,b):I=F,I}function xe(){let I,F,j;if(I=b,F=[],fA.test(i.charAt(b))?(j=i.charAt(b),b++):(j=A,L===0&&X(Ae)),j!==A)for(;j!==A;)F.push(j),fA.test(i.charAt(b))?(j=i.charAt(b),b++):(j=A,L===0&&X(Ae));else F=A;return F!==A?I=i.substring(I,b):I=F,I}function pe(){let I,F,j,d,_;return I=b,i.substr(b,9)===JA?(F=JA,b+=9):(F=A,L===0&&X(ee)),F!==A?(j=Q(),j!==A?(d=NA(),d!==A?(_=Q(),_!==A?(q=I,F=je(d),I=F):(b=I,I=A)):(b=I,I=A)):(b=I,I=A)):(b=I,I=A),I}function He(){let I,F,j,d,_;if(I=b,F=Q(),F!==A)if(i.substr(b,7)===se?(j=se,b+=7):(j=A,L===0&&X(be)),j!==A){if(d=[],_=Ee(),_!==A)for(;_!==A;)d.push(_),_=Ee();else d=A;d!==A?(q=I,F=FA(d),I=F):(b=I,I=A)}else b=I,I=A;else b=I,I=A;return I}function Ee(){let I,F,j;return I=b,F=Q(),F!==A?(j=Je(),j!==A?(q=I,F=UA(j),I=F):(b=I,I=A)):(b=I,I=A),I}function Je(){let I;return I=Se(),I===A&&(ze.test(i.charAt(b))?(I=i.charAt(b),b++):(I=A,L===0&&X(Te))),I}function Se(){let I,F;return I=b,YA.test(i.charAt(b))?(F=i.charAt(b),b++):(F=A,L===0&&X(SA)),F!==A&&(q=I,F=te(F)),I=F,I}function PA(){let I,F,j;return I=b,F=Q(),F!==A?(j=ke(),j!==A?(q=I,F=Z(j),I=F):(b=I,I=A)):(b=I,I=A),I}function ke(){let I,F,j;return I=b,F=V(),F!==A?(j=NA(),j===A&&(j=xA(),j===A&&(j=Ve())),j!==A?(q=I,F=Z(j),I=F):(b=I,I=A)):(b=I,I=A),I}function NA(){let I,F;return I=b,YA.test(i.charAt(b))?(F=i.charAt(b),b++):(F=A,L===0&&X(SA)),F!==A&&(q=I,F=te(F)),I=F,I}function xA(){let I,F;return I=b,i.charCodeAt(b)===48?(F=K,b++):(F=A,L===0&&X(Be)),F!==A&&(q=I,F=kA()),I=F,I}function Ve(){let I;return i.substr(b,2)===TA?(I=TA,b+=2):(I=A,L===0&&X(ce)),I===A&&(yA.test(i.charAt(b))?(I=i.charAt(b),b++):(I=A,L===0&&X(mA))),I}function Q(){let I,F;if(L++,I=[],F=oA(),F!==A)for(;F!==A;)I.push(F),F=oA();else I=A;return L--,I===A&&(F=A,L===0&&X(ye)),I}function V(){let I,F;for(L++,I=[],F=oA();F!==A;)I.push(F),F=oA();return L--,I===A&&(F=A,L===0&&X(Re)),I}function oA(){let I;return RA.test(i.charAt(b))?(I=i.charAt(b),b++):(I=A,L===0&&X(dA)),I}if(DA=M(),DA!==A&&b===i.length)return DA;throw DA!==A&&b<i.length&&X(de()),re(GA,k<i.length?i.charAt(k):null,k<i.length?IA(k,k+1):IA(k,k))}var on=nn;function Nt(i){return i===uM}function jt(i){return GM.includes(i)}var Zt=class i{constructor(e,A,t,c){this.name=e,this.baseFret=A,this.frets=t,this.fingers=c||[]}static parse(e){let{name:A,baseFret:t,frets:c,fingers:M}=on(e.trim());return new i(A,t,c,M)}clone(){return new i(this.name,this.baseFret,[...this.frets],[...this.fingers])}},KB=Zt,En={barres:[],chord:"",fretCount:4,markers:[],openStrings:[],stringCount:6,unusedStrings:[],baseFret:0},an={titleY:28,neckWidth:120,neckHeight:160,nutThickness:10,fretThickness:4,nutColor:0,fretColor:"#929292",stringIndicatorSize:14,fingerIndicatorSize:16,stringColor:0,fingerIndicatorOffset:0,stringThickness:3,fretLineThickness:4,openStringIndicatorThickness:2,unusedStringIndicatorThickness:2,markerThickness:2,barreThickness:2,titleFontSize:48,baseFretFontSize:8,fingerNumberFontSize:28,showFingerNumbers:!0,diagramSpacing:7};function _c(i,e){Array.from({length:i}).forEach((A,t)=>e(t))}var Xt=class{constructor(e,A){this.chordDiagramDefinition={...En,...e},this.config={...an,...A}}get neckX(){return this.config.stringIndicatorSize/2+1}get neckY(){return this.stringIndicatorY+this.config.stringIndicatorSize+3}get stringIndicatorY(){return this.config.titleY+this.config.titleFontSize/2}get fingerNumberIndicatorsY(){return this.neckY+this.config.neckHeight+this.config.fingerNumberFontSize+this.config.fingerNumberFontSize/2}get nutThicknessCorrection(){return this.config.nutThickness-this.config.fretThickness}render(e){this.renderer=e,this.renderTitle(e),this.renderFrets(e),this.renderStrings(e),this.renderNut(e),this.renderOpenStringIndicators(e),this.renderUnusedStringIndicators(e),this.renderStringMarkers(e),this.renderBarres(e),this.renderFingerNumberIndicators(e)}renderTitle(e){let{chord:A}=this.chordDiagramDefinition;e.text(A,{fontSize:this.config.titleFontSize,x:this.neckX+this.config.neckWidth/2,y:this.config.titleY})}renderStrings(e){let{stringCount:A}=this.chordDiagramDefinition;_c(A,t=>{e.line({x1:this.neckX+t*(this.config.neckWidth/(A-1)),y1:this.neckY,x2:this.neckX+t*(this.config.neckWidth/(A-1)),y2:this.neckY+this.config.neckHeight,thickness:this.config.stringThickness,color:this.config.stringColor})})}renderNut(e){this.chordDiagramDefinition.baseFret===1?this.renderStandardNut(e):this.renderTransposedNut(e)}renderTransposedNut(e){e.text(this.chordDiagramDefinition.baseFret.toString(),{fontSize:this.config.baseFretFontSize,x:this.neckX-this.config.fretThickness-this.config.baseFretFontSize*3,y:this.neckY+this.config.neckHeight/this.chordDiagramDefinition.fretCount}),e.line({x1:this.neckX-this.config.fretThickness,y1:this.neckY+this.nutThicknessCorrection+1,x2:this.neckX-this.config.fretThickness+this.config.neckWidth+2*this.config.fretThickness,y2:this.neckY+this.nutThicknessCorrection,thickness:this.config.fretThickness,color:this.config.nutColor})}renderStandardNut(e){e.line({x1:this.neckX-this.config.stringThickness,y1:this.neckY,x2:this.neckX+this.config.neckWidth+this.config.stringThickness,y2:this.neckY,thickness:this.config.nutThickness})}renderFrets(e){let{fretCount:A}=this.chordDiagramDefinition,t=(this.config.neckHeight-this.nutThicknessCorrection)/A;_c(A,c=>{e.line({x1:this.neckX-this.config.fretThickness,y1:this.neckY+this.nutThicknessCorrection+(c+1)*t,x2:this.neckX-this.config.fretThickness+this.config.neckWidth+2*this.config.fretThickness,y2:this.neckY+this.nutThicknessCorrection+(c+1)*t,thickness:this.config.fretLineThickness,color:this.config.fretColor})})}renderOpenStringIndicators(e){let{openStrings:A,stringCount:t}=this.chordDiagramDefinition;A.forEach(c=>{e.circle({size:this.config.stringIndicatorSize,x:this.neckX+(c-1)*(this.config.neckWidth/(t-1)),y:this.stringIndicatorY,thickness:this.config.openStringIndicatorThickness})})}renderUnusedStringIndicators(e){let{stringCount:A,unusedStrings:t}=this.chordDiagramDefinition;t.forEach(c=>{let M=this.neckX+(c-1)*(this.config.neckWidth/(A-1)),o=this.stringIndicatorY,a=this.config.stringIndicatorSize;e.line({x1:M-a/2,y1:o-a/2,x2:M+a/2,y2:o+a/2,thickness:this.config.unusedStringIndicatorThickness}),e.line({x1:M+a/2,y1:o-a/2,x2:M-a/2,y2:o+a/2,thickness:this.config.unusedStringIndicatorThickness})})}renderStringMarkers(e){let{fretCount:A,markers:t}=this.chordDiagramDefinition,c=(this.config.neckHeight-this.nutThicknessCorrection)/A;t.forEach(({string:M,fret:o})=>{e.circle({x:this.neckX+(M-1)*(this.config.neckWidth/5),y:this.neckY+this.nutThicknessCorrection+o*c-c/2+this.config.fingerIndicatorOffset,size:this.config.fingerIndicatorSize,fill:!0,thickness:this.config.markerThickness})})}renderBarres(e){let{barres:A,fretCount:t,stringCount:c}=this.chordDiagramDefinition,M=(this.config.neckHeight-this.nutThicknessCorrection)/t,o=M/3,a=this.config.neckWidth/(c-1);A.forEach(({from:U,to:Y,fret:N})=>{let R=Y-U;e.rect({x:this.neckX+(U-1.5)*a,y:this.neckY+this.nutThicknessCorrection+(N-.5)*M-o/2+this.config.fingerIndicatorOffset,width:(R+1)*a,height:o,thickness:this.config.barreThickness,radius:8,fill:!0})})}renderFingerNumberIndicators(e){if(!this.config.showFingerNumbers)return;let{markers:A,stringCount:t}=this.chordDiagramDefinition,c=this.config.neckWidth/(t-1);A.forEach(({string:M,finger:o})=>{o&&e.text(`${o}`,{fontSize:this.config.fingerNumberFontSize,x:this.neckX+(M-1)*c,y:this.fingerNumberIndicatorsY})})}},Qn=Xt,vt=class{constructor(e,A){this.context=e,this.config=A}render(){if(!this.config.enabled)return;let e=this.config.overrides||{global:{},byKey:{}},A=this.calculateDiagramLayout();this.initializeDiagramPosition(A.height);let t=0,c=this.context.song.key||0;this.getChordDefinitions().forEach(M=>{let o=this.renderSingleChordDiagram(M,c,e,A,t);o!==null&&(t=o)})}calculateDiagramLayout(){let{renderingConfig:e}=this.config,A=e?.diagramSpacing||7,t=e?.maxDiagramsPerRow||null,{columnWidth:c}=this.context.dimensions,M=30,o=35,a=Math.floor(c/(M+A)),U=t?Math.min(t,a):a,Y=(U-1)*A,N=Math.max(M,Math.min(o,(c-Y)/U)),R=qc.calculateHeight(N);return{width:N,height:R,spacing:A,perRow:U}}initializeDiagramPosition(e){let A=this.context.getX(),t=this.context.getY(),c=this.context.getCurrentColumn();t===this.context.getMinY()&&this.context.getCurrentPage()>1&&(c=1,A=this.context.getColumnStartX()),t+e>this.context.getColumnBottomY()&&(this.context.moveToNextColumn(),t=this.context.getMinY(),A=this.context.getColumnStartX(),c=this.context.getCurrentColumn()),this.context.setPosition(A,t,c,this.context.getCurrentPage())}renderSingleChordDiagram(e,A,t,c,M){let{shouldHide:o,customDefinition:a}=this.processChordOverrides(e.name,A,t);if(o)return null;let U=a?KB.parse(a):e,Y=this.positionForNextDiagram(c,M);return this.drawChordDiagram(U,c.width,c.spacing),Y+1}positionForNextDiagram(e,A){let{count:t,x:c,y:M}=this.getInitialPosition(A);return{count:t,x:c,y:M}=this.handleRowBreak(e,t,c,M),{count:t,x:c,y:M}=this.handleColumnBreak(e,t,c,M),this.context.setPosition(c,M,this.context.getCurrentColumn(),this.context.getCurrentPage()),t}getInitialPosition(e){return{count:e,x:this.context.getX(),y:this.context.getY()}}handleRowBreak(e,A,t,c){let{columnWidth:M}=this.context.dimensions,o=this.context.getColumnStartX();return A>=e.perRow||t+e.width>o+M?{count:0,x:this.context.getColumnStartX(),y:c+e.height+e.spacing}:{count:A,x:t,y:c}}handleColumnBreak(e,A,t,c){return c+e.height>this.context.getColumnBottomY()?(this.context.moveToNextColumn(),{count:0,x:this.context.getColumnStartX(),y:this.context.getMinY()}):{count:A,x:t,y:c}}drawChordDiagram(e,A,t){let{fonts:c}=this.config,M=this.buildChordDiagram(e),o=this.context.getX(),a=this.context.getY(),U=new qc(this.context.doc,{x:o,y:a,width:A,fonts:c});M.render(U);let Y=o+A+t;this.context.setPosition(Y,a,this.context.getCurrentColumn(),this.context.getCurrentPage())}processChordOverrides(e,A,t){let c=t?.byKey?.[A]?.[e];if(c)return this.extractOverrideValues(c);let M=t?.global?.[e];return M?this.extractOverrideValues(M):{shouldHide:!1,customDefinition:null}}extractOverrideValues(e){return{shouldHide:e.hide??!1,customDefinition:e.definition??null}}getChordDefinitions(){let e=this.context.song.chordDefinitions.withDefaults();return this.context.song.getChords().map(A=>e.get(A)).filter(A=>A!==null)}buildChordDiagram(e){let A=e.frets.map((U,Y)=>Nt(U)?Y+1:null).filter(U=>U!==null),t=e.frets.map((U,Y)=>jt(U)?Y+1:null).filter(U=>U!==null),c=[],M=[];this.processFingeringAndBarres(e,c,M);let o=c.filter(U=>!M.some(Y=>U.string>=Y.from&&U.string<=Y.to&&U.fret===Y.fret)),{renderingConfig:a}=this.config;return new Qn({chord:e.name,openStrings:A,unusedStrings:t,markers:o,barres:M,baseFret:e.baseFret},a)}processFingeringAndBarres(e,A,t){if(!e.fingers||e.fingers.length===0){this.addMarkersWithoutFingering(e.frets,A);return}let c=this.groupFretsByFinger(e);this.processFretFingerGroups(c,A,t)}addMarkersWithoutFingering(e,A){e.forEach((t,c)=>{!jt(t)&&!Nt(t)&&A.push({string:c+1,fret:t,finger:0})})}groupFretsByFinger(e){let A={};return e.frets.forEach((t,c)=>{if(jt(t)||Nt(t))return;let M=e.fingers[c]||0;if(M===0)return;let o=t;A[o]=A[o]||{},A[o][M]=A[o][M]||[],A[o][M].push(c+1)}),A}processFretFingerGroups(e,A,t){Object.entries(e).forEach(([c,M])=>{let o=parseInt(c,10);Object.entries(M).forEach(([a,U])=>{this.processFingerGroup(o,parseInt(a,10),U,A,t)})})}processFingerGroup(e,A,t,c,M){t.length>1?this.addBarreIfValid(e,t,M):c.push({string:t[0],fret:e,finger:A})}addBarreIfValid(e,A,t){A.sort((o,a)=>o-a);let c=A[0],M=A[A.length-1];c>=1&&c<=6&&M>=1&&M<=6&&c<=M&&t.push({from:c,to:M,fret:e})}},fn=vt,In={capo:0,contextKey:null,decapo:!1,normalizeChords:!0,normalizeChordSuffix:!0,renderKey:null,songKey:null,style:null,transposeKey:null,useUnicodeModifier:!1},Kt=class{constructor(e={}){let A={...In,...e};this.capo=A.decapo?A.capo:0,this.contextKey=A.contextKey,this.normalizeChords=A.normalizeChords,this.normalizeChordSuffix=A.normalizeChordSuffix,this.renderKey=A.renderKey,this.songKey=A.songKey,this.style=A.style,this.transposeKey=A.transposeKey,this.useUnicodeModifier=A.useUnicodeModifier}render(e){if(e instanceof $A)return this.renderChord(e);let A=$A.parse(e);return A?this.renderChord(A):e}renderChord(e){return Jr(e,[A=>A.transpose(this.effectiveTransposeDistance),A=>this.accidental?A.useAccidental(this.accidental):A,A=>this.normalizeChords?A.normalize(this.effectiveKey,{normalizeSuffix:this.normalizeChordSuffix}):A,A=>this.changeChordType(A)]).toString({useUnicodeModifier:this.useUnicodeModifier})}changeChordType(e){switch(this.style){case"symbol":return e.toChordSymbol(this.effectiveKey);case"solfege":return e.toChordSolfege(this.effectiveKey);case"numeral":return e.toNumeral(this.effectiveKey);case"number":return e.toNumeric(this.effectiveKey);default:return e}}get accidental(){return this.renderKey?.accidental||this.contextKey?.accidental||null}get effectiveKey(){return this.renderKey?this.renderKey:this.contextKey?this.contextKey.transpose(this.effectiveTransposeDistance):null}get effectiveTransposeDistance(){let e=-1*this.capo;return this.songKey&&(this.transposeKey&&(e+=ln(this.transposeKey,this.songKey)),this.renderKey&&(e+=eA.distance(this.songKey,this.renderKey))),e}},wn=Kt;var Cn={solfege:{Sib:{1:"La",3:"Sol",6:"Mi",8:"Re",10:"Do"},Re:{2:"Do",5:"La",7:"Sol",10:"Mi"},Mib:{1:"Re",3:"Do",6:"La",8:"Sol"},Db:{1:"Do",4:"La",6:"Sol",9:"Mi"},"Do#":{1:"Do",4:"La",6:"Sol",9:"Mi"},Mi:{2:"Re",4:"Do",7:"La",9:"Sol"},Fa:{1:"Mi",3:"Re",5:"Do",6:"Si",8:"La",10:"Sol"},"Fa#":{2:"Mi",4:"Re",6:"Do",7:"Si",9:"La"},Solb:{2:"Mi",4:"Re",6:"Do",7:"Si",9:"La"},Sol:{3:"Mi",5:"Re",7:"Do",8:"Si",10:"La"},"Sol#":{1:"Sol",4:"Mi",6:"Re",8:"Do"},Lab:{1:"Sol",4:"Mi",6:"Re",8:"Do"},La:{2:"Sol",5:"Mi",7:"Re",9:"Do"},Si:{2:"La",4:"Sol",7:"Mi",9:"Re"},Do:{3:"La",5:"Sol",8:"Mi",10:"Re"},Lam:{3:"Fa#m",5:"Mim",7:"Rem"},Sibm:{1:"Lam",4:"Fa#m",6:"Mim",8:"Rem"},Sim:{2:"Lam",5:"Fa#m",7:"Mim"},Dom:{1:"Sim",3:"Lam",6:"Fa#m",8:"Mim"},"Do#m":{2:"Sim",4:"Lam",7:"Fa#m",9:"Mim"},Rem:{3:"Sim",5:"Lam",8:"Fa#m"},Mibm:{1:"Rem",4:"Sim",6:"Lam"},"Re#m":{1:"Rem",4:"Sim",6:"Lam"}},symbol:{Bb:{1:"A",3:"G",6:"E",8:"D",10:"C"},D:{2:"C",5:"A",7:"G",10:"E"},Eb:{1:"D",3:"C",6:"A",8:"G"},Db:{1:"C",4:"A",6:"G",9:"E"},"C#":{1:"C",4:"A",6:"G",9:"E"},E:{2:"D",4:"C",7:"A",9:"G"},F:{1:"E",3:"D",5:"C",6:"B",8:"A",10:"G"},"F#":{2:"E",4:"D",6:"C",7:"B",9:"A"},Gb:{2:"E",4:"D",6:"C",7:"B",9:"A"},G:{3:"E",5:"D",7:"C",8:"B",10:"A"},"G#":{1:"G",4:"E",6:"D",8:"C"},Ab:{1:"G",4:"E",6:"D",8:"C"},A:{2:"G",5:"E",7:"D",9:"C"},B:{2:"A",4:"G",7:"E",9:"D"},C:{3:"A",5:"G",8:"E",10:"D"},Am:{3:"F#m",5:"Em",7:"Dm"},Bbm:{1:"Am",4:"F#m",6:"Em",8:"Dm"},Bm:{2:"Am",5:"F#m",7:"Em"},Cm:{1:"Bm",3:"Am",6:"F#m",8:"Em"},"C#m":{2:"Bm",4:"Am",7:"F#m",9:"Em"},Dm:{3:"Bm",5:"Am",8:"F#m"},Ebm:{1:"Dm",4:"Bm",6:"Am"},"D#m":{1:"Dm",4:"Bm",6:"Am"}}};function ln(i,e){return/^\d+$/.test(i)?parseInt(i,10):eA.distance(e,i)}var hn={renderKey:null,useUnicodeModifier:!1,normalizeChords:!0,normalizeChordSuffix:!0,decapo:!1};function OB(i,e,A,t={}){let{renderKey:c,useUnicodeModifier:M,normalizeChords:o,normalizeChordSuffix:a,decapo:U}={...hn,...t},Y=A.metadata.getSingle(ns);return new wn({capo:Y?parseInt(Y,10):0,contextKey:eA.wrap(e.key||A.key),decapo:U,normalizeChords:o,normalizeChordSuffix:a,renderKey:c,songKey:eA.wrap(A.key),style:A.metadata.getSingle(yi),transposeKey:e.transposeKey,useUnicodeModifier:M}).render(i)}function Fn(i){let A=eA.wrapOrFail(i).type==="solfege"?"solfege":"symbol";return Cn[A][eA.toString(i)]}function bt({selector:i,isNegated:e},{configuration:A,metadata:t}){if(i===A.instrument?.type||i===A.user?.name)return!e;let c=t.getSingle(i),M=c&&c!=="";return e?!M:!!M}var qs=class i{static[Symbol.hasInstance](e){return as(e,di)}constructor(e){this.parentLine=null,this.content=e}isRenderable(){return!1}clone(){return new i(this.content)}toString(){return`Comment(content=${this.content})`}};Es(qs.prototype,di);var _e=qs,Ot=class extends Ei{},$B=Ot,_s=class i extends $B{static[Symbol.hasInstance](e){return as(e,Gi)}constructor(e){super(),this.string=e}evaluate(){return this.string}isRenderable(){return!0}clone(){return new i(this.string)}};Es(_s.prototype,Gi);var OA=_s,At=class i{static[Symbol.hasInstance](e){return as(e,ui)}constructor(e=" "){this.content=e}clone(){return new i}};Es(At.prototype,ui);var gA=At,ki={};ki=JSON.parse('{"A":"A base-fret 0 frets x 0 2 2 2 0","A/B":"A/B base-fret 0 frets x 2 2 2 2 0","A/C#":"A/C# base-fret 0 frets x 4 2 2 2 0","A/D":"A/D base-fret 0 frets x x 0 2 2 0","A/E":"A/E base-fret 0 frets 0 0 2 2 2 0","A/F#":"A/F# base-fret 0 frets 2 0 2 2 0 0","A/G":"A/G base-fret 0 frets 3 0 2 2 0 0","A/G#":"A/G# base-fret 0 frets 4 x 2 2 0 0","A11":"A11 base-fret 2 frets 4 3 x 1 2 2","A13":"A13 base-fret 0 frets x 0 2 0 2 2","A13(b9)":"A13(b9) base-fret 3 frets 2 x 2 3 4 3","A2":"A2 base-fret 0 frets x 0 2 2 0 0","A2/C#":"A2/C# base-fret 0 frets x 4 2 2 0 0","A2/E":"A2/E base-fret 0 frets 0 0 2 2 0 0","A2/F#":"A2/F# base-fret 0 frets 2 0 2 2 0 0","A2/G#":"A2/G# base-fret 0 frets 4 x 2 2 0 0","Asus":"Asus base-fret 0 frets x 0 2 2 3 0","Asus/E":"Asus/E base-fret 0 frets 0 0 2 2 3 0","A5":"A5 base-fret 0 frets x 0 2 2 x x","A5/E":"A5/E base-fret 2 frets 0 0 1 1 1 4","A6":"A6 base-fret 0 frets x 0 2 2 2 2","A7":"A7 base-fret 0 frets x 0 2 0 2 0","Aaug7":"Aaug7 base-fret 0 frets x 0 3 2 2 1","A7(#5b9)":"A7(#5b9) base-fret 3 frets 2 x 2 3 3 3","A7(b5)":"A7(b5) base-fret 0 frets x 0 1 0 2 x","A7/E":"A7/E base-fret 0 frets 0 0 2 0 2 0","A7/G":"A7/G base-fret 0 frets 3 x 2 0 2 0","A7(b9)":"A7(b9) base-fret 0 frets x 0 2 3 2 3","A7sus4":"A7sus4 base-fret 0 frets x 0 2 0 3 0","A7sus4/E":"A7sus4/E base-fret 0 frets 0 0 2 0 3 0","A9":"A9 base-fret 0 frets x 0 2 4 0 0","A9(b5)":"A9(b5) base-fret 0 frets x 0 1 4 2 3","Aaug":"Aaug base-fret 0 frets x 0 3 2 2 1","G#9":"G#9 base-fret 0 frets 4 3 4 3 4 x","G#sus":"G#sus base-fret 0 frets 4 4 1 1 2 x","G#ma9":"G#ma9 base-fret 0 frets 4 x 1 3 1 3","Adim":"Adim base-fret 0 frets x 0 1 2 1 2","Adim7":"Adim7 base-fret 0 frets x 0 1 2 1 2","Am":"Am base-fret 0 frets x 0 2 2 1 0","Am/C":"Am/C base-fret 0 frets x 3 2 2 1 0","Am13":"Am13 base-fret 0 frets x 0 0 0 1 2","Am6":"Am6 base-fret 0 frets x 0 2 2 1 2","Am7":"Am7 base-fret 0 frets x 0 2 0 1 0","Am7(b5)":"Am7(b5) base-fret 0 frets x 0 1 0 1 x","Am7/G":"Am7/G base-fret 0 frets 3 x 2 0 1 0","Am9":"Am9 base-fret 0 frets x 0 2 4 1 0","Ama7":"Ama7 base-fret 0 frets x 0 2 1 2 0","Ama9":"Ama9 base-fret 0 frets x 0 2 4 2 4","B":"B base-fret 0 frets x 2 4 4 4 2","B/A":"B/A base-fret 0 frets x 0 4 4 4 2","B/A#":"B/A# base-fret 0 frets x 1 4 3 0 x","B/C#":"B/C# base-fret 0 frets x 4 4 4 4 x","B/D#":"B/D# base-fret 0 frets x x 1 4 4 2","B/E":"B/E base-fret 0 frets 0 2 4 4 0 0","B/F#":"B/F# base-fret 0 frets 2 2 4 4 4 2","B/G#":"B/G# base-fret 0 frets 4 x 4 4 4 x","B11":"B11 base-fret 0 frets x 2 1 2 0 0","B13":"B13 base-fret 0 frets x 2 4 2 4 4","B13(b9)":"B13(b9) base-fret 5 frets 2 x 2 3 4 3","B2":"B2 base-fret 0 frets x 2 4 4 2 2","B2/A#":"B2/A# base-fret 0 frets x 1 4 4 0 x","B2/D#":"B2/D# base-fret 0 frets x 3 1 1 0 x","B2/G#":"B2/G# base-fret 0 frets 4 x 4 4 0 x","Bsus":"Bsus base-fret 0 frets x 2 4 4 0 0","Bsus/F#":"Bsus/F# base-fret 0 frets 2 2 4 4 0 0","B5":"B5 base-fret 0 frets x 2 4 4 0 x","B5/F#":"B5/F# base-fret 0 frets 2 2 4 4 2 2","B6":"B6 base-fret 0 frets x 2 4 4 4 4","B6/9":"B6/9 base-fret 0 frets x 2 1 1 2 x","B7":"B7 base-fret 0 frets x 2 1 2 0 2","Baug7":"Baug7 base-fret 0 frets x 2 1 0 0 3","B7(#5b9)":"B7(#5b9) base-fret 5 frets 2 x 2 3 3 3","B7(#9)":"B7(#9) base-fret 0 frets x 2 1 2 3 x","B7(b5)":"B7(b5) base-fret 0 frets x 2 3 2 4 x","B7(b9)":"B7(b9) base-fret 0 frets x 2 1 2 1 x","B7/A":"B7/A base-fret 0 frets x 0 1 2 0 2","B7/D#":"B7/D# base-fret 0 frets x x 1 2 0 2","B7/F#":"B7/F# base-fret 0 frets 2 2 4 2 4 2","B7sus4":"B7sus4 base-fret 0 frets x 2 2 2 0 2","B7sus4/F#":"B7sus4/F# base-fret 0 frets 2 2 4 4 0 0","B7sus4/G#":"B7sus4/G# base-fret 0 frets 4 x 4 4 0 0","B9":"B9 base-fret 0 frets x 2 1 2 2 2","B9(b5)":"B9(b5) base-fret 0 frets x 2 1 2 2 1","Baug":"Baug base-fret 0 frets x 2 1 0 0 3","A#/D":"A#/D base-fret 0 frets x x 0 3 3 1","A#/F":"A#/F base-fret 0 frets 1 1 3 3 3 1","A#11":"A#11 base-fret 0 frets x 1 0 3 4 4","A#13":"A#13 base-fret 0 frets x 1 3 1 3 3","A#2":"A#2 base-fret 0 frets x 1 3 3 1 1","A#2/F":"A#2/F base-fret 0 frets 1 1 3 3 1 1","A#sus":"A#sus base-fret 0 frets x 1 3 3 4 1","A#7":"A#7 base-fret 0 frets x 1 3 1 3 1","A#aug7":"A#aug7 base-fret 0 frets x 1 4 1 3 x","A#7(#5b9)":"A#7(#5b9) base-fret 5 frets 1 x 1 2 2 2","A#7(#9)":"A#7(#9) base-fret 0 frets x 1 0 1 2 x","A#7(b5)":"A#7(b5) base-fret 0 frets 2 x 2 3 1 x","A#7/F":"A#7/F base-fret 0 frets 1 1 3 1 3 1","A#9":"A#9 base-fret 0 frets x 1 3 3 1 1","A#9(b5)":"A#9(b5) base-fret 0 frets x 1 0 1 1 0","A#m11":"A#m11 base-fret 0 frets 4 x 4 4 2 x","A#m13":"A#m13 base-fret 0 frets x 1 1 1 2 3","A#m6":"A#m6 base-fret 0 frets x x x 3 2 3","A#m7":"A#m7 base-fret 0 frets x 1 3 1 2 1","A#m7(b5)":"A#m7(b5) base-fret 0 frets x 1 2 1 2 0","A#m9":"A#m9 base-fret 0 frets x 4 3 1 1 1","A#ma9":"A#ma9 base-fret 0 frets x 1 0 2 1 x","Bdim":"Bdim base-fret 0 frets x 2 0 1 0 1","Bdim7":"Bdim7 base-fret 0 frets x 2 0 1 0 1","Bm":"Bm base-fret 0 frets x 2 4 4 3 2","Bm/D":"Bm/D base-fret 0 frets x x 0 4 3 2","Bm11":"Bm11 base-fret 0 frets x 2 0 2 2 0","Bm13":"Bm13 base-fret 0 frets x 2 0 1 2 2","Bm6":"Bm6 base-fret 0 frets x 2 0 1 0 2","Bm7":"Bm7 base-fret 0 frets x 2 4 2 3 2","Bm7(b5)":"Bm7(b5) base-fret 0 frets x 2 3 2 3 x","Bm7/A":"Bm7/A base-fret 0 frets x 0 4 2 3 2","Bm9":"Bm9 base-fret 0 frets x 2 x 2 2 2","Bma7":"Bma7 base-fret 0 frets 2 2 4 3 4 2","Bma9":"Bma9 base-fret 0 frets x 2 1 3 2 x","C":"C base-fret 0 frets x 3 2 0 3 3","C#/G#":"C#/G# base-fret 0 frets x x x 1 2 1","C#11":"C#11 base-fret 0 frets x 4 3 4 2 2","C#13":"C#13 base-fret 2 frets x 2 1 2 2 4","C#2":"C#2 base-fret 0 frets x 4 1 1 2 x","C#2/G#":"C#2/G# base-fret 3 frets 1 1 3 3 1 1","C#7":"C#7 base-fret 0 frets x 4 3 4 2 x","C#aug7":"C#aug7 base-fret 0 frets x 4 3 2 0 x","C#7(b5)":"C#7(b5) base-fret 0 frets x 4 3 4 x 3","C#7(b9)":"C#7(b9) base-fret 0 frets x 4 3 4 3 x","C#7/G#":"C#7/G# base-fret 3 frets 1 1 3 1 3 1","C#9":"C#9 base-fret 0 frets x 4 3 4 4 4","C#m7":"C#m7 base-fret 0 frets x 4 2 4 2 4","C#m7(b5)":"C#m7(b5) base-fret 0 frets x 4 2 4 2 3","C#m9":"C#m9 base-fret 0 frets x 4 2 4 4 4","C/A":"C/A base-fret 0 frets x 0 2 0 1 0","C/B":"C/B base-fret 0 frets x 2 x 0 1 3","C/A#":"C/A# base-fret 0 frets x 1 2 0 2 0","C/D":"C/D base-fret 0 frets x x 0 0 1 0","C/E":"C/E base-fret 0 frets 0 3 2 0 1 0","C/F":"C/F base-fret 0 frets x x 3 0 1 0","C/G":"C/G base-fret 0 frets 3 3 2 0 1 0","C11":"C11 base-fret 0 frets x 3 3 3 3 3","C2":"C2 base-fret 0 frets x 3 2 0 3 3","C2/A":"C2/A base-fret 0 frets x 0 2 0 3 3","C2/B":"C2/B base-fret 0 frets x 2 2 0 3 3","C2/E":"C2/E base-fret 0 frets 0 3 2 0 3 3","C2/G":"C2/G base-fret 0 frets 3 3 2 0 3 3","Csus":"Csus base-fret 0 frets x 3 3 2 1 1","C5":"C5 base-fret 0 frets x 3 x 0 1 3","C5/B":"C5/B base-fret 0 frets x 2 x 0 1 3","C5/G":"C5/G base-fret 0 frets 3 3 x 0 1 3","C6":"C6 base-fret 0 frets x 3 2 2 1 0","C7":"C7 base-fret 0 frets 0 3 2 3 1 0","Caug7":"Caug7 base-fret 0 frets x 3 2 1 1 0","C7(b9)":"C7(b9) base-fret 0 frets x 3 2 3 2 x","C7/E":"C7/E base-fret 0 frets 0 3 2 3 1 0","C7/G":"C7/G base-fret 0 frets 3 x 2 3 1 0","C7sus4":"C7sus4 base-fret 0 frets x 3 3 3 1 3","C9":"C9 base-fret 0 frets x 3 2 0 3 0","Caug":"Caug base-fret 0 frets x 3 2 1 1 0","Cdim":"Cdim base-fret 0 frets x x 1 2 1 2","Cdim7":"Cdim7 base-fret 0 frets x 3 4 2 4 x","Cm":"Cm base-fret 0 frets x 3 1 x 1 3","Cm/D#":"Cm/D# base-fret 0 frets x x 1 x 1 3","Cm11":"Cm11 base-fret 0 frets x 3 3 3 4 3","Cm7":"Cm7 base-fret 0 frets x 3 1 3 4 x","Cm9":"Cm9 base-fret 0 frets x 3 1 3 3 3","Cma7":"Cma7 base-fret 0 frets x 3 2 0 0 0","Cma9":"Cma9 base-fret 0 frets x 3 2 4 3 x","D":"D base-fret 0 frets x x 0 2 3 2","D/A":"D/A base-fret 0 frets x 0 0 2 3 2","D/B":"D/B base-fret 0 frets x 2 0 2 3 2","D/C":"D/C base-fret 0 frets x 3 x 2 3 2","D/C#":"D/C# base-fret 0 frets x 4 x 2 3 2","D/E":"D/E base-fret 0 frets x x 2 2 3 2","D/F#":"D/F# base-fret 0 frets 2 0 0 2 3 2","D/G":"D/G base-fret 0 frets 3 x 0 0 3 2","D11":"D11 base-fret 3 frets x 2 2 2 2 2","D13":"D13 base-fret 0 frets x x 0 2 0 2","D2":"D2 base-fret 0 frets x x 0 2 2 0","D2/A":"D2/A base-fret 0 frets x 0 0 2 3 0","D2/B":"D2/B base-fret 0 frets x 2 0 2 3 0","D2/C#":"D2/C# base-fret 0 frets x 4 0 2 3 0","D2/F#":"D2/F# base-fret 0 frets 2 x 0 2 3 0","Dsus":"Dsus base-fret 0 frets x x 0 2 3 3","Dsus/A":"Dsus/A base-fret 0 frets x 0 0 2 3 3","D5":"D5 base-fret 0 frets x x 0 2 3 x","D5/A":"D5/A base-fret 2 frets x 0 0 1 2 4","D5/C#":"D5/C# base-fret 2 frets x x 0 1 1 4","D6":"D6 base-fret 0 frets x x 0 2 0 2","D7":"D7 base-fret 0 frets x x 0 2 1 2","Daug7":"Daug7 base-fret 0 frets x x 0 3 3 2","D7(#9)":"D7(#9) base-fret 3 frets x 2 1 2 3 x","D7(b5)":"D7(b5) base-fret 0 frets x x 0 1 1 2","D7(b9)":"D7(b9) base-fret 2 frets x 4 x 4 3 1","D7/A":"D7/A base-fret 0 frets x 0 0 2 1 2","D7/C":"D7/C base-fret 0 frets x 3 0 2 1 2","D7/F#":"D7/F# base-fret 0 frets 2 0 0 2 1 2","D7sus4":"D7sus4 base-fret 0 frets x x 0 2 1 1","D9":"D9 base-fret 0 frets x x 0 2 2 0","Daug":"Daug base-fret 0 frets x x 0 3 3 2","Ddim":"Ddim base-fret 0 frets x x 0 1 0 1","Ddim7":"Ddim7 base-fret 0 frets x x 0 1 0 1","Dm":"Dm base-fret 0 frets x x 0 2 1 3","Dm/F":"Dm/F base-fret 0 frets x x 3 2 1 3","Dm7":"Dm7 base-fret 0 frets x x 0 2 1 1","Dm7(b5)":"Dm7(b5) base-fret 0 frets x x 0 1 1 1","Dm7/C":"Dm7/C base-fret 0 frets x 3 x 2 1 1","Dma7":"Dma7 base-fret 0 frets x x 0 2 2 2","Dma9":"Dma9 base-fret 2 frets x 4 3 1 1 0","E":"E base-fret 0 frets 0 2 2 1 0 0","E/A":"E/A base-fret 0 frets x 0 2 1 0 0","E/B":"E/B base-fret 0 frets x 2 2 1 0 0","E/C#":"E/C# base-fret 0 frets x 4 2 4 0 0","E/D":"E/D base-fret 0 frets x x 0 1 0 0","E/D#":"E/D# base-fret 0 frets x x 1 1 0 0","E/F#":"E/F# base-fret 0 frets 2 2 2 1 0 x","E/G#":"E/G# base-fret 0 frets 4 x 2 4 0 0","E11":"E11 base-fret 0 frets 0 2 0 2 3 4","E13":"E13 base-fret 0 frets 0 2 0 1 3 0","E2":"E2 base-fret 0 frets 0 2 4 1 0 0","E2/B":"E2/B base-fret 0 frets x 2 4 1 0 0","E2/C#":"E2/C# base-fret 5 frets x 2 4 4 0 0","E2/D#":"E2/D# base-fret 5 frets x 2 4 3 0 0","E2/G#":"E2/G# base-fret 0 frets 4 x 4 4 0 0","Esus":"Esus base-fret 0 frets 0 2 2 2 0 0","Esus/B":"Esus/B base-fret 0 frets x 2 2 2 0 0","E5":"E5 base-fret 0 frets 0 2 4 4 0 0","E5/B":"E5/B base-fret 0 frets x 2 2 4 0 0","E5/D#":"E5/D# base-fret 5 frets x 1 4 3 0 0","E6":"E6 base-fret 0 frets 0 2 2 1 2 0","E7":"E7 base-fret 0 frets 0 2 0 1 0 0","E7(#9)":"E7(#9) base-fret 0 frets 0 2 0 1 3 3","E7(b5)":"E7(b5) base-fret 0 frets 0 1 2 1 3 x","E7(b9)":"E7(b9) base-fret 0 frets 0 x 0 1 0 1","E7/B":"E7/B base-fret 0 frets x 2 0 1 0 0","E7/D":"E7/D base-fret 0 frets x x 0 1 0 0","E7/G#":"E7/G# base-fret 0 frets 4 x 0 4 0 0","E7sus4":"E7sus4 base-fret 0 frets 0 2 0 2 0 0","E9":"E9 base-fret 0 frets 0 2 4 1 0 0","Eaug":"Eaug base-fret 0 frets 0 3 2 1 1 0","Eaug7":"Eaug7 base-fret 0 frets 0 3 2 1 1 0","D#/A#":"D#/A# base-fret 0 frets x x x 3 4 3","D#11":"D#11 base-fret 0 frets x x 1 0 2 4","D#13":"D#13 base-fret 4 frets x 2 1 2 2 4","D#2":"D#2 base-fret 0 frets x x 1 3 4 1","D#2/A#":"D#2/A# base-fret 5 frets 1 1 3 3 1 1","D#7":"D#7 base-fret 0 frets x x 1 3 2 3","D#aug7":"D#aug7 base-fret 0 frets x x 1 4 2 3","D#7(#9)":"D#7(#9) base-fret 0 frets x x 1 0 2 2","D#7(b5)":"D#7(b5) base-fret 0 frets x x 1 2 2 3","D#7(b9)":"D#7(b9) base-fret 0 frets x x 1 0 2 0","D#7/A#":"D#7/A# base-fret 5 frets 2 2 4 2 4 2","D#9":"D#9 base-fret 0 frets x x 1 0 2 1","D#dim7":"D#dim7 base-fret 0 frets x x 1 2 1 2","D#m9":"D#m9 base-fret 3 frets x 3 1 3 3 3","D#ma9":"D#ma9 base-fret 0 frets x x 1 0 3 1","Edim":"Edim base-fret 0 frets 0 1 2 0 2 0","Edim7":"Edim7 base-fret 0 frets 0 1 2 0 2 0","Em":"Em base-fret 0 frets 0 2 2 0 0 0","Em/G":"Em/G base-fret 0 frets 3 2 2 0 3 3","Em7":"Em7 base-fret 0 frets 0 2 2 0 3 0","Em7/D":"Em7/D base-fret 0 frets x x 0 0 3 3","Em9":"Em9 base-fret 0 frets 0 2 4 0 0 0","Ema7":"Ema7 base-fret 0 frets 0 2 1 1 0 0","Ema9":"Ema9 base-fret 0 frets 0 2 4 4 4 4","F":"F base-fret 0 frets x x 3 2 1 1","F#/C#":"F#/C# base-fret 0 frets x 4 4 3 2 2","F#13":"F#13 base-fret 0 frets 2 4 2 3 4 2","F#sus":"F#sus base-fret 0 frets 2 4 4 4 2 2","F#6":"F#6 base-fret 0 frets 2 x 1 3 2 x","F#7":"F#7 base-fret 0 frets 1 4 1 3 1 1","F#7(b5)":"F#7(b5) base-fret 0 frets 2 x 2 3 1 x","F#7/C#":"F#7/C# base-fret 0 frets x 4 2 3 2 2","F#dim7":"F#dim7 base-fret 0 frets 2 3 4 2 4 2","F#m/A":"F#m/A base-fret 0 frets x 0 2 2 0 0","F#m6":"F#m6 base-fret 0 frets 2 x 1 2 2 2","F#m7":"F#m7 base-fret 0 frets 2 x 2 2 0 0","F#m7(b5)":"F#m7(b5) base-fret 0 frets 2 x 2 2 1 0","F#m7/E":"F#m7/E base-fret 0 frets 0 4 4 2 0 0","F#ma9":"F#ma9 base-fret 0 frets 2 1 3 1 2 1","F/A":"F/A base-fret 0 frets x 0 3 2 1 1","F/A#":"F/A# base-fret 0 frets x 1 3 2 1 1","F/C":"F/C base-fret 0 frets x 3 3 2 1 1","F/D":"F/D base-fret 0 frets x x 0 2 1 1","F/E":"F/E base-fret 0 frets x x 2 2 1 1","F/D#":"F/D# base-fret 0 frets x x 1 2 1 1","F/G":"F/G base-fret 0 frets 3 x 3 2 1 1","F13":"F13 base-fret 0 frets 1 3 1 2 3 1","F2":"F2 base-fret 0 frets x x 3 0 1 1","F2/A":"F2/A base-fret 0 frets x 0 3 0 1 1","F2/C":"F2/C base-fret 0 frets x 3 3 0 1 1","F2/D":"F2/D base-fret 0 frets x x 0 0 1 1","F2/E":"F2/E base-fret 0 frets x x 2 0 1 1","Fsus":"Fsus base-fret 0 frets x x 3 3 1 1","F5":"F5 base-fret 0 frets 1 3 3 x 1 1","F6":"F6 base-fret 0 frets x x 3 2 3 1","F7":"F7 base-fret 0 frets 1 3 1 2 1 1","F7(#9)":"F7(#9) base-fret 0 frets x x 3 2 4 4","F7(b9)":"F7(b9) base-fret 0 frets x x 3 2 4 2","F7/A":"F7/A base-fret 0 frets x 0 1 2 1 1","F7/C":"F7/C base-fret 0 frets x 3 1 2 1 1","F7sus4":"F7sus4 base-fret 0 frets 1 3 1 3 1 1","F9":"F9 base-fret 0 frets x x 3 0 1 1","Faug":"Faug base-fret 0 frets x x 3 2 2 1","Faug7":"Faug7 base-fret 0 frets x x 3 2 2 1","Fdim":"Fdim base-fret 0 frets x x 0 1 0 1","Fdim7":"Fdim7 base-fret 0 frets x x 3 4 3 4","Fm":"Fm base-fret 0 frets x x 3 1 1 1","Fm6":"Fm6 base-fret 0 frets 1 x 0 1 1 1","Fm7":"Fm7 base-fret 0 frets 1 3 1 1 1 1","Fm7(b5)":"Fm7(b5) base-fret 0 frets 1 x 1 1 0 x","Fm9":"Fm9 base-fret 0 frets 1 3 1 1 1 3","Fma7":"Fma7 base-fret 0 frets x x 3 2 1 0","G":"G base-fret 0 frets 3 2 0 0 3 3","G#/D#":"G#/D# base-fret 0 frets x x 1 1 1 4","G#11":"G#11 base-fret 0 frets 4 x 4 3 2 x","G#13":"G#13 base-fret 0 frets 4 x 4 3 1 1","G#13(b9)":"G#13(b9) base-fret 3 frets 1 x 1 2 3 2","G#7":"G#7 base-fret 0 frets 4 3 4 1 1 x","G#aug7":"G#aug7 base-fret 0 frets 4 x 3 4 4 4","G#7(#5b9)":"G#7(#5b9) base-fret 3 frets 1 x 1 2 2 2","G#7(#9)":"G#7(#9) base-fret 0 frets 4 x 4 2 4 x","G#7(b5)":"G#7(b5) base-fret 0 frets 4 3 4 x 3 4","G#7/D#":"G#7/D# base-fret 3 frets x 3 1 2 1 1","G#9(b5)":"G#9(b5) base-fret 0 frets 4 x 4 3 3 x","G#m11":"G#m11 base-fret 0 frets 4 x 4 4 2 x","G#m13":"G#m13 base-fret 0 frets 4 2 4 x x 1","G#m6":"G#m6 base-fret 0 frets 4 3 4 3 4 4","G#m7":"G#m7 base-fret 0 frets 4 2 4 4 x 2","G#m7(b5)":"G#m7(b5) base-fret 0 frets 4 x 4 4 3 x","G#m7(b9)":"G#m7(b9) base-fret 2 frets 2 x 1 2 2 4","G#m9":"G#m9 base-fret 0 frets 4 2 4 3 x 2","G/A":"G/A base-fret 0 frets x 0 0 0 3 3","G/B":"G/B base-fret 0 frets x 2 0 0 3 3","G/C":"G/C base-fret 0 frets x 3 0 0 3 3","G/D":"G/D base-fret 0 frets x x 0 0 3 3","G/E":"G/E base-fret 0 frets 0 2 0 0 3 3","G/F":"G/F base-fret 0 frets 1 2 0 0 3 3","G/F#":"G/F# base-fret 0 frets 2 2 0 0 3 3","G11":"G11 base-fret 0 frets 3 x 3 2 1 x","G13":"G13 base-fret 0 frets 3 2 3 0 0 0","G13(b9)":"G13(b9) base-fret 3 frets 1 x 1 2 3 2","G2":"G2 base-fret 0 frets 3 x 0 2 3 3","G2/B":"G2/B base-fret 0 frets x 2 0 2 3 3","G2/D":"G2/D base-fret 0 frets x x 0 2 3 3","G2/E":"G2/E base-fret 0 frets x x 2 2 3 3","G2/F#":"G2/F# base-fret 0 frets 2 x 0 2 3 3","Gsus":"Gsus base-fret 0 frets 3 x 0 0 1 3","Gsus/D":"Gsus/D base-fret 0 frets x x 0 0 1 3","G5":"G5 base-fret 0 frets 3 x 0 0 3 3","G5/D":"G5/D base-fret 0 frets x x 0 0 3 3","G5/F#":"G5/F# base-fret 0 frets 2 x 0 0 3 3","G6":"G6 base-fret 0 frets 3 2 0 0 0 0","G7":"G7 base-fret 0 frets 3 2 3 0 0 3","Gaug7":"Gaug7 base-fret 0 frets 3 x 3 4 2 x","G7(#5b9)":"G7(#5b9) base-fret 0 frets 3 x 3 4 4 4","G7(#9)":"G7(#9) base-fret 0 frets 3 2 3 3 3 x","G7(b5)":"G7(b5) base-fret 0 frets 3 x 3 4 4 x","G7(b9)":"G7(b9) base-fret 0 frets 3 x 3 1 3 x","G7/B":"G7/B base-fret 0 frets x 2 3 0 3 3","G7/D":"G7/D base-fret 0 frets x x 0 0 0 1","G7sus4":"G7sus4 base-fret 0 frets 3 x 0 0 1 1","G9":"G9 base-fret 0 frets 3 x 0 2 3 3","G9(b5)":"G9(b5) base-fret 0 frets 3 x 3 2 2 x","Gaug":"Gaug base-fret 0 frets 3 2 1 0 0 3","F#9":"F#9 base-fret 0 frets 2 4 2 3 2 4","Gdim":"Gdim base-fret 0 frets x 1 2 0 2 0","Gdim7":"Gdim7 base-fret 0 frets x 1 2 0 2 0","Gm11":"Gm11 base-fret 0 frets 3 x 3 3 1 x","Gm6":"Gm6 base-fret 0 frets 3 x 2 3 3 3","Gm7(b5)":"Gm7(b5) base-fret 0 frets 3 x 3 3 2 x","Gma7":"Gma7 base-fret 0 frets 3 x 3 0 0 2","Gma9":"Gma9 base-fret 0 frets 3 x 4 2 3 x","A9(#5)":"A9(#5) base-fret 0 frets x 0 3 4 2 3","A9sus4":"A9sus4 base-fret 0 frets x 0 0 0 0 0","G#":"G# base-fret 0 frets 4 3 1 1 1 x","G#(2)":"G#(2) base-fret 0 frets 4 1 1 3 1 4","G#6":"G#6 base-fret 0 frets 4 3 1 1 1 1","G#6/9":"G#6/9 base-fret 0 frets 4 3 3 3 4 x","G#7(b9)":"G#7(b9) base-fret 0 frets x x 1 2 1 2","G#7sus4":"G#7sus4 base-fret 0 frets x x 1 1 2 2","G#9(#5)":"G#9(#5) base-fret 0 frets 4 3 2 3 x 2","G#9sus4":"G#9sus4 base-fret 0 frets 4 x 4 3 2 2","G#dim7":"G#dim7 base-fret 0 frets x x 0 1 0 1","G#m":"G#m base-fret 0 frets 4 2 1 1 4 x","G#m(ma7)":"G#m(ma7) base-fret 0 frets 4 2 1 1 x 3","G#ma7":"G#ma7 base-fret 0 frets 4 3 1 1 1 3","G#ma7(#11)":"G#ma7(#11) base-fret 3 frets 2 x 3 3 1 x","G#2":"G#2 base-fret 0 frets 4 1 1 1 4 x","Am11":"Am11 base-fret 0 frets x 0 0 0 1 0","Ama7(#11)":"Ama7(#11) base-fret 0 frets x 0 1 1 2 x","B(2)":"B(2) base-fret 0 frets x 2 1 x 2 2","B9(#5)":"B9(#5) base-fret 0 frets x 2 1 2 2 3","B9sus4":"B9sus4 base-fret 0 frets x 2 2 2 2 2","A#(2)":"A#(2) base-fret 0 frets x 1 0 3 1 x","A#5":"A#5 base-fret 0 frets x 1 3 3 x x","A#6":"A#6 base-fret 0 frets x 1 3 3 3 3","A#6/9":"A#6/9 base-fret 0 frets x 1 0 0 1 1","A#7(b9)":"A#7(b9) base-fret 0 frets x 1 0 1 0 1","A#7sus4":"A#7sus4 base-fret 0 frets x 1 3 1 4 1","A#9(#5)":"A#9(#5) base-fret 0 frets x x 0 1 1 2","A#9sus4":"A#9sus4 base-fret 0 frets x 1 1 1 1 1","A#dim7":"A#dim7 base-fret 0 frets x x 2 3 2 3","A#m":"A#m base-fret 0 frets x 1 3 0 2 x","A#m(ma7)":"A#m(ma7) base-fret 0 frets x 1 3 2 2 1","A#ma7":"A#ma7 base-fret 0 frets x 1 3 2 3 1","A#ma7(#11)":"A#ma7(#11) base-fret 0 frets x 1 2 2 3 0","Bm(ma7)":"Bm(ma7) base-fret 0 frets x 2 0 3 3 2","Bma7(#11)":"Bma7(#11) base-fret 0 frets x 2 3 3 4 x","C(2)":"C(2) base-fret 0 frets x 3 2 0 3 0","C#5":"C#5 base-fret 0 frets x x x x 2 4","C13":"C13 base-fret 2 frets x 2 1 2 2 4","C6/9":"C6/9 base-fret 0 frets x 3 2 2 3 x","C7(#9)":"C7(#9) base-fret 0 frets x 3 2 3 4 x","C7(b5)":"C7(b5) base-fret 0 frets x 3 2 3 x 2","C9(#5)":"C9(#5) base-fret 0 frets x 3 2 3 3 4","C9(b5)":"C9(b5) base-fret 0 frets x 3 2 3 3 2","C9sus4":"C9sus4 base-fret 0 frets x 3 3 3 3 3","Cm(ma7)":"Cm(ma7) base-fret 0 frets x 3 1 0 0 3","Cm13":"Cm13 base-fret 3 frets x 1 1 1 2 3","Cm6":"Cm6 base-fret 0 frets x 3 1 2 1 x","Cm7(b5)":"Cm7(b5) base-fret 0 frets x 3 4 3 4 x","Cma7(#11)":"Cma7(#11) base-fret 3 frets x 1 2 2 3 x","D(2)":"D(2) base-fret 2 frets x 4 3 1 4 0","D6/9":"D6/9 base-fret 2 frets x 4 x 3 4 1","D9(#5)":"D9(#5) base-fret 3 frets x 3 2 3 3 4","D9(b5)":"D9(b5) base-fret 3 frets x 3 2 3 3 2","D9sus4":"D9sus4 base-fret 0 frets x x 0 0 1 0","C#":"C# base-fret 0 frets x 4 3 1 2 1","C#(2)":"C#(2) base-fret 0 frets x 4 3 1 4 x","C#6":"C#6 base-fret 0 frets x 4 3 3 2 x","C#6/9":"C#6/9 base-fret 0 frets x 4 3 3 4 4","C#7(#9)":"C#7(#9) base-fret 0 frets x 4 3 4 0 0","C#7sus4":"C#7sus4 base-fret 0 frets x 4 4 4 2 x","C#9(#5)":"C#9(#5) base-fret 3 frets x 2 1 2 2 3","C#9(b5)":"C#9(b5) base-fret 0 frets x 4 3 4 4 3","C#9sus4":"C#9sus4 base-fret 0 frets x 4 4 4 4 4","C#dim7":"C#dim7 base-fret 0 frets x x 2 3 2 3","C#m":"C#m base-fret 0 frets x 4 2 x 2 4","C#m(ma7)":"C#m(ma7) base-fret 0 frets x 4 2 1 1 4","C#m11":"C#m11 base-fret 0 frets x 4 2 4 4 2","C#m13":"C#m13 base-fret 3 frets x 2 2 2 3 4","C#m6":"C#m6 base-fret 0 frets x 4 2 3 2 x","C#ma7":"C#ma7 base-fret 0 frets x 4 3 1 1 1","C#ma7(#11)":"C#ma7(#11) base-fret 3 frets x 2 3 3 4 x","C#ma9":"C#ma9 base-fret 3 frets x 2 1 4 3 x","C#sus":"C#sus base-fret 0 frets x 4 4 1 2 x","Dm9":"Dm9 base-fret 2 frets x 4 2 1 4 0","Dm(ma7)":"Dm(ma7) base-fret 0 frets x x 0 2 2 1","Dm11":"Dm11 base-fret 3 frets x 3 1 3 3 1","Dm13":"Dm13 base-fret 5 frets x 1 1 1 2 3","Dm6":"Dm6 base-fret 0 frets x x 0 2 0 1","Dma7(#11)":"Dma7(#11) base-fret 5 frets x 1 2 2 3 x","E(2)":"E(2) base-fret 0 frets 0 2 2 1 0 2","E6/9":"E6/9 base-fret 0 frets 0 x 2 1 2 2","E9(#5)":"E9(#5) base-fret 0 frets 0 x 0 1 1 2","E9(b5)":"E9(b5) base-fret 0 frets 0 1 2 1 3 2","E9sus4":"E9sus4 base-fret 0 frets 0 2 0 2 0 2","D#":"D# base-fret 0 frets x x 1 3 4 3","D#aug":"D#aug base-fret 0 frets x x 1 0 0 x","D#(2)":"D#(2) base-fret 3 frets x 4 3 1 4 x","D#(b5)":"D#(b5) base-fret 5 frets x 2 1 2 2 1","D#5":"D#5 base-fret 0 frets x x 1 3 4 x","D#6":"D#6 base-fret 0 frets x x 1 3 1 3","D#6/9":"D#6/9 base-fret 0 frets x x 1 0 1 1","D#7sus4":"D#7sus4 base-fret 0 frets x x 1 1 2 4","D#9sus4":"D#9sus4 base-fret 0 frets x x 1 1 2 1","D#m":"D#m base-fret 0 frets x x 1 3 4 2","D#m(ma7)":"D#m(ma7) base-fret 0 frets x x 1 3 3 2","D#m11":"D#m11 base-fret 3 frets x 4 2 4 4 2","D#m13":"D#m13 base-fret 5 frets x 2 2 2 3 4","D#m7":"D#m7 base-fret 0 frets x x 1 3 2 2","D#m7(b5)":"D#m7(b5) base-fret 0 frets x x 1 2 2 2","D#ma7":"D#ma7 base-fret 0 frets x x 1 3 3 3","D#ma7(#11)":"D#ma7(#11) base-fret 5 frets x 2 3 3 4 x","D#sus":"D#sus base-fret 0 frets x 1 1 3 4 4","Em(ma7)":"Em(ma7) base-fret 0 frets 0 2 1 0 0 0","Em11":"Em11 base-fret 0 frets 0 2 0 2 0 3","Em13":"Em13 base-fret 0 frets 0 4 4 0 2 0","Em6":"Em6 base-fret 0 frets 0 2 2 0 2 0","Em7(b5)":"Em7(b5) base-fret 0 frets 0 x 2 3 3 3","Ema7(#11)":"Ema7(#11) base-fret 0 frets 0 x 2 3 4 4","F(2)":"F(2) base-fret 0 frets x x 3 2 1 3","F#":"F# base-fret 0 frets 3 x 2 4 3 x","F#(2)":"F#(2) base-fret 0 frets 2 x 4 3 2 4","F#5":"F#5 base-fret 0 frets 2 4 4 x x x","F#6/9":"F#6/9 base-fret 0 frets 2 1 1 1 2 x","F#aug7":"F#aug7 base-fret 0 frets 2 x 2 3 3 x","F#7(#9)":"F#7(#9) base-fret 0 frets 2 1 2 2 x x","F#7(b9)":"F#7(b9) base-fret 0 frets x x 2 3 2 3","F#7sus4":"F#7sus4 base-fret 0 frets 1 4 2 4 2 2","F#9(#5)":"F#9(#5) base-fret 0 frets 2 1 2 1 3 x","F#9(b5)":"F#9(b5) base-fret 0 frets 2 1 2 1 3 x","F#9sus4":"F#9sus4 base-fret 0 frets 2 4 2 4 2 4","F#m":"F#m base-fret 0 frets 2 4 4 2 2 2","F#m9":"F#m9 base-fret 0 frets 2 4 4 2 2 4","F#m(ma7)":"F#m(ma7) base-fret 0 frets 2 4 3 2 2 x","F#m11":"F#m11 base-fret 0 frets 2 x 2 2 0 0","F#m13":"F#m13 base-fret 0 frets 1 3 1 1 3 1","F#ma7":"F#ma7 base-fret 0 frets 2 x 3 3 2 x","F#ma7(#11)":"F#ma7(#11) base-fret 0 frets 2 x 3 3 1 x","F#2":"F#2 base-fret 0 frets 2 4 4 x 2 4","F6/9":"F6/9 base-fret 0 frets 1 0 0 0 1 1","F7(b5)":"F7(b5) base-fret 0 frets 1 0 1 2 0 x","F9(#5)":"F9(#5) base-fret 0 frets 1 0 1 0 2 x","F9(b5)":"F9(b5) base-fret 0 frets 1 0 1 0 0 x","F9sus4":"F9sus4 base-fret 0 frets 1 3 1 3 1 3","Fm(ma7)":"Fm(ma7) base-fret 0 frets 1 x x 1 1 0","Fm11":"Fm11 base-fret 0 frets 1 3 1 3 1 4","Fm13":"Fm13 base-fret 0 frets 1 3 1 1 3 1","Fma7(#11)":"Fma7(#11) base-fret 0 frets 1 x 2 2 0 0","Fma9":"Fma9 base-fret 0 frets 1 0 2 0 1 0","G(2)":"G(2) base-fret 0 frets 3 x 0 2 0 3","G#5":"G#5 base-fret 0 frets 4 x 1 1 4 x","G6/9":"G6/9 base-fret 0 frets 3 2 2 2 3 0","G9(#5)":"G9(#5) base-fret 0 frets 3 2 3 2 4 x","G9sus4":"G9sus4 base-fret 0 frets 3 x 3 2 1 1","Gm":"Gm base-fret 0 frets 3 x 0 3 3 3","Gm9":"Gm9 base-fret 0 frets 3 1 3 2 x 1","Gm(ma7)":"Gm(ma7) base-fret 0 frets 3 x 0 3 3 2","Gm13":"Gm13 base-fret 0 frets 3 x 3 3 1 0","Gm7":"Gm7 base-fret 0 frets 3 x 3 3 3 x","Gma7(#11)":"Gma7(#11) base-fret 0 frets 3 x 4 4 2 x","A(2)":"A(2) base-fret 0 frets x 0 2 4 2 0","A(4)":"A(4) base-fret 0 frets x 0 0 2 2 0","Am2":"Am2 base-fret 0 frets x 0 2 4 1 0","A6/9":"A6/9 base-fret 0 frets x 0 2 4 2 2","A13sus4":"A13sus4 base-fret 0 frets x 0 0 0 0 2","Am(ma7)":"Am(ma7) base-fret 0 frets x x x 2 1 4","Am(ma9)":"Am(ma9) base-fret 2 frets x 0 1 4 0 3","A7(#9)":"A7(#9) base-fret 2 frets 4 x 1 4 1 2","Am7(#5)":"Am7(#5) base-fret 0 frets x 0 3 2 1 3","Ama7(#5)":"Ama7(#5) base-fret 0 frets x 0 3 1 2 1","Ama7(b5)":"Ama7(b5) base-fret 0 frets x 0 1 2 2 4","A/A#":"A/A# base-fret 0 frets x 1 2 2 2 x","A/C":"A/C base-fret 0 frets x 3 2 2 2 x","A/D#":"A/D# base-fret 0 frets x x 1 2 2 0","A/F":"A/F base-fret 0 frets 1 4 2 2 x x","Am/A#":"Am/A# base-fret 0 frets x 1 2 2 1 x","Am/B":"Am/B base-fret 0 frets x 2 2 2 1 x","Am/C#":"Am/C# base-fret 0 frets x 4 2 2 1 x","Am/D":"Am/D base-fret 0 frets x x 0 2 1 0","Am/D#":"Am/D# base-fret 0 frets x x 1 2 1 0","Am/E":"Am/E base-fret 0 frets x x 2 2 1 x","Am/F":"Am/F base-fret 0 frets 1 3 2 2 x x","Am/F#":"Am/F# base-fret 0 frets 2 3 2 2 x x","Am/G":"Am/G base-fret 0 frets 3 3 2 2 x x","Am/G#":"Am/G# base-fret 0 frets 4 3 2 2 x x","A#":"A# base-fret 0 frets x 1 3 3 3 1","A#(4)":"A#(4) base-fret 0 frets x 1 1 3 3 1","A#m2":"A#m2 base-fret 3 frets 4 1 1 4 4 x","A#aug":"A#aug base-fret 0 frets x x x 3 3 2","A#dim":"A#dim base-fret 0 frets x 1 2 3 2 x","A#13sus4":"A#13sus4 base-fret 0 frets x 1 1 1 4 3","A#m(ma9)":"A#m(ma9) base-fret 3 frets 4 2 x 3 4 3","A#m7(#5)":"A#m7(#5) base-fret 0 frets x 1 4 1 2 x","A#ma7(#5)":"A#ma7(#5) base-fret 0 frets x 1 4 2 3 2","A#ma7(b5)":"A#ma7(b5) base-fret 0 frets x 1 2 2 3 x","A#/A":"A#/A base-fret 0 frets x 0 0 3 3 1","A#/B":"A#/B base-fret 0 frets x 2 3 3 3 x","A#/C":"A#/C base-fret 0 frets x 3 3 3 3 x","A#/C#":"A#/C# base-fret 0 frets x 4 3 3 3 x","A#/D#":"A#/D# base-fret 0 frets x x 1 3 3 1","A#/E":"A#/E base-fret 0 frets 0 1 3 x 3 1","A#/F#":"A#/F# base-fret 0 frets 2 1 3 x 3 1","A#/G":"A#/G base-fret 0 frets 3 1 3 x 3 1","A#/G#":"A#/G# base-fret 0 frets 4 1 3 x 3 1","A#m/A":"A#m/A base-fret 0 frets x 0 3 3 2 1","A#m/B":"A#m/B base-fret 0 frets x 2 3 3 2 x","A#m/C":"A#m/C base-fret 0 frets x 3 3 3 2 x","A#m/C#":"A#m/C# base-fret 0 frets x 4 3 3 x x","A#m/D":"A#m/D base-fret 0 frets x x 0 3 2 1","A#m/D#":"A#m/D# base-fret 0 frets x x 1 3 2 1","A#m/E":"A#m/E base-fret 0 frets 0 1 3 3 2 1","A#m/F":"A#m/F base-fret 0 frets 1 1 3 3 2 1","A#m/F#":"A#m/F# base-fret 0 frets 2 1 x 3 2 1","A#m/G":"A#m/G base-fret 0 frets 3 1 3 x 2 1","A#m/G#":"A#m/G# base-fret 0 frets 4 1 3 x 2 1","B(4)":"B(4) base-fret 0 frets x 2 2 4 4 2","Bm2":"Bm2 base-fret 0 frets x 2 0 x 2 2","B13sus4":"B13sus4 base-fret 0 frets x 2 2 2 0 4","Bm(ma9)":"Bm(ma9) base-fret 0 frets x 2 0 3 2 x","Bm7(#5)":"Bm7(#5) base-fret 0 frets x 2 x 2 3 3","Bma7(#5)":"Bma7(#5) base-fret 0 frets x 2 1 3 4 3","Bma7(b5)":"Bma7(b5) base-fret 0 frets x 2 1 3 4 1","B/C":"B/C base-fret 0 frets x 3 4 4 4 x","B/D":"B/D base-fret 0 frets x x 0 4 4 2","B/F":"B/F base-fret 0 frets 1 2 1 x 4 2","B/G":"B/G base-fret 0 frets 3 2 4 x 4 2","Bm/A":"Bm/A base-fret 0 frets x 0 0 4 3 2","Bm/A#":"Bm/A# base-fret 0 frets x 1 x 4 3 2","Bm/C":"Bm/C base-fret 0 frets x 3 4 4 3 x","Bm/C#":"Bm/C# base-fret 0 frets x 4 4 4 3 x","Bm/D#":"Bm/D# base-fret 0 frets x x 1 4 3 2","Bm/E":"Bm/E base-fret 0 frets 0 2 0 4 3 2","Bm/F":"Bm/F base-fret 0 frets x x 3 4 3 2","Bm/F#":"Bm/F# base-fret 0 frets 2 2 4 4 3 2","Bm/G":"Bm/G base-fret 0 frets 3 2 x 4 3 2","Bm/G#":"Bm/G# base-fret 0 frets 4 2 4 x 3 2","C(4)":"C(4) base-fret 0 frets x 3 2 0 1 1","Cm2":"Cm2 base-fret 0 frets x 3 1 x 3 3","C13sus4":"C13sus4 base-fret 3 frets x 1 1 1 4 3","Cm(ma9)":"Cm(ma9) base-fret 0 frets x 3 1 4 3 3","Cm7(#5)":"Cm7(#5) base-fret 0 frets x 3 1 3 1 4","Cma7(#5)":"Cma7(#5) base-fret 0 frets x 3 2 4 x 4","Cma7(b5)":"Cma7(b5) base-fret 0 frets x 3 2 4 x 2","C/C#":"C/C# base-fret 0 frets x 4 2 x 1 3","C/D#":"C/D# base-fret 0 frets x x 1 0 1 0","C/F#":"C/F# base-fret 0 frets 2 3 2 x x 3","C/G#":"C/G# base-fret 0 frets 4 3 2 0 x 0","Cm/A":"Cm/A base-fret 0 frets x 0 1 0 1 3","Cm/A#":"Cm/A# base-fret 0 frets x 1 1 x 1 3","Cm/B":"Cm/B base-fret 0 frets x 2 1 x 1 3","Cm/C#":"Cm/C# base-fret 0 frets x 4 1 0 1 x","Cm/D":"Cm/D base-fret 3 frets x 3 3 3 2 x","Cm/E":"Cm/E base-fret 0 frets 0 3 1 0 1 x","Cm/F":"Cm/F base-fret 0 frets 1 3 1 0 x x","Cm/F#":"Cm/F# base-fret 0 frets 2 3 1 x x 3","Cm/G":"Cm/G base-fret 0 frets 3 3 1 x x x","Cm/G#":"Cm/G# base-fret 0 frets 4 3 x x 4 3","C#(4)":"C#(4) base-fret 0 frets x 4 4 1 2 1","C#m2":"C#m2 base-fret 0 frets x 4 2 1 4 4","C#aug":"C#aug base-fret 0 frets x 4 3 2 2 x","C#dim":"C#dim base-fret 0 frets x 4 2 x 2 3","C#13sus4":"C#13sus4 base-fret 3 frets x 2 2 2 0 4","C#m(ma9)":"C#m(ma9) base-fret 2 frets x 3 1 4 3 3","C#m7(#5)":"C#m7(#5) base-fret 0 frets x 4 2 2 0 0","C#ma7(#5)":"C#ma7(#5) base-fret 0 frets x 4 3 2 1 1","C#ma7(b5)":"C#ma7(b5) base-fret 0 frets x 4 3 x 1 3","C#/A":"C#/A base-fret 0 frets x 0 3 1 2 1","C#/A#":"C#/A# base-fret 0 frets x 1 3 1 2 1","C#/B":"C#/B base-fret 0 frets x 2 3 1 2 1","C#/C":"C#/C base-fret 0 frets x 3 3 1 2 1","C#/D":"C#/D base-fret 0 frets x x 0 1 2 1","C#/D#":"C#/D# base-fret 0 frets x x 1 1 2 1","C#/E":"C#/E base-fret 0 frets 0 4 3 1 2 1","C#/F":"C#/F base-fret 0 frets x x 3 1 2 1","C#/F#":"C#/F# base-fret 0 frets x x 4 1 2 1","C#/G":"C#/G base-fret 0 frets 3 x 3 1 2 1","C#m/A":"C#m/A base-fret 0 frets x 0 2 1 2 0","C#m/A#":"C#m/A# base-fret 0 frets x 1 2 1 2 x","C#m/B":"C#m/B base-fret 0 frets x 2 2 1 2 x","C#m/C":"C#m/C base-fret 0 frets x 3 2 1 2 x","C#m/D":"C#m/D base-fret 0 frets x x 0 1 2 0","C#m/D#":"C#m/D# base-fret 0 frets x x 1 1 2 0","C#m/E":"C#m/E base-fret 0 frets x x 2 1 2 x","C#m/F":"C#m/F base-fret 0 frets 1 x 2 1 2 4","C#m/F#":"C#m/F# base-fret 0 frets 2 x 2 1 2 x","C#m/G":"C#m/G base-fret 0 frets 3 4 2 1 x x","C#m/G#":"C#m/G# base-fret 0 frets 4 4 2 x x x","D(4)":"D(4) base-fret 2 frets x 4 4 1 2 1","Dm2":"Dm2 base-fret 2 frets x 4 2 1 4 4","D13sus4":"D13sus4 base-fret 5 frets x 1 1 1 4 3","Dm(ma9)":"Dm(ma9) base-fret 2 frets x 4 2 x 1 0","Dm7(#5)":"Dm7(#5) base-fret 0 frets x x 0 3 1 1","Dma7(#5)":"Dma7(#5) base-fret 0 frets x x 0 3 2 2","Dma7(b5)":"Dma7(b5) base-fret 0 frets x x 0 1 2 2","D/A#":"D/A# base-fret 0 frets x 1 4 2 3 2","D/D#":"D/D# base-fret 0 frets x x 1 2 3 2","D/F":"D/F base-fret 0 frets x x 3 2 3 2","D/G#":"D/G# base-fret 0 frets 4 x 4 2 3 2","Dm/A":"Dm/A base-fret 0 frets x x x 2 3 1","Dm/A#":"Dm/A# base-fret 0 frets x 1 3 2 3 1","Dm/B":"Dm/B base-fret 0 frets x 2 3 2 3 x","Dm/C":"Dm/C base-fret 0 frets x 3 3 2 3 x","Dm/C#":"Dm/C# base-fret 0 frets x 4 3 2 3 x","Dm/D#":"Dm/D# base-fret 0 frets x x 1 2 3 1","Dm/E":"Dm/E base-fret 0 frets 0 0 0 2 3 1","Dm/F#":"Dm/F# base-fret 0 frets 2 x 3 2 3 x","Dm/G":"Dm/G base-fret 0 frets 3 x 3 2 3 x","Dm/G#":"Dm/G# base-fret 0 frets 4 x 3 2 3 x","D#(4)":"D#(4) base-fret 3 frets x 4 4 1 2 1","D#m2":"D#m2 base-fret 3 frets x 4 2 1 4 4","D#dim":"D#dim base-fret 0 frets x x 1 2 4 2","D#m6":"D#m6 base-fret 0 frets x x 1 3 1 2","D#13sus4":"D#13sus4 base-fret 6 frets x 1 1 1 4 3","D#m(ma9)":"D#m(ma9) base-fret 4 frets x 3 1 4 3 x","D#m7(#5)":"D#m7(#5) base-fret 0 frets x x 1 4 2 2","D#ma7(#5)":"D#ma7(#5) base-fret 0 frets x x 1 4 3 3","D#ma7(b5)":"D#ma7(b5) base-fret 0 frets x x 1 2 3 3","D#9(#5)":"D#9(#5) base-fret 5 frets x 2 1 2 2 3","D#9(b5)":"D#9(b5) base-fret 5 frets x 2 1 2 2 1","D#/A":"D#/A base-fret 0 frets x 0 1 3 4 3","D#/B":"D#/B base-fret 0 frets x 2 1 3 4 3","D#/C":"D#/C base-fret 0 frets x 3 1 3 4 3","D#/C#":"D#/C# base-fret 0 frets x 4 x 3 4 3","D#/D":"D#/D base-fret 0 frets x x 0 3 4 3","D#/E":"D#/E base-fret 0 frets 0 x 1 3 4 3","D#/F":"D#/F base-fret 0 frets 1 1 1 3 4 3","D#/F#":"D#/F# base-fret 0 frets x x 4 3 4 3","D#/G":"D#/G base-fret 0 frets 3 1 1 3 4 x","D#/G#":"D#/G# base-fret 0 frets 4 x x 3 4 3","D#m/A":"D#m/A base-fret 0 frets x 0 4 3 4 2","D#m/A#":"D#m/A# base-fret 0 frets x 1 1 3 4 2","D#m/B":"D#m/B base-fret 0 frets x 2 4 3 4 2","D#m/C":"D#m/C base-fret 0 frets x 3 4 3 4 x","D#m/C#":"D#m/C# base-fret 0 frets x 4 4 3 4 x","D#m/D":"D#m/D base-fret 0 frets x x 0 3 4 2","D#m/E":"D#m/E base-fret 0 frets 0 x 4 3 4 2","D#m/F":"D#m/F base-fret 0 frets 1 1 1 3 4 2","D#m/F#":"D#m/F# base-fret 0 frets 2 x 4 3 4 2","D#m/G":"D#m/G base-fret 0 frets 3 1 1 3 x 2","D#m/G#":"D#m/G# base-fret 0 frets 4 1 1 3 x 2","E(4)":"E(4) base-fret 0 frets 0 0 2 1 0 0","Em2":"Em2 base-fret 0 frets 0 2 4 0 0 0","E13sus4":"E13sus4 base-fret 0 frets 0 0 0 2 2 2","Em(ma9)":"Em(ma9) base-fret 0 frets 0 2 1 0 0 2","Em7(#5)":"Em7(#5) base-fret 0 frets 0 3 0 0 3 0","Ema7(#5)":"Ema7(#5) base-fret 0 frets 0 3 2 1 4 0","Ema7(b5)":"Ema7(b5) base-fret 0 frets x x 2 3 4 4","E/A#":"E/A# base-fret 0 frets x 1 2 4 x 4","E/C":"E/C base-fret 0 frets x 3 2 4 x 4","E/F":"E/F base-fret 0 frets 1 2 2 1 x 4","E/G":"E/G base-fret 0 frets 3 2 2 4 x 4","Em/A":"Em/A base-fret 0 frets x 0 2 0 0 0","Em/A#":"Em/A# base-fret 0 frets x 1 2 4 x 3","Em/B":"Em/B base-fret 0 frets x 2 2 4 x 3","Em/C":"Em/C base-fret 0 frets x 3 2 4 x 3","Em/C#":"Em/C# base-fret 0 frets x 4 2 4 x 3","Em/D":"Em/D base-fret 0 frets x x 0 0 0 0","Em/D#":"Em/D# base-fret 0 frets x x 1 0 0 0","Em/F":"Em/F base-fret 0 frets 1 2 2 4 x 3","Em/F#":"Em/F# base-fret 0 frets x x 4 0 0 0","Em/G#":"Em/G# base-fret 0 frets 4 2 2 4 x 3","F(4)":"F(4) base-fret 0 frets 1 1 3 2 1 1","Fm2":"Fm2 base-fret 0 frets x x 3 1 1 3","F11":"F11 base-fret 0 frets 1 0 1 3 x x","F13sus4":"F13sus4 base-fret 0 frets 1 3 1 3 3 1","Fm(ma9)":"Fm(ma9) base-fret 0 frets 1 3 2 1 1 3","Fm7(#5)":"Fm7(#5) base-fret 0 frets 1 4 1 1 4 1","Fma7(#5)":"Fma7(#5) base-fret 0 frets 1 x 2 2 2 1","Fma7(b5)":"Fma7(b5) base-fret 0 frets 1 2 2 2 x 1","F/B":"F/B base-fret 0 frets x 2 3 2 1 1","F/C#":"F/C# base-fret 0 frets x 4 3 2 1 1","F/F#":"F/F# base-fret 0 frets 2 x 3 2 1 1","F/G#":"F/G# base-fret 0 frets 4 3 x 2 1 1","Fm/A":"Fm/A base-fret 0 frets x 0 3 1 1 1","Fm/A#":"Fm/A# base-fret 0 frets x 1 3 1 1 1","Fm/B":"Fm/B base-fret 0 frets x 2 3 1 1 1","Fm/C":"Fm/C base-fret 0 frets x 3 3 1 1 1","Fm/C#":"Fm/C# base-fret 0 frets x 4 3 1 1 1","Fm/D":"Fm/D base-fret 0 frets x x 0 1 1 1","Fm/D#":"Fm/D# base-fret 0 frets x x 1 1 1 1","Fm/E":"Fm/E base-fret 0 frets 0 3 3 1 1 1","Fm/F#":"Fm/F# base-fret 0 frets 2 3 3 1 1 1","Fm/G":"Fm/G base-fret 0 frets 3 3 3 1 1 1","Fm/G#":"Fm/G# base-fret 0 frets x x x 1 1 1","F#(4)":"F#(4) base-fret 0 frets 2 2 4 3 2 2","F#m2":"F#m2 base-fret 0 frets x x 4 2 2 4","F#aug":"F#aug base-fret 0 frets 2 x 4 3 3 2","F#dim":"F#dim base-fret 0 frets 2 3 4 2 x x","F#11":"F#11 base-fret 0 frets 2 1 2 4 x x","F#13sus4":"F#13sus4 base-fret 0 frets 2 4 2 4 4 2","F#m(ma9)":"F#m(ma9) base-fret 0 frets 2 4 3 2 2 4","F#m7(#5)":"F#m7(#5) base-fret 0 frets 2 x 2 2 3 2","F#ma7(#5)":"F#ma7(#5) base-fret 0 frets 2 x 3 3 3 2","F#ma7(b5)":"F#ma7(b5) base-fret 0 frets 2 1 3 3 1 1","F#/A":"F#/A base-fret 0 frets x 0 4 3 2 2","F#/A#":"F#/A# base-fret 0 frets x x x 3 2 2","F#/B":"F#/B base-fret 0 frets x 2 4 3 2 2","F#/C":"F#/C base-fret 0 frets x 3 4 3 2 2","F#/D":"F#/D base-fret 0 frets x x 0 3 2 2","F#/D#":"F#/D# base-fret 0 frets x x 1 3 2 2","F#/E":"F#/E base-fret 0 frets 0 4 4 3 2 2","F#/F":"F#/F base-fret 0 frets x x 3 3 2 2","F#/G":"F#/G base-fret 0 frets 3 x 4 3 2 2","F#/G#":"F#/G# base-fret 0 frets 4 x 4 3 2 2","F#m/A#":"F#m/A# base-fret 0 frets x 1 4 2 2 2","F#m/B":"F#m/B base-fret 0 frets x 2 4 2 2 2","F#m/C":"F#m/C base-fret 0 frets x 3 4 2 2 2","F#m/C#":"F#m/C# base-fret 0 frets x 4 4 2 2 2","F#m/D":"F#m/D base-fret 0 frets x x 0 2 2 2","F#m/D#":"F#m/D# base-fret 0 frets x x 1 2 2 2","F#m/E":"F#m/E base-fret 0 frets 0 0 4 2 2 2","F#m/F":"F#m/F base-fret 0 frets x x 3 2 2 2","F#m/G":"F#m/G base-fret 0 frets 3 4 4 2 2 2","F#m/G#":"F#m/G# base-fret 0 frets 4 4 4 2 2 2","G(4)":"G(4) base-fret 0 frets 3 3 x 4 3 3","Gm2":"Gm2 base-fret 0 frets 3 1 x 2 3 x","G13sus4":"G13sus4 base-fret 0 frets 3 3 3 0 3 0","Gm(ma9)":"Gm(ma9) base-fret 0 frets 3 1 4 2 x x","Gm7(#5)":"Gm7(#5) base-fret 0 frets 3 1 1 3 4 1","Gma7(#5)":"Gma7(#5) base-fret 0 frets 3 2 4 x 4 2","Gma7(b5)":"Gma7(b5) base-fret 0 frets 3 2 4 4 2 2","G/A#":"G/A# base-fret 0 frets x 1 x 4 3 3","G/C#":"G/C# base-fret 0 frets x 4 x 4 3 3","G/D#":"G/D# base-fret 0 frets x x 1 4 3 3","G/G#":"G/G# base-fret 0 frets 4 x x 4 3 3","Gm/A":"Gm/A base-fret 0 frets x 0 0 3 3 3","Gm/A#":"Gm/A# base-fret 0 frets x x x 3 3 3","Gm/B":"Gm/B base-fret 0 frets x 2 x 3 3 3","Gm/C":"Gm/C base-fret 0 frets x 3 x 3 3 3","Gm/C#":"Gm/C# base-fret 0 frets x 4 x 3 3 3","Gm/D":"Gm/D base-fret 0 frets x x 0 3 3 3","Gm/D#":"Gm/D# base-fret 0 frets x x 1 3 3 3","Gm/E":"Gm/E base-fret 0 frets 0 1 0 0 x x","Gm/F":"Gm/F base-fret 0 frets x x 3 3 3 3","Gm/F#":"Gm/F# base-fret 0 frets x x 4 3 3 3","Gm/G#":"Gm/G# base-fret 0 frets 4 x x 3 3 3","G#(4)":"G#(4) base-fret 0 frets 4 3 1 1 2 x","G#m2":"G#m2 base-fret 0 frets 4 1 1 4 4 x","G#aug":"G#aug base-fret 0 frets 4 x 2 1 1 4","G#dim":"G#dim base-fret 0 frets 4 2 x 4 3 x","G#13sus4":"G#13sus4 base-fret 0 frets 4 x 4 1 2 1","G#m(ma9)":"G#m(ma9) base-fret 0 frets 4 2 x 3 4 3","G#m7(#5)":"G#m7(#5) base-fret 0 frets 4 2 2 4 x 2","G#ma7(#5)":"G#ma7(#5) base-fret 0 frets 4 x 2 1 1 3","G#ma7(b5)":"G#ma7(b5) base-fret 0 frets 4 3 x 1 3 3","G#/A":"G#/A base-fret 0 frets x 0 1 1 1 x","G#/A#":"G#/A# base-fret 0 frets x 1 1 1 1 4","G#/B":"G#/B base-fret 0 frets x 2 1 1 1 4","G#/C":"G#/C base-fret 0 frets x 3 1 1 4 x","G#/C#":"G#/C# base-fret 0 frets x 4 1 1 1 4","G#/D":"G#/D base-fret 3 frets x 3 4 3 2 2","G#/E":"G#/E base-fret 0 frets 0 3 1 1 x x","G#/F":"G#/F base-fret 0 frets 1 3 1 1 1 4","G#/F#":"G#/F# base-fret 0 frets 2 3 1 1 4 x","G#/G":"G#/G base-fret 0 frets 3 3 1 1 4 x","G#m/A":"G#m/A base-fret 0 frets x 0 1 4 4 4","G#m/A#":"G#m/A# base-fret 0 frets x 1 1 4 4 4","G#m/B":"G#m/B base-fret 0 frets x x x 4 4 4","G#m/C":"G#m/C base-fret 0 frets x 3 1 4 4 4","G#m/C#":"G#m/C# base-fret 0 frets x 4 x 4 4 4","G#m/D":"G#m/D base-fret 0 frets x x 0 4 4 4","G#m/D#":"G#m/D# base-fret 0 frets x x 1 4 4 4","G#m/E":"G#m/E base-fret 0 frets 0 2 1 1 0 x","G#m/F":"G#m/F base-fret 0 frets 1 2 1 1 4 x","G#m/F#":"G#m/F# base-fret 0 frets x x 4 4 4 4","G#m/G":"G#m/G base-fret 0 frets 3 2 1 1 4 x","Am(4)":"Am(4) base-fret 0 frets x 0 0 2 1 0","Am6/9":"Am6/9 base-fret 0 frets x 0 4 4 1 2","Am6/7":"Am6/7 base-fret 0 frets x 0 2 0 1 2","Am7(4)":"Am7(4) base-fret 0 frets x 0 0 0 1 0","Ama11":"Ama11 base-fret 3 frets 3 2 4 x 1 x","Ama13":"Ama13 base-fret 0 frets x 0 2 1 2 2","A#m(4)":"A#m(4) base-fret 0 frets x 1 1 3 2 1","A#m6/9":"A#m6/9 base-fret 3 frets 4 2 1 3 x 1","A#m6/7":"A#m6/7 base-fret 0 frets x 1 3 1 2 3","A#m7(4)":"A#m7(4) base-fret 0 frets x 1 1 1 2 1","A#ma11":"A#ma11 base-fret 0 frets x 1 0 2 4 x","A#ma13":"A#ma13 base-fret 0 frets x 1 3 2 3 3","Bm(4)":"Bm(4) base-fret 0 frets x 2 2 4 3 2","Bm6/9":"Bm6/9 base-fret 0 frets x 2 0 1 2 2","Bm6/7":"Bm6/7 base-fret 0 frets x 2 4 2 3 4","Bm7(4)":"Bm7(4) base-fret 0 frets x 2 2 2 3 2","Bma11":"Bma11 base-fret 0 frets x 2 1 3 0 0","Bma13":"Bma13 base-fret 0 frets x 2 4 3 4 4","Cm(4)":"Cm(4) base-fret 0 frets x 3 3 x 4 3","Cm6/9":"Cm6/9 base-fret 0 frets x 3 1 2 3 x","Cm6/7":"Cm6/7 base-fret 3 frets x 1 3 1 2 3","Cm7(4)":"Cm7(4) base-fret 0 frets x 3 1 3 1 1","Cma11":"Cma11 base-fret 0 frets x 3 2 4 1 1","Cma13":"Cma13 base-fret 2 frets x 2 1 3 4 4","C#m(4)":"C#m(4) base-fret 0 frets x 4 2 1 2 2","C#m6/9":"C#m6/9 base-fret 0 frets x 4 2 3 4 4","C#m6/7":"C#m6/7 base-fret 0 frets x 4 2 3 0 0","C#m7(4)":"C#m7(4) base-fret 0 frets x 4 2 4 2 2","C#ma11":"C#ma11 base-fret 0 frets x 4 3 1 1 2","C#ma13":"C#ma13 base-fret 3 frets x 2 4 3 4 4","Dm(4)":"Dm(4) base-fret 2 frets x 4 2 1 2 2","Dm6/9":"Dm6/9 base-fret 2 frets x 4 2 1 0 0","Dm6/7":"Dm6/7 base-fret 0 frets x x 0 4 1 1","Dm7(4)":"Dm7(4) base-fret 0 frets x x 0 0 1 1","Dma11":"Dma11 base-fret 2 frets x 4 3 1 1 2","Dma13":"Dma13 base-fret 4 frets x 2 1 3 4 4","D#m(4)":"D#m(4) base-fret 3 frets x 4 2 1 2 2","D#m6/9":"D#m6/9 base-fret 3 frets x 4 2 3 4 4","D#m6/7":"D#m6/7 base-fret 5 frets x 2 4 2 3 4","D#m7(4)":"D#m7(4) base-fret 0 frets x x 1 1 2 2","D#ma11":"D#ma11 base-fret 0 frets x x 1 0 3 4","D#ma13":"D#ma13 base-fret 5 frets x 2 4 3 4 4","Em(4)":"Em(4) base-fret 0 frets 0 0 2 0 0 0","Em6/9":"Em6/9 base-fret 0 frets 0 2 2 0 2 2","Em6/7":"Em6/7 base-fret 0 frets 0 4 0 0 0 0","Em7(4)":"Em7(4) base-fret 0 frets x x 2 2 3 3","Ema11":"Ema11 base-fret 0 frets 0 x 4 2 4 4","Ema13":"Ema13 base-fret 0 frets 0 2 1 1 2 0","Fm(4)":"Fm(4) base-fret 0 frets 1 1 3 1 1 1","Fm6/9":"Fm6/9 base-fret 0 frets 1 3 x 1 3 3","Fm6/7":"Fm6/7 base-fret 0 frets 1 3 1 1 3 1","Fm7(4)":"Fm7(4) base-fret 0 frets 1 1 1 1 1 1","Fma11":"Fma11 base-fret 0 frets 1 0 2 3 1 0","Fma13":"Fma13 base-fret 0 frets 1 x 2 2 3 1","F#m(4)":"F#m(4) base-fret 0 frets 2 2 4 2 2 2","F#m6/9":"F#m6/9 base-fret 0 frets 2 x 4 2 4 4","F#m6/7":"F#m6/7 base-fret 0 frets 2 4 2 2 4 2","F#m7(4)":"F#m7(4) base-fret 0 frets 2 2 2 2 2 2","F#ma11":"F#ma11 base-fret 0 frets 2 1 3 4 x 1","F#ma13":"F#ma13 base-fret 0 frets 2 1 3 1 4 1","Gm(4)":"Gm(4) base-fret 0 frets 3 3 x 3 3 3","Gm6/9":"Gm6/9 base-fret 0 frets 3 1 2 2 3 x","Gm6/7":"Gm6/7 base-fret 0 frets 3 1 2 3 x 1","Gm7(4)":"Gm7(4) base-fret 0 frets 3 3 3 3 3 3","Gma11":"Gma11 base-fret 0 frets 3 2 4 x 1 x","Gma13":"Gma13 base-fret 0 frets 3 2 4 0 0 0","G#m(4)":"G#m(4) base-fret 0 frets 4 2 1 1 2 x","G#m6/9":"G#m6/9 base-fret 0 frets 4 2 1 3 x 1","G#m6/7":"G#m6/7 base-fret 0 frets 4 2 3 4 x 2","G#m7(4)":"G#m7(4) base-fret 0 frets 4 4 4 4 4 4","G#ma11":"G#ma11 base-fret 0 frets 4 3 x x 2 3","G#ma13":"G#ma13 base-fret 0 frets 4 3 x 0 2 1"}');var $t=class i{constructor(e){this.definitions=e||{}}get(e){return this.definitions[e]||null}withDefaults(){let e=yM(ki),A=this.clone();return Object.keys(e).forEach(t=>{let c=KB.parse(e[t]);A.has(t)||A.add(t,c)}),A}add(e,A){this.definitions[e]=A}has(e){return e in this.definitions}clone(){let e=new i;return Object.keys(this.definitions).forEach(A=>{e.add(A,this.definitions[A].clone())}),e}},Un=$t,qt=class i{constructor({font:e,size:A,colour:t}={font:null,size:null,colour:null}){this.font=null,this.size=null,this.colour=null,this.font=e?e.replace(/"/g,"'"):null,this.size=A||null,this.colour=t||null}clone(){return new i({font:this.font,size:this.size,colour:this.colour})}toCssString(){let e={};return this.colour&&(e.color=this.colour),this.font&&this.size?e.font=`${this.size} ${this.font}`:this.font?e["font-family"]=this.font:this.size&&(e["font-size"]=`${this.size}`),Object.keys(e).map(A=>`${A}: ${e[A]}`).join("; ")}},et=qt,_t=class i{constructor({type:e,items:A}={type:ve,items:[]}){this.items=[],this.type=ve,this.currentChordLyricsPair=new H,this.key=null,this.transposeKey=null,this.lineNumber=null,this.selector=null,this.selectorIsNegated=!1,this.textFont=new et,this.chordFont=new et,this.type=e,this.items=A}isEmpty(){return this.items.length===0}addItem(e){if(e instanceof J)this.addTag(e);else if(e instanceof H)this.addChordLyricsPair(e);else if(e instanceof _e)this.addComment(e);else{let A=e;A.parentLine=this,this.items.push(A)}}hasRenderableItems(){return this.items.some(e=>e.isRenderable())}clone(){return this.mapItems(null)}mapItems(e){let A=new i;return A.items=this.items.map(t=>{let c=t.clone();return e?e(c):c}).filter(t=>t!==null),A.type=this.type,A}isBridge(){return this.type===Ms}isChorus(){return this.type===Fe}isGrid(){return this.type===dB}isTab(){return this.type===GB}isVerse(){return this.type===rs}isPart(){return this.type===Bt}hasContent(){return this.hasRenderableItems()}addChordLyricsPair(e=null,A=null){return e instanceof H?this.currentChordLyricsPair=e:this.currentChordLyricsPair=new H(e||"",A||""),this.currentChordLyricsPair.parentLine=this,this.items.push(this.currentChordLyricsPair),this.currentChordLyricsPair}ensureChordLyricsPair(){this.currentChordLyricsPair||this.addChordLyricsPair()}chords(e){this.ensureChordLyricsPair(),this.currentChordLyricsPair.chords+=e}lyrics(e){this.ensureChordLyricsPair(),this.currentChordLyricsPair.lyrics+=e}addTag(e,A=null){let t=e instanceof J?e:new J(e,A);return t.parentLine=this,this.items.push(t),t}addComment(e){let A=e instanceof _e?e:new _e(e);return A.parentLine=this,this.items.push(A),A}set(e){return new i({type:this.type,items:this.items,...e})}get _tag(){if(this.items.length!==1)return null;let e=this.items[0];return e instanceof J?e:null}isSectionStart(){return this._tag?.isSectionStart()||!1}isSectionEnd(){return this._tag?.isSectionEnd()||!1}},Ye=_t,AB=class i{static expand(e,A){return new i(e,A).expand()}constructor(e,A){this.line=e,this.song=A}expand(){let e=this.line.items.flatMap(A=>A instanceof J&&A.name===Fe?this.getLastChorusBefore(this.line.lineNumber):[]);return[this.line,...e]}getLastChorusBefore(e){let A=[];if(!e)return A;for(let t=e-1;t>=0;t-=1){let c=this.song.lines[t];if(c.type!==Fe&&A.length>0)break;c.type===Fe&&(c.isEmpty()||this.lineHasMoreThanChorusDirectives(c))&&A.unshift(c)}return A}lineHasMoreThanChorusDirectives(e){return e.items.some(A=>!(A instanceof J&&(A.name===os||A.name===ct)))}},Yn=AB,eB=class{get key(){return this.getSingleMetadataValue(Ue)}get title(){return this.getSingleMetadataValue(WB)}get subtitle(){return this.getSingleMetadataValue(PB)}get capo(){return this.getMetadataValue(ns)}get duration(){return this.getSingleMetadataValue(Ci)}get tempo(){return this.getSingleMetadataValue(ji)}get time(){return this.getMetadataValue(bi)}get year(){return this.getSingleMetadataValue(Ti)}get album(){return this.getMetadataValue(Qi)}get copyright(){return this.getSingleMetadataValue(wi)}get lyricist(){return this.getMetadataValue(Ui)}get artist(){return this.getMetadataValue(fi)}get composer(){return this.getMetadataValue(Ii)}},Vi=eB;function mn(i,e){i.includes(e)||i.push(e)}var sB=class i extends Vi{constructor(e={}){super(),this.metadata={},this.providers=new Map,e instanceof i?this.assign(e.metadata):this.assign(e)}merge(e={}){let A=this.clone();return e instanceof i?A.assign(e.metadata):A.assign(e),A}contains(e){return e in this.metadata}add(e,A){if(!Lc(e)){if(!(e in this.metadata)){this.metadata[e]=A;return}this.appendValue(e,A)}}appendValue(e,A){let t=this.metadata[e];if(t!==A){if(t instanceof Array){mn(t,A);return}this.metadata[e]=[t,A]}}setProvider(e,A){this.providers.set(e,A)}set(e,A){A?this.metadata[e]=A:delete this.metadata[e]}getMetadataValue(e){return this.get(e)}getSingleMetadataValue(e){return this.getSingle(e)}get(e){if(e===Js)return this.calculateKeyFromCapo();if(e in this.metadata)return this.metadata[e];let A=this.providers.get(e);return A?A():this.getArrayItem(e)}all(){let e={};this.providers.forEach((t,c)=>{let M=t();M!==null&&(e[c]=M)}),Object.assign(e,this.metadata);let A=this.calculateKeyFromCapo();return A&&(e[Js]=A),e}ownMetadata(){let e={...this.metadata},A=this.calculateKeyFromCapo();return A&&(e[Js]=A),e}[Symbol.iterator](){return Object.entries(this.ownMetadata())[Symbol.iterator]()}getSingle(e){let A=this.get(e);return Array.isArray(A)?A[0]:A}parseArrayKey(e){let A=e.match(/(.+)\.(-?\d+)$/);if(!A)return null;let t=A[1],c=parseInt(A[2],10);return[t,c]}getArrayItem(e){let A=this.parseArrayKey(e);if(A===null)return null;let[t,c]=A,M=this.metadata[t]||[],o=c;return o<0?o=M.length+o:o>0&&(o-=1),M[o]}clone(){let e=new i(this.metadata);return this.providers.forEach((A,t)=>e.setProvider(t,A)),e}calculateKeyFromCapo(){let e=this.getSingle(ns),A=this.getSingle(Ue);if(e&&A){let t=eA.parse(A);if(!t)throw new Error(`Could not parse ${A}`);let c=parseInt(e,10);return t.transpose(c).normalize().toString()}return null}assign(e){Object.keys(e).filter(A=>!Lc(A)).forEach(A=>{let t=e[A];t!=null&&(t instanceof Array?this.metadata[A]=[...t]:t===null?delete this.metadata[A]:this.metadata[A]=t.toString())})}},st=sB;function Ai(i,e){let A=[...new Set(i)];return A.length===1?A[0]:e}var tB=class{addLine(e){this.lines.push(e)}isLiteral(){let{lines:e}=this;return this.isEmpty()?!1:e.every(A=>A.items.every(t=>!!(t instanceof OA||t instanceof J&&t.isSectionDelimiter())))}get contents(){return this.lines.filter(e=>e.items.every(A=>A instanceof OA)).map(e=>e.items.map(A=>A.string).join("")).join(`
`)}get label(){if(this.lines.length===0)return null;let e=this.lines[0].items.find(A=>A instanceof J&&A.isSectionDelimiter());return e?e.label:null}get type(){let e=this.lines.map(A=>A.type);return Ai(e,dM)}get selector(){let e=this.lines.map(A=>A.selector).filter(A=>A!==null);return Ai(e,null)}get selectorIsNegated(){return this.lines.some(e=>e.selectorIsNegated)}hasRenderableItems(){return this.lines.some(e=>e.hasRenderableItems())}isEmpty(){return this.lines.length===0}constructor(){this.lines=[]}},As=tB,BB=class i{constructor(e,A){this.fontSize=e,this.unit=A}clone(){return new i(this.fontSize,this.unit)}multiply(e){return new i(this.fontSize*e/100,this.unit)}toString(){return`${this.fontSize}${this.unit}`}static parse(e,A){let t=e.trim(),c=parseFloat(t);return Number.isNaN(c)?this.parseNotANumber(A):t.slice(-1)==="%"?this.parsePercentage(c,A):new i(c,"px")}static parseNotANumber(e){return e?e.clone():new i(100,"%")}static parsePercentage(e,A){return A?A.multiply(e):new i(e,"%")}},Dn=BB,cB=class{applyTag(e){switch(e.name){case vs:this.textFont.font=this.pushOrPopTag(e);break;case Ks:this.textFont.size=this.pushOrPopSizeTag(e);break;case xt:this.textFont.colour=this.pushOrPopTag(e);break;case Zs:this.chordFont.font=this.pushOrPopTag(e);break;case Xs:this.chordFont.size=this.pushOrPopSizeTag(e);break;case ut:this.chordFont.colour=this.pushOrPopTag(e);break;default:break}}pushOrPopTag(e){let{value:A}=e;return e.hasValue()?this.fontAndColourStacks[e.name].push(A):(this.fontAndColourStacks[e.name].pop(),A=this.fontAndColourStacks[e.name].slice(-1)[0]||null),A}pushOrPopSizeTag(e){let{value:A}=e;if(e.hasValue()){let t=this.sizeStacks[e.name].slice(-1)[0]||null,c=Dn.parse(A,t);return this.sizeStacks[e.name].push(c),c}return this.sizeStacks[e.name].pop(),this.sizeStacks[e.name].slice(-1)[0]||null}constructor(){this.fontAndColourStacks={[ut]:[],[Zs]:[],[xt]:[],[vs]:[]},this.sizeStacks={[Xs]:[],[Ks]:[]},this.textFont=new et,this.chordFont=new et}},Nn=cB,iB=class{constructor(e,A,t){this.lineNumber=null,this.column=null,this.message=e,this.lineNumber=A,this.column=t}toString(){return`Warning: ${this.message} on line ${this.lineNumber||"?"} column ${this.column||"?"}`}},jn=iB,gB=class{constructor(e){this.currentKey=null,this.currentLine=null,this.fontStack=new Nn,this.lines=[],this.metadata=new st,this.sectionType=ve,this.selector=null,this.selectorIsNegated=!1,this.transposeKey=null,this.warnings=[],this.song=e,this.song.lines=this.lines,this.song.warnings=this.warnings}get previousLine(){let e=this.lines.length;return e>=2?this.lines[e-2]:null}addLine(e){return e?this.currentLine=e:(this.currentLine=new Ye,this.lines.push(this.currentLine)),this.setCurrentProperties(this.sectionType,this.selector,this.selectorIsNegated),this.currentLine.transposeKey=this.transposeKey??this.currentKey,this.currentLine.key=this.currentKey||this.song.getMetadata().getSingle(Ue),this.currentLine.lineNumber=this.lines.length-1,this.currentLine}setCurrentProperties(e,A=null,t=!1){if(!this.currentLine)throw new Error("Expected this.currentLine to be present");this.currentLine.type=e,this.currentLine.selector=A,this.currentLine.selectorIsNegated=t,this.currentLine.textFont=this.fontStack.textFont.clone(),this.currentLine.chordFont=this.fontStack.chordFont.clone()}addItem(e){if(e instanceof J)this.addTag(e);else{if(this.ensureLine(),!this.currentLine)throw new Error("Expected this.currentLine to be present");this.currentLine.addItem(e)}}chords(e){if(!this.currentLine)throw new Error("Expected this.currentLine to be present");this.currentLine.chords(e)}lyrics(e){if(this.ensureLine(),!this.currentLine)throw new Error("Expected this.currentLine to be present");this.currentLine.lyrics(e)}addTag(e){let A=J.parseOrFail(e);return this.applyTagOnSong(A),this.applyTagOnLine(A),A}ensureLine(){this.currentLine===null&&this.addLine()}applyTagOnSong(e){e.name===VM?this.transposeKey=e.value:e.name===zi?this.currentKey=e.value:e.isSectionDelimiter()?this.setSectionTypeFromTag(e):e.isInlineFontTag()&&this.fontStack.applyTag(e)}applyTagOnLine(e){if(this.ensureLine(),!this.currentLine)throw new Error("Expected this.currentLine to be present");this.currentLine.addTag(e)}setSectionTypeFromTag(e){let[A,t]=Ht.interpret(e.name,e.value);t&&(A===Ke?this.startSection(t,e):A===Oe&&this.endSection(t===ai?this.sectionType:t,e))}startSection(e,A){this.checkCurrentSectionType(ve,A),this.selector=A.selector,this.selectorIsNegated=A.isNegated,e===Bt&&A.value?this.sectionType=A.value.split(" ")[0].toLowerCase():this.sectionType=e,this.setCurrentProperties(e,A.selector,A.isNegated)}endSection(e,A){this.checkCurrentSectionType(e,A),this.sectionType=ve,this.selector=null,this.selectorIsNegated=!1}checkCurrentSectionType(e,A){this.sectionType!==e&&!(e==="part"&&A.name==="end_of_part")&&this.addWarning(`Unexpected tag {${A.originalName}}, current section is: ${this.sectionType}`,A)}addWarning(e,{line:A,column:t}){let c=new jn(e,A||null,t||null);this.warnings.push(c)}},tt=gB,MB=class{constructor(e){this.addedLine=!1,this.song=e,this.clonedSong=new cs,this.builder=new tt(this.clonedSong)}mapItems(e){return this.song.lines.forEach(A=>{this.mapLineItems(A,e)}),this.clonedSong}mapLineItems(e,A){e.items.forEach(t=>{this.mapItem(A,t)}),e.isEmpty()&&this.ensureLine(),this.addedLine=!1}mapItem(e,A){let t=e(A);if(t===null)return;let c=Array.isArray(t);(!c||t.length>0)&&this.ensureLine(),c?t.forEach(M=>this.builder.addItem(M)):this.builder.addItem(t)}ensureLine(){this.addedLine||(this.builder.addLine(),this.addedLine=!0)}},bn=MB;function zn(i){let e=i.getFullYear(),A=String(i.getMonth()+1).padStart(2,"0"),t=String(i.getDate()).padStart(2,"0");return`${e}-${A}-${t}`}function Li(){let i=new Map;return i.set("chordpro",()=>"ChordPro"),i.set("chordpro.version",()=>"14.0.0"),i.set("today",()=>zn(new Date)),i}function Tn(i){let e=Li(),{instrument:A,user:t}=i;return e.set("instrument",()=>A?.type??null),e.set("instrument.type",()=>A?.type??null),e.set("instrument.description",()=>A?.description??null),e.set("tuning",()=>A?.tuning??null),e.set("user",()=>t?.name??null),e.set("user.name",()=>t?.name??null),e.set("user.fullname",()=>t?.fullname??null),e}var rB=class i extends Vi{constructor(e=null){super(),this.lines=[],this._bodyLines=null,this._bodyParagraphs=null,this._renderParagraphs=null,this.warnings=[],this._metadata=null,e&&(this._metadata=new st(e))}get bodyLines(){return this._bodyLines??=this.selectRenderableItems(this.lines),this._bodyLines}get bodyParagraphs(){return this._bodyParagraphs??=this.selectRenderableItems(this.paragraphs),this._bodyParagraphs}get renderParagraphs(){return this._renderParagraphs??this.bodyParagraphs}set renderParagraphs(e){this._renderParagraphs=e}selectRenderableItems(e){let A=[...e];for(;A.length&&!A[0].hasRenderableItems();)A.shift();return A}get paragraphs(){return this.linesToParagraphs(this.lines)}get expandedBodyParagraphs(){let e=this.lines.flatMap(A=>Yn.expand(A,this));return this.selectRenderableItems(this.linesToParagraphs(e))}filterParagraphs(e,A){let t={configuration:A,metadata:this.metadata};return e.filter(c=>{let{selector:M,selectorIsNegated:o}=c;return M?bt({selector:M,isNegated:o},t):!0})}linesToParagraphs(e){let A=new As,t=[A];return e.forEach((c,M)=>{let o=e[M+1]||null;c.isEmpty()||c.isSectionEnd()&&o&&!o.isEmpty()?(A=new As,t.push(A)):c.hasRenderableItems()&&A.addLine(c)}),t}clone(){let e=new i;return e.warnings=[...this.warnings],e.lines=this.lines.map(A=>A.clone()),e}getMetadataValue(e){return this.metadata.getMetadataValue(e)}getSingleMetadataValue(e){return this.metadata.getSingleMetadataValue(e)}setKey(e){let A=e?e.toString():null;return this.changeMetadata(Ue,A)}setCapo(e){let A=e?e.toString():null;return this.changeMetadata(ns,A)}updateDirectives(e){let A={...e},t=Or(A,(M,o)=>o!==null&&!this.metadata.contains(M)),c=this.changeOrDeleteDirectives(A);return c.insertDirectives(t),c}changeOrDeleteDirectives(e){let A={...e};return this.mapItems(t=>{if(!(t instanceof J))return t;let c=t;if(!(c.name in A&&c.isMetaTag()))return t;let M=A[c.name];return M===null?null:(A[c.name]=null,[M].flat().map(o=>new J(c.name,o)))})}insertDirectives(e,{after:A=null}={}){let t=this.findInsertIndex(A),c=this.adjustInsertIndex(t),M=this.createMetadataLines(e),o=this.prepareLinesToInsert(M,t,c);this.spliceLines(c,o)}findInsertIndex(e){return this.lines.findIndex(A=>A.items.some(t=>this.isInsertionPoint(t,e)))}isInsertionPoint(e,A){return!(e instanceof J)||A&&e.name===A?!0:e.isComment()||e.isSectionDelimiter()}adjustInsertIndex(e){if(e<=0)return e;for(let A=e-1;A>=0;A-=1)if(!this.lines[A].isEmpty())return A+1;return 0}createMetadataLines(e){return Object.keys(e).flatMap(A=>[e[A]].flat().map(c=>{let M=new Ye;return M.addTag(A,c),M}))}prepareLinesToInsert(e,A,t){let c=[...e];return!(A>t&&this.lines.slice(t,A).some(o=>o.isEmpty()))&&e.length>0&&c.push(new Ye),c}spliceLines(e,A){let{lines:t}=this;this.lines=[...t.slice(0,e),...A,...t.slice(e)]}transpose(e,{accidental:A=null,normalizeChordSuffix:t=!1}={}){let c=null;return this.mapItems(M=>M instanceof J&&M.name===Ue?(c=eA.wrapOrFail(M.value).transpose(e).normalize(),A&&(c=c.useAccidental(A)),M.set({value:c.toString()})):M instanceof H?i.transposeChordLyricsPair(M,e,c,t,A):M instanceof OA&&i.isMusicalSection(M.parentLine)?i.transposeLiteral(M,e,c,t,A):M)}static transposeChordLyricsPair(e,A,t,c,M){let o=e.transpose(A,t,{normalizeChordSuffix:c});return M&&(o=o.useAccidental(M)),o}static transposeLiteral(e,A,t,c,M){return i.mapChordsInLiteral(e,o=>{let a=o.transpose(A);return t&&(a=a.normalize(t,{normalizeSuffix:c})),M&&(a=a.useAccidental(M)),a})}transposeUp({normalizeChordSuffix:e=!1}={}){return this.transpose(1,{normalizeChordSuffix:e})}transposeDown({normalizeChordSuffix:e=!1}={}){return this.transpose(-1,{normalizeChordSuffix:e})}changeKey(e){let A=this.requireCurrentKey(),t=eA.wrapOrFail(e),c=A.distanceTo(t);return this.transpose(c,{accidental:t.accidental})}useAccidental(e){let{currentKey:A}=this,t=this.mapChordLyricsPairs(c=>c.useAccidental(e));return A&&A.accidental!==e&&(t=t.changeKey(A.useAccidental(e))),t}useModifier(e){return HA("useModifier is deprecated, use useAccidental instead"),this.useAccidental(e)}normalizeChords(e=null,{normalizeSuffix:A=!0}={}){return this.changeChords(t=>t.normalize(e,{normalizeSuffix:A}))}mapChordLyricsPairs(e){return this.mapItems(A=>A instanceof H?e(A):A)}changeChords(e){return this.mapItems(A=>A instanceof H?A.changeChord(e):A instanceof OA&&i.isMusicalSection(A.parentLine)?i.mapChordsInLiteral(A,e):A)}static isMusicalSection(e){return e===null||![xB,uB,pB,HB].includes(e.type)}static mapChordsInLiteral(e,A){let t=e.string.replace(/(\s|^)(\S+)(?=\s|$)/g,(c,M,o)=>{let a=$A.parse(o);return a?`${M}${A(a).toString()}`:`${M}${o}`});return new OA(t)}get currentKey(){return eA.wrap(this.key)}requireCurrentKey(){let{currentKey:e}=this;if(!e)throw new Error(`
Cannot change song key, the original key is unknown.

Either ensure a key directive is present in the song (when using chordpro):
  \`{key: C}\`

Or set the song key before changing key:
  \`song.setKey('C');\``.substring(1));return e}changeMetadata(e,A){if(typeof e=="string"){if(typeof A>"u")throw new Error("Value is required when name is a string");return this.changeMetadata({[e]:A})}let t=this.updateDirectives(e);return t.metadata.assign(e),t}addLine(e){this.lines.push(e)}mapItems(e){return new bn(this).mapItems(e)}foreachItem(e){this.lines.forEach(A=>{A.items.forEach(e)})}getChords(){let e=new Set;return this.foreachItem(A=>{if(!(A instanceof H))return;let t=A.chords;if(t&&t.length>0){let c=$A.parse(t);c&&e.add(c.toString())}}),Array.from(e)}getChordDefinitions(e){let A={};return this.foreachItem(t=>{if(!(t instanceof J))return;let{chordDefinition:c,selector:M,isNegated:o}=t;M&&e&&!bt({selector:M,isNegated:o},e)||c&&(A[c.name]=c.clone())}),A}get metadata(){return this._metadata||(this._metadata=this.getMetadata()),this._metadata}getMetadata(e){let A=new st;(e?Tn(e):Li()).forEach((M,o)=>A.setProvider(o,M));let c=()=>this.getChords();return A.setProvider("chords",()=>{let M=c();return M.length?M.join(", "):null}),A.setProvider("numchords",()=>c().length.toString()),A.setProvider("key_actual",()=>A.getSingle("_key")??A.getSingle("key")),A.setProvider("key_from",()=>A.getSingle("key")),this.foreachItem(M=>{if(!(M instanceof J))return;let o=M;if(!o.isMetaTag())return;let{selector:a,isNegated:U}=o;a&&e&&!bt({selector:a,isNegated:U},{metadata:A,configuration:e})||A.add(M.name,M.value)}),A}get chordDefinitions(){return new Un(this.getChordDefinitions())}mapLines(e){let A=new i,t=new tt(A);return this.lines.forEach(c=>{let M=e(c);M&&(t.addLine(),M.items.forEach(o=>t.addItem(o)))}),A}updateItem(e,A,t){let c=!1,M=this.mapItems(o=>e(o)?(c=!0,A(o)):o);return c?M:t(M)}removeItem(e){return this.mapLines(A=>{let{items:t}=A,c=t.findIndex(e);return c===-1?A:t.length===1?null:A.set({items:[...t.slice(0,c),...t.slice(c+1)]})})}},cs=rB,nB=class i extends $B{constructor(e,A=null){super(),this.expressions=[],this.expressions=e,this.variable=A}evaluate(e,A){return this.expressions.map(t=>t.evaluate(e,A,this.variable)).join("")}isRenderable(){return!0}clone(){return new i(this.expressions.map(e=>e.clone()),this.variable)}},ei=nB,oB=class extends Error{constructor(e,A=null,t=null,c=null){super(`${e} on line ${A} column ${t}`),this.line=null,this.column=null,this.offset=null,this.name="ExpressionError",this.line=A,this.column=t,this.offset=c}},yn=oB,EB=class i extends $B{constructor({variable:e=null,valueTest:A=null,trueExpression:t=[],falseExpression:c=[],line:M=null,column:o=null,offset:a=null}){super({line:M,column:o,offset:a}),this.trueExpression=[],this.falseExpression=[],this.variable=e||null,this.valueTest=A||null,this.trueExpression=t,this.falseExpression=c}evaluate(e,A,t=null){if(this.variable)return this.evaluateWithVariable(e,A);if(!t)throw new yn("Unexpected empty expression",this.line,this.column,this.offset);return this.evaluateToString(e.get(t)||"",A)}evaluateToString(e,A){return Array.isArray(e)?e.join(A):e}evaluateWithVariable(e,A){if(!this.variable)throw new Error("Expected this.variable to be present");let t=e.get(this.variable);return t&&(XB(this.valueTest)||t===this.valueTest)?this.evaluateForTruthyValue(e,A,t):this.falseExpression.length?new ei(this.falseExpression,this.variable).evaluate(e,A):""}evaluateForTruthyValue(e,A,t){return this.trueExpression.length?new ei(this.trueExpression,this.variable).evaluate(e,A):this.evaluateToString(t,A)}isRenderable(){return!0}clone(){return new i({variable:this.variable,valueTest:this.valueTest,trueExpression:this.trueExpression.map(e=>e.clone()),falseExpression:this.falseExpression.map(e=>e.clone()),line:this.line,column:this.column,offset:this.offset})}},si=EB,ti="chordLyricsPair",Bi="chordSheet",ci="comment",ii="line",gi="softLineBreak",Mi="tag",ri="ternary",aB=class{serialize(e){return{type:Bi,lines:e.lines.map(A=>this.serializeLine(A))}}serializeLine(e){return{type:ii,items:e.items.map(A=>this.serializeItem(A))}}serializeItem(e){let t=new Map([[J,this.serializeTag.bind(this)],[H,this.serializeChordLyricsPair.bind(this)],[si,this.serializeTernary.bind(this)],[OA,this.serializeLiteral.bind(this)],[_e,this.serializeComment.bind(this)],[gA,()=>({type:gi})]]).get(e.constructor);if(!t)throw new Error(`Don't know how to serialize ${e.constructor.name}`);return t(e)}serializeChordDefinition(e){return{name:e.name,baseFret:e.baseFret,frets:e.frets,fingers:e.fingers}}serializeTag(e){let A={type:Mi,name:e.originalName,value:e.value,attributes:e.attributes||{}};return e.chordDefinition&&(A.chordDefinition=this.serializeChordDefinition(e.chordDefinition)),A}serializeChordLyricsPair(e){let A={type:ti,chords:e.chords,chord:null,lyrics:e.lyrics,annotation:e.annotation};return e.isRhythmSymbol&&(A.isRhythmSymbol=!0),A}serializeTernary(e){return{type:ri,variable:e.variable,valueTest:e.valueTest,trueExpression:this.serializeExpression(e.trueExpression),falseExpression:this.serializeExpression(e.falseExpression)}}serializeLiteral(e){return e.string}serializeExpression(e){return e.map(A=>this.serializeItem(A))}serializeComment(e){return{type:ci,comment:e.content}}deserialize(e,A={}){return this.notation=A.notation??null,this.parseAstComponent(e),this.song}parseAstComponent(e){if(typeof e=="string")return new OA(e);switch(e.type){case Bi:this.parseChordSheet(e);break;case ti:return this.parseChordLyricsPair(e);case ci:return this.parseComment(e);case gi:return new gA;case Mi:return this.parseTag(e);case ri:return this.parseTernary(e);case ii:this.parseLine(e);break;default:Si(`Unhandled AST component "${e.type}"`)}return null}parseChordSheet(e){let{lines:A}=e;this.song=new cs,this.songBuilder=new tt(this.song),A.forEach(t=>this.parseAstComponent(t))}parseLine(e){let{items:A}=e;this.songBuilder.addLine(),A.forEach(t=>{let c=this.parseAstComponent(t);this.songBuilder.addItem(c)})}parseChordLyricsPair(e){let{chord:A,chords:t,lyrics:c,annotation:M,isRhythmSymbol:o}=e,a=A?new $A(A).toString():t,U=this.notation&&a?$A.parse(a,{notation:this.notation}):null;return new H(U?U.toString():a,c,M,U,o||!1)}parseTag(e){let{name:A,value:t,location:{offset:c=null,line:M=null,column:o=null}={},chordDefinition:a,attributes:U,selector:Y,isNegated:N}=e,R=new J(A,t,{line:M,column:o,offset:c},U);return R.selector=Y||null,R.isNegated=N||!1,a&&(R.chordDefinition=new KB(a.name,a.baseFret,a.frets,a.fingers)),R}parseComment(e){let{comment:A}=e;return new _e(A)}parseTernary(e){let{variable:A,valueTest:t,trueExpression:c,falseExpression:M,location:{offset:o=null,line:a=null,column:U=null}={}}=e;return new si({variable:A,valueTest:t,trueExpression:this.parseExpression(c),falseExpression:this.parseExpression(M),offset:o,line:a,column:U})}parseExpression(e){return(e||[]).map(A=>this.parseAstComponent(A)).filter(A=>A!==null)}constructor(){this.song=new cs,this.songBuilder=new tt(this.song),this.notation=null}},Rn=aB,QB=class{trace(){}},dn=QB;function Gn(i){return i.replace(/\n$/,"").split(`
`)}function Ss(i){return{type:"line",items:i}}function un(i,e,A){return[Ss([i]),...Gn(A).map(t=>Ss([t])),Ss([e])]}function zt(i,e,A,t){return{type:"tag",name:i,location:t.start,value:e?.value||"",attributes:e?.attributes||{},selector:A?.value,isNegated:A?.isNegated}}function xn(i,e,A,t=c=>c){let c=new RegExp(e,"g"),M=Array.from(i.matchAll(c)),o=[],a=0;M.forEach(Y=>{let N=i.slice(a,Y.index);N!==""&&o.push(t(N)),o.push(A(Y[0])),a=Y.index+Y[0].length});let U=i.slice(a);return U!==""&&o.push(t(U)),o}function Pi(i){return xn(i,"\xA0",()=>({type:"softLineBreak"}),e=>({type:"chordLyricsPair",chords:"",lyrics:e}))}function pn(i,e){let A=Pi(e||""),[t,...c]=A,M=A[0],o=null;return i!==""&&(!M||M.type==="softLineBreak"?o={type:"chordLyricsPair",chords:i,lyrics:""}:M={...M,chords:i}),[o,M||null,...c].filter(a=>a!==null)}function ni(i){return typeof i!="string"&&i.type==="chordLyricsPair"}function Hn(i,e){return ni(i)&&ni(e)&&i.chords.length>0&&e.chords.length===0}function Jn(i,e){return{...i,lyrics:`${i.lyrics}${e.lyrics}`}}function Sn(i,e){if(e!==!1)return i;let A=[];for(let t=0,{length:c}=i;t<c;t+=1)i[t+1]&&Hn(i[t],i[t+1])?(A.push(Jn(i[t],i[t+1])),t+=1):A.push(i[t]);return A}function Tt(i,e,A){return A=A||" ",i.length>e?i:(e-=i.length,A+=A.repeat(e),i+A.slice(0,e))}var es=class i extends Error{static buildMessage(e,A){function t(Y){return Y.charCodeAt(0).toString(16).toUpperCase()}function c(Y){return Y.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function M(Y){return Y.replace(/\\/g,"\\\\").replace(/\]/g,"\\]").replace(/\^/g,"\\^").replace(/-/g,"\\-").replace(/\0/g,"\\0").replace(/\t/g,"\\t").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/[\x00-\x0F]/g,N=>"\\x0"+t(N)).replace(/[\x10-\x1F\x7F-\x9F]/g,N=>"\\x"+t(N))}function o(Y){switch(Y.type){case"literal":return'"'+c(Y.text)+'"';case"class":let N=Y.parts.map(R=>Array.isArray(R)?M(R[0])+"-"+M(R[1]):M(R));return"["+(Y.inverted?"^":"")+N+"]";case"any":return"any character";case"end":return"end of input";case"other":return Y.description}}function a(Y){let N=Y.map(o),R,W;if(N.sort(),N.length>0){for(R=1,W=1;R<N.length;R++)N[R-1]!==N[R]&&(N[W]=N[R],W++);N.length=W}switch(N.length){case 1:return N[0];case 2:return N[0]+" or "+N[1];default:return N.slice(0,-1).join(", ")+", or "+N[N.length-1]}}function U(Y){return Y?'"'+c(Y)+'"':"end of input"}return"Expected "+a(e)+" but "+U(A)+" found."}constructor(e,A,t,c){super(),this.message=e,this.expected=A,this.found=t,this.location=c,this.name="PeggySyntaxError",typeof Object.setPrototypeOf=="function"?Object.setPrototypeOf(this,i.prototype):this.__proto__=i.prototype,typeof Error.captureStackTrace=="function"&&Error.captureStackTrace(this,i)}format(e){let A="Error: "+this.message;if(this.location){let t=null,c;for(c=0;c<e.length;c++)if(e[c].grammarSource===this.location.source){t=e[c].text.split(/\r\n|\n|\r/g);break}let M=this.location.start,o=this.location.source+":"+M.line+":"+M.column;if(t){let a=this.location.end,U=Tt("",M.line.toString().length," "),Y=t[M.line-1],N=M.line===a.line?a.column:Y.length+1;A+=`
 --> `+o+`
`+U+` |
`+M.line+" | "+Y+`
`+U+" | "+Tt("",M.column-1," ")+Tt("",N-M.column,"^")}else A+=`
 at `+o}return A}};function kn(i,e){e=e!==void 0?e:{};let A={},t=e.grammarSource,c={ChordSheet:hc},M=hc,o=function(s,g){return{type:"chordSheet",lines:[...s,g].flat()}},a=function(s){return s},U=function(s,g,r,n){return Ss([s?{type:"chordLyricsPair",chords:"",lyrics:s}:null,...Sn(g.flat(),e.chopFirstWord),r?{type:"chordLyricsPair",chords:r,lyrics:""}:null,n?{type:"comment",comment:n}:null].filter(f=>f!==null))},Y=function(s){return Pi(s)},N="#",R=x("#",!1),W=/^[^\r\n]/,qA=AA(["\r",`
`],!0,!1),me=function(s){return s},_A=function(s,g,r){return{type:"chordLyricsPair",annotation:s||"",lyrics:g+(r||"")}},De=function(s,g,r){let n=g.map(f=>f.char||f).join("")+(r||"");return pn(s||"",n)},Ne=function(s){return s.map(g=>g.char||g).join("")},lA="[*",hA=x("[*",!1),QA="]",fA=x("]",!1),Ae=function(s){return s.map(g=>g.char||g).join("")},JA="[",ee=x("[",!1),je=function(s){return s.map(g=>g.char||g).join("")},se=/^[^\]\r\n]/,be=AA(["]","\r",`
`],!0,!1),FA="\\",UA=x("\\",!1),ze=function(){return{type:"char",char:"\\"}},Te=function(){return{type:"char",char:"]"}},YA=function(s){return s},SA="%{",te=x("%{",!1),Z="}",K=x("}",!1),Be=function(s,g,r){return{type:"ternary",variable:s.length>0?s:null,valueTest:g,...r,location:we().start}},kA="=",TA=x("=",!1),ce=function(s){return s},yA="|",mA=x("|",!1),ye=function(s,g){return{type:"ternary",trueExpression:s,falseExpression:g,location:we().start}},Re=function(s){return s},RA=/^[a-zA-Z0-9\-_]/,dA=AA([["a","z"],["A","Z"],["0","9"],"-","_"],!1,!1),b="%",q=x("%",!1),v="{",k=x("{",!1),GA=function(){return{type:"char",char:"%"}},L=function(){return{type:"char",char:"]"}},DA=function(){return{type:"char",char:"|"}},uA=function(){return{type:"char",char:"}"}},Qs=/^[^|[\]\\{}%#\r\n\t ]/,fs=AA(["|","[","]","\\","{","}","%","#","\r",`
`,"	"," "],!0,!1),ie=function(){return{type:"char",char:"\\"}},tA=function(){return{type:"char",char:"["}},BA=function(){return{type:"char",char:"{"}},Is=function(){return{type:"char",char:"#"}},de=" ",VA=x(" ",!1),ge=function(){return{type:"char",char:e?.softLineBreaks?"\xA0":" "}},IA="chord",X=x("chord",!1),Me="define",re=x("define",!1),LA=":",ne=x(":",!1),Ge=function(s,g,r){let{text:n,...f}=r;return{type:"tag",name:s,value:n,chordDefinition:f,location:we().start,selector:g?.value,isNegated:g?.isNegated}},ue=function(s,g,r){return zt(s,r,g,we())},oe="-",xe=x("-",!1),pe=function(s,g){return{value:s,isNegated:!!g}},He="!",Ee=x("!",!1),Je=function(){return!0},Se=function(s){return s},PA=function(s){return{value:s}},ke=function(s){return{attributes:s}},NA=function(s){let g={};return s.forEach(r=>{g[r[0]]=r[1]}),g},xA=function(s){return s},Ve=function(s,g){return[s,g]},Q=/^[a-zA-Z_]/,V=AA([["a","z"],["A","Z"],"_"],!1,!1),oA=function(s){return s.map(g=>g.char||g).join("")},I=/^[^}\\\r\n]/,F=AA(["}","\\","\r",`
`],!0,!1),j=/^[a-zA-Z\-_]/,d=AA([["a","z"],["A","Z"],"-","_"],!1,!1),_='"',MA=x('"',!1),cA=function(s){return s},qB=/^[^"}]/,P=AA(['"',"}"],!0,!1),WA=function(){return{type:"char",char:'"'}},Mt="frets",rt=x("frets",!1),nt=function(s,g,r,n){return{name:s,baseFret:g||1,frets:r,fingers:n,text:Ng()}},ws="/",ZA=x("/",!1),p=/^[A-Ga-g]/,ot=AA([["A","G"],["a","g"]],!1,!1),Cs=/^[b#\u266D\u266F]/,ls=AA(["b","#","\u266D","\u266F"],!1,!1),EA="es",hs=x("es",!1),Fs="s",Us=x("s",!1),Le="is",Ys=x("is",!1),ms=/^[a-zA-Z0-9#\u266Fb\u266D()+\-\/\xF8\u0394\u2212]/,Pe=AA([["a","z"],["A","Z"],["0","9"],"#","\u266F","b","\u266D","(",")","+","-","/","\xF8","\u0394","\u2212"],!1,!1),We="base-fret",Ds=x("base-fret",!1),Ns=function(s){return s},Ze="fingers",js=x("fingers",!1),bs=function(s){return s},ae=function(s){return s},Et=/^[\-A-Za-z]/,zs=AA(["-",["A","Z"],["a","z"]],!1,!1),E=/^[0-9]/,C=AA([["0","9"]],!1,!1),m=function(s){return parseInt(s,10)},T=function(s){return s},u="0",Xi=x("0",!1),vi=function(){return 0},_B="-1",Ki=x("-1",!1),Oi=/^[NXnx]/,$i=AA(["N","X","n","x"],!1,!1),Qe=function(s,g,r){return un(s,r,g)},Ac="start_of_abc",qi=x("start_of_abc",!1),fe=function(s,g,r){return zt(s,r,g,we())},ec="end_of_abc",_i=x("end_of_abc",!1),Ie=function(s){return zt(s,null,null,we())},sc="start_of_grid",Ag=x("start_of_grid",!1),tc="sog",eg=x("sog",!1),Bc="end_of_grid",sg=x("end_of_grid",!1),cc="eog",tg=x("eog",!1),ic="start_of_ly",Bg=x("start_of_ly",!1),gc="end_of_ly",cg=x("end_of_ly",!1),Mc="start_of_svg",ig=x("start_of_svg",!1),rc="end_of_svg",gg=x("end_of_svg",!1),nc="start_of_tab",Mg=x("start_of_tab",!1),oc="sot",rg=x("sot",!1),Ec="end_of_tab",ng=x("end_of_tab",!1),ac="eot",og=x("eot",!1),Qc="start_of_textblock",Eg=x("start_of_textblock",!1),fc="end_of_textblock",ag=x("end_of_textblock",!1),Qg=jg(),fg=Rs("whitespace"),Ig=Rs("optional whitespace"),wg=/^[ \t\n\r]/,Cg=AA([" ","	",`
`,"\r"],!1,!1),lg=Rs("space"),Ic=/^[ \t]/,wc=AA([" ","	"],!1,!1),hg=/^[\n\r]/,Fg=AA([`
`,"\r"],!1,!1),Ug=`
`,Yg=x(`
`,!1),mg="\r",Dg=x("\r",!1),B=0,y=0,Ts=[{line:1,column:1}],wA=0,at=[],h=0,ys;if(e.startRule!==void 0){if(!(e.startRule in c))throw new Error(`Can't start parsing from rule "`+e.startRule+'".');M=c[e.startRule]}function Ng(){return i.substring(y,B)}function we(){return Xe(y,B)}function go(s,g){throw g=g!==void 0?g:Xe(y,B),lc([Rs(s)],i.substring(y,B),g)}function Mo(s,g){throw g=g!==void 0?g:Xe(y,B),zg(s,g)}function x(s,g){return{type:"literal",text:s,ignoreCase:g}}function AA(s,g,r){return{type:"class",parts:s,inverted:g,ignoreCase:r}}function jg(){return{type:"any"}}function bg(){return{type:"end"}}function Rs(s){return{type:"other",description:s}}function Cc(s){let g=Ts[s],r;if(g)return g;for(r=s-1;!Ts[r];)r--;for(g=Ts[r],g={line:g.line,column:g.column};r<s;)i.charCodeAt(r)===10?(g.line++,g.column=1):g.column++,r++;return Ts[s]=g,g}function Xe(s,g){let r=Cc(s),n=Cc(g);return{source:t,start:{offset:s,line:r.line,column:r.column},end:{offset:g,line:n.line,column:n.column}}}function D(s){B<wA||(B>wA&&(wA=B,at=[]),at.push(s))}function zg(s,g){return new es(s,[],"",g)}function lc(s,g,r){return new es(es.buildMessage(s,g),s,g,r)}function hc(){let s,g,r;for(s=B,g=[],r=Fc();r!==A;)g.push(r),r=Fc();return g!==A?(r=Uc(),r===A&&(r=null),r!==A?(y=s,g=o(g,r),s=g):(B=s,s=A)):(B=s,s=A),s}function Fc(){let s,g,r;return s=B,g=Uc(),g!==A?(r=KA(),r!==A?(y=s,g=a(g),s=g):(B=s,s=A)):(B=s,s=A),s}function Uc(){let s;return s=rM(),s===A&&(s=Tg()),s}function Tg(){let s,g,r,n,f,w,l;if(s=B,g=B,r=ds(),r===A&&(r=null),r!==A?g=i.substring(g,B):g=r,g!==A){for(r=[],n=Yc();n!==A;)r.push(n),n=Yc();if(r!==A)if(n=Dc(),n===A&&(n=null),n!==A)if(f=yg(),f===A&&(f=null),f!==A){for(w=[],l=jA();l!==A;)w.push(l),l=jA();w!==A?(y=s,g=U(g,r,n,f),s=g):(B=s,s=A)}else B=s,s=A;else B=s,s=A;else B=s,s=A}else B=s,s=A;return s}function Yc(){let s,g;return s=Sg(),s===A&&(s=kg(),s===A&&(s=Rg(),s===A&&(s=dg(),s===A&&(s=Qt(),s===A&&(s=B,g=ds(),g!==A&&(y=s,g=Y(g)),s=g))))),s}function yg(){let s,g,r,n,f,w;if(s=B,g=jA(),g===A&&(g=null),g!==A)if(i.charCodeAt(B)===35?(r=N,B++):(r=A,h===0&&D(R)),r!==A){for(n=B,f=[],W.test(i.charAt(B))?(w=i.charAt(B),B++):(w=A,h===0&&D(qA));w!==A;)f.push(w),W.test(i.charAt(B))?(w=i.charAt(B),B++):(w=A,h===0&&D(qA));f!==A?n=i.substring(n,B):n=f,n!==A?(y=s,g=me(n),s=g):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function Rg(){let s,g,r,n,f,w;if(s=B,g=Gg(),g!==A){for(r=B,n=[],f=ds();f!==A;)n.push(f),f=ds();if(n!==A?r=i.substring(r,B):r=n,r!==A){for(n=B,f=[],w=jA();w!==A;)f.push(w),w=jA();f!==A?n=i.substring(n,B):n=f,n!==A?(y=s,g=_A(g,r,n),s=g):(B=s,s=A)}else B=s,s=A}else B=s,s=A;return s}function dg(){let s,g,r,n,f,w;if(s=B,g=Dc(),g!==A){for(r=[],n=ft();n!==A;)r.push(n),n=ft();if(r!==A){for(n=B,f=[],w=jA();w!==A;)f.push(w),w=jA();f!==A?n=i.substring(n,B):n=f,n!==A?(y=s,g=De(g,r,n),s=g):(B=s,s=A)}else B=s,s=A}else B=s,s=A;return s}function ds(){let s,g,r;if(s=B,g=[],r=mc(),r!==A)for(;r!==A;)g.push(r),r=mc();else g=A;return g!==A&&(y=s,g=Ne(g)),s=g,s}function mc(){let s;return s=ft(),s===A&&(s=jA()),s}function Gg(){let s,g,r,n,f;if(s=B,g=B,h++,r=le(),h--,r===A?g=void 0:(B=g,g=A),g!==A)if(i.substr(B,2)===lA?(r=lA,B+=2):(r=A,h===0&&D(hA)),r!==A){if(n=[],f=Gs(),f!==A)for(;f!==A;)n.push(f),f=Gs();else n=A;n!==A?(i.charCodeAt(B)===93?(f=QA,B++):(f=A,h===0&&D(fA)),f!==A?(y=s,g=Ae(n),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function Dc(){let s,g,r,n,f;if(s=B,g=B,h++,r=le(),h--,r===A?g=void 0:(B=g,g=A),g!==A)if(i.charCodeAt(B)===91?(r=JA,B++):(r=A,h===0&&D(ee)),r!==A){for(n=[],f=Gs();f!==A;)n.push(f),f=Gs();n!==A?(i.charCodeAt(B)===93?(f=QA,B++):(f=A,h===0&&D(fA)),f!==A?(y=s,g=je(n),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function Gs(){let s,g,r,n;return se.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(be)),s===A&&(s=B,g=le(),g!==A?(r=B,i.charCodeAt(B)===92?(n=FA,B++):(n=A,h===0&&D(UA)),n!==A&&(y=r,n=ze()),r=n,r===A&&(r=B,i.charCodeAt(B)===93?(n=QA,B++):(n=A,h===0&&D(fA)),n!==A&&(y=r,n=Te()),r=n),r!==A?(y=s,g=YA(r),s=g):(B=s,s=A)):(B=s,s=A)),s}function Qt(){let s,g,r,n,f,w,l,z,S;return s=B,i.substr(B,2)===SA?(g=SA,B+=2):(g=A,h===0&&D(te)),g!==A?(r=G(),r!==A?(n=B,f=Jg(),f===A&&(f=null),f!==A?n=i.substring(n,B):n=f,n!==A?(f=ug(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=pg(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=Be(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function ug(){let s,g,r,n;return s=B,i.charCodeAt(B)===61?(g=kA,B++):(g=A,h===0&&D(TA)),g!==A?(r=G(),r!==A?(n=xg(),n!==A?(y=s,g=ce(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function xg(){let s,g,r;if(s=B,g=[],r=Ce(),r!==A)for(;r!==A;)g.push(r),r=Ce();else g=A;return g!==A?s=i.substring(s,B):s=g,s}function pg(){let s,g,r,n,f,w,l;return s=B,i.charCodeAt(B)===124?(g=yA,B++):(g=A,h===0&&D(mA)),g!==A?(r=G(),r!==A?(n=Nc(),n!==A?(f=G(),f!==A?(w=Hg(),w===A&&(w=null),w!==A?(l=G(),l!==A?(y=s,g=ye(n,w),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Hg(){let s,g,r,n;return s=B,i.charCodeAt(B)===124?(g=yA,B++):(g=A,h===0&&D(mA)),g!==A?(r=G(),r!==A?(n=Nc(),n!==A?(y=s,g=Re(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Jg(){let s,g;if(s=[],RA.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(dA)),g!==A)for(;g!==A;)s.push(g),RA.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(dA));else s=A;return s}function Nc(){let s,g,r,n;if(s=[],g=B,r=[],n=Ce(),n!==A)for(;n!==A;)r.push(n),n=Ce();else r=A;if(r!==A?g=i.substring(g,B):g=r,g===A&&(g=Qt()),g!==A)for(;g!==A;){if(s.push(g),g=B,r=[],n=Ce(),n!==A)for(;n!==A;)r.push(n),n=Ce();else r=A;r!==A?g=i.substring(g,B):g=r,g===A&&(g=Qt())}else s=A;return s}function ft(){let s,g,r,n;return s=jc(),s===A&&(s=B,i.charCodeAt(B)===37?(g=b,B++):(g=A,h===0&&D(q)),g!==A?(r=B,h++,i.charCodeAt(B)===123?(n=v,B++):(n=A,h===0&&D(k)),h--,n===A?r=void 0:(B=r,r=A),r!==A?(y=s,g=GA(),s=g):(B=s,s=A)):(B=s,s=A),s===A&&(s=B,i.charCodeAt(B)===93?(g=QA,B++):(g=A,h===0&&D(fA)),g!==A&&(y=s,g=L()),s=g,s===A&&(s=B,i.charCodeAt(B)===124?(g=yA,B++):(g=A,h===0&&D(mA)),g!==A&&(y=s,g=DA()),s=g,s===A&&(s=B,i.charCodeAt(B)===125?(g=Z,B++):(g=A,h===0&&D(K)),g!==A&&(y=s,g=uA()),s=g)))),s}function Ce(){let s;return s=jc(),s===A&&(s=jA()),s}function jc(){let s,g,r,n;return Qs.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(fs)),s===A&&(s=B,g=le(),g!==A?(r=B,i.charCodeAt(B)===92?(n=FA,B++):(n=A,h===0&&D(UA)),n!==A&&(y=r,n=ie()),r=n,r===A&&(r=B,i.charCodeAt(B)===124?(n=yA,B++):(n=A,h===0&&D(mA)),n!==A&&(y=r,n=DA()),r=n,r===A&&(r=B,i.charCodeAt(B)===91?(n=JA,B++):(n=A,h===0&&D(ee)),n!==A&&(y=r,n=tA()),r=n,r===A&&(r=B,i.charCodeAt(B)===93?(n=QA,B++):(n=A,h===0&&D(fA)),n!==A&&(y=r,n=L()),r=n,r===A&&(r=B,i.charCodeAt(B)===123?(n=v,B++):(n=A,h===0&&D(k)),n!==A&&(y=r,n=BA()),r=n,r===A&&(r=B,i.charCodeAt(B)===125?(n=Z,B++):(n=A,h===0&&D(K)),n!==A&&(y=r,n=uA()),r=n,r===A&&(r=B,i.charCodeAt(B)===37?(n=b,B++):(n=A,h===0&&D(q)),n!==A&&(y=r,n=GA()),r=n,r===A&&(r=B,i.charCodeAt(B)===35?(n=N,B++):(n=A,h===0&&D(R)),n!==A&&(y=r,n=Is()),r=n,r===A&&(r=B,i.charCodeAt(B)===32?(n=de,B++):(n=A,h===0&&D(VA)),n!==A&&(y=r,n=ge()),r=n)))))))),r!==A?(y=s,g=YA(r),s=g):(B=s,s=A)):(B=s,s=A)),s}function Sg(){let s,g,r,n,f,w,l,z,S,xc,Ut;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,5)===IA?(n=IA,B+=5):(n=A,h===0&&D(X)),n===A&&(i.substr(B,6)===Me?(n=Me,B+=6):(n=A,h===0&&D(re))),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(i.charCodeAt(B)===58?(l=LA,B++):(l=A,h===0&&D(ne)),l!==A?(z=G(),z!==A?(S=$g(),S!==A?(xc=G(),xc!==A?(i.charCodeAt(B)===125?(Ut=Z,B++):(Ut=A,h===0&&D(K)),Ut!==A?(y=s,g=Ge(n,f,S),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function kg(){let s,g,r,n,f,w,l,z;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(n=B,f=vg(),f!==A?n=i.substring(n,B):n=f,n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=XA(),w===A&&(w=Pg()),w===A&&(w=null),w!==A?(l=G(),l!==A?(i.charCodeAt(B)===125?(z=Z,B++):(z=A,h===0&&D(K)),z!==A?(y=s,g=ue(n,f,w),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function pA(){let s,g,r,n;return s=B,i.charCodeAt(B)===45?(g=oe,B++):(g=A,h===0&&D(xe)),g!==A?(r=Lg(),r!==A?(n=Vg(),n===A&&(n=null),n!==A?(y=s,g=pe(r,n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Vg(){let s,g;return s=B,i.charCodeAt(B)===33?(g=He,B++):(g=A,h===0&&D(Ee)),g!==A&&(y=s,g=Je()),s=g,s}function Lg(){let s,g,r;if(s=B,g=[],RA.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(dA)),r!==A)for(;r!==A;)g.push(r),RA.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(dA));else g=A;return g!==A?s=i.substring(s,B):s=g,s}function XA(){let s,g,r,n;return s=B,g=G(),g!==A?(i.charCodeAt(B)===58?(r=LA,B++):(r=A,h===0&&D(ne)),r!==A?(n=Wg(),n!==A?(y=s,g=Se(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Pg(){let s,g,r;return s=B,g=vA(),g!==A?(r=zc(),r!==A?(y=s,g=PA(r),s=g):(B=s,s=A)):(B=s,s=A),s}function Wg(){let s,g;return s=B,g=Zg(),g!==A&&(y=s,g=ke(g)),s=g,s===A&&(s=B,g=zc(),g!==A&&(y=s,g=PA(g)),s=g),s}function Zg(){let s,g,r;if(s=B,g=[],r=bc(),r!==A)for(;r!==A;)g.push(r),r=bc();else g=A;return g!==A&&(y=s,g=NA(g)),s=g,s}function bc(){let s,g,r;return s=B,g=vA(),g!==A?(r=Xg(),r!==A?(y=s,g=xA(r),s=g):(B=s,s=A)):(B=s,s=A),s}function Xg(){let s,g,r,n,f,w;return s=B,g=Kg(),g!==A?(r=G(),r!==A?(i.charCodeAt(B)===61?(n=kA,B++):(n=A,h===0&&D(TA)),n!==A?(f=G(),f!==A?(w=Og(),w!==A?(y=s,g=Ve(g,w),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function vg(){let s,g;if(s=[],Q.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(V)),g!==A)for(;g!==A;)s.push(g),Q.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(V));else s=A;return s}function zc(){let s,g,r,n;if(s=B,g=G(),g!==A){for(r=[],n=Tc();n!==A;)r.push(n),n=Tc();r!==A?(y=s,g=oA(r),s=g):(B=s,s=A)}else B=s,s=A;return s}function Tc(){let s,g,r,n;return I.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(F)),s===A&&(s=B,g=le(),g!==A?(r=B,i.charCodeAt(B)===92?(n=FA,B++):(n=A,h===0&&D(UA)),n!==A&&(y=r,n=ie()),r=n,r===A&&(r=B,i.charCodeAt(B)===125?(n=Z,B++):(n=A,h===0&&D(K)),n!==A&&(y=r,n=uA()),r=n,r===A&&(r=B,i.charCodeAt(B)===123?(n=v,B++):(n=A,h===0&&D(k)),n!==A&&(y=r,n=BA()),r=n)),r!==A?(y=s,g=YA(r),s=g):(B=s,s=A)):(B=s,s=A)),s}function Kg(){let s,g,r;if(s=B,g=[],j.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(d)),r!==A)for(;r!==A;)g.push(r),j.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(d));else g=A;return g!==A?s=i.substring(s,B):s=g,s}function Og(){let s,g,r,n,f;if(s=B,i.charCodeAt(B)===34?(g=_,B++):(g=A,h===0&&D(MA)),g!==A){for(r=B,n=[],f=yc();f!==A;)n.push(f),f=yc();n!==A?r=i.substring(r,B):r=n,r!==A?(i.charCodeAt(B)===34?(n=_,B++):(n=A,h===0&&D(MA)),n!==A?(y=s,g=cA(r),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;return s}function yc(){let s,g,r,n;return qB.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(P)),s===A&&(s=B,g=le(),g!==A?(r=B,i.charCodeAt(B)===92?(n=FA,B++):(n=A,h===0&&D(UA)),n!==A&&(y=r,n=ie()),r=n,r===A&&(r=B,i.charCodeAt(B)===125?(n=Z,B++):(n=A,h===0&&D(K)),n!==A&&(y=r,n=uA()),r=n,r===A&&(r=B,i.charCodeAt(B)===34?(n=_,B++):(n=A,h===0&&D(MA)),n!==A&&(y=r,n=WA()),r=n)),r!==A?(y=s,g=YA(r),s=g):(B=s,s=A)):(B=s,s=A)),s}function $g(){let s,g,r,n,f,w,l;if(s=B,g=qg(),g!==A)if(r=G(),r!==A)if(n=sM(),n===A&&(n=null),n!==A)if(i.substr(B,5)===Mt?(f=Mt,B+=5):(f=A,h===0&&D(rt)),f!==A){if(w=[],l=Gc(),l!==A)for(;l!==A;)w.push(l),l=Gc();else w=A;w!==A?(l=tM(),l===A&&(l=null),l!==A?(y=s,g=nt(g,n,w,l),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;else B=s,s=A;else B=s,s=A;return s}function qg(){let s,g,r,n;return s=B,g=B,r=_g(),r!==A?(n=AM(),n===A&&(n=null),n!==A?(r=[r,n],g=r):(B=g,g=A)):(B=g,g=A),g!==A?s=i.substring(s,B):s=g,s}function _g(){let s,g,r,n;return s=B,g=B,r=Rc(),r!==A?(n=eM(),n===A&&(n=null),n!==A?(r=[r,n],g=r):(B=g,g=A)):(B=g,g=A),g!==A?s=i.substring(s,B):s=g,s}function AM(){let s,g,r,n;return s=B,g=B,i.charCodeAt(B)===47?(r=ws,B++):(r=A,h===0&&D(ZA)),r!==A?(n=Rc(),n!==A?(r=[r,n],g=r):(B=g,g=A)):(B=g,g=A),g!==A?s=i.substring(s,B):s=g,s}function Rc(){let s,g,r,n;return s=B,g=B,p.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(ot)),r!==A?(Cs.test(i.charAt(B))?(n=i.charAt(B),B++):(n=A,h===0&&D(ls)),n===A&&(i.substr(B,2)===EA?(n=EA,B+=2):(n=A,h===0&&D(hs)),n===A&&(i.charCodeAt(B)===115?(n=Fs,B++):(n=A,h===0&&D(Us)),n===A&&(i.substr(B,2)===Le?(n=Le,B+=2):(n=A,h===0&&D(Ys))))),n===A&&(n=null),n!==A?(r=[r,n],g=r):(B=g,g=A)):(B=g,g=A),g!==A?s=i.substring(s,B):s=g,s}function eM(){let s,g,r;if(s=B,g=[],ms.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(Pe)),r!==A)for(;r!==A;)g.push(r),ms.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(Pe));else g=A;return g!==A?s=i.substring(s,B):s=g,s}function sM(){let s,g,r,n,f;return s=B,i.substr(B,9)===We?(g=We,B+=9):(g=A,h===0&&D(Ds)),g!==A?(r=vA(),r!==A?(n=uc(),n!==A?(f=vA(),f!==A?(y=s,g=Ns(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function tM(){let s,g,r,n,f;if(s=B,g=vA(),g!==A)if(i.substr(B,7)===Ze?(r=Ze,B+=7):(r=A,h===0&&D(js)),r!==A){if(n=[],f=dc(),f!==A)for(;f!==A;)n.push(f),f=dc();else n=A;n!==A?(y=s,g=bs(n),s=g):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function dc(){let s,g,r;return s=B,g=vA(),g!==A?(r=BM(),r!==A?(y=s,g=ae(r),s=g):(B=s,s=A)):(B=s,s=A),s}function BM(){let s;return s=cM(),s===A&&(Et.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(zs))),s}function cM(){let s,g;return s=B,E.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(C)),g!==A&&(y=s,g=m(g)),s=g,s}function Gc(){let s,g,r;return s=B,g=vA(),g!==A?(r=iM(),r!==A?(y=s,g=T(r),s=g):(B=s,s=A)):(B=s,s=A),s}function iM(){let s,g,r;return s=B,g=G(),g!==A?(r=uc(),r===A&&(r=gM(),r===A&&(r=MM())),r!==A?(y=s,g=T(r),s=g):(B=s,s=A)):(B=s,s=A),s}function uc(){let s,g;return s=B,E.test(i.charAt(B))?(g=i.charAt(B),B++):(g=A,h===0&&D(C)),g!==A&&(y=s,g=m(g)),s=g,s}function gM(){let s,g;return s=B,i.charCodeAt(B)===48?(g=u,B++):(g=A,h===0&&D(Xi)),g!==A&&(y=s,g=vi()),s=g,s}function MM(){let s;return i.substr(B,2)===_B?(s=_B,B+=2):(s=A,h===0&&D(Ki)),s===A&&(Oi.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D($i))),s}function rM(){let s;return s=nM(),s===A&&(s=EM(),s===A&&(s=QM(),s===A&&(s=IM(),s===A&&(s=CM(),s===A&&(s=hM()))))),s}function nM(){let s,g,r,n,f,w,l,z;if(s=B,g=oM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=It(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=It(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=It(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function oM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,12)===Ac?(n=Ac,B+=12):(n=A,h===0&&D(qi)),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function It(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,10)===ec?(n=ec,B+=10):(n=A,h===0&&D(_i)),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function EM(){let s,g,r,n,f,w,l,z;if(s=B,g=aM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=wt(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=wt(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=wt(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function aM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,13)===sc?(n=sc,B+=13):(n=A,h===0&&D(Ag)),n===A&&(i.substr(B,3)===tc?(n=tc,B+=3):(n=A,h===0&&D(eg))),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function wt(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,11)===Bc?(n=Bc,B+=11):(n=A,h===0&&D(sg)),n===A&&(i.substr(B,3)===cc?(n=cc,B+=3):(n=A,h===0&&D(tg))),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function QM(){let s,g,r,n,f,w,l,z;if(s=B,g=fM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=Ct(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=Ct(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=Ct(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function fM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,11)===ic?(n=ic,B+=11):(n=A,h===0&&D(Bg)),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Ct(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,9)===gc?(n=gc,B+=9):(n=A,h===0&&D(cg)),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function IM(){let s,g,r,n,f,w,l,z;if(s=B,g=wM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=lt(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=lt(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=lt(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function wM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,12)===Mc?(n=Mc,B+=12):(n=A,h===0&&D(ig)),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function lt(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,10)===rc?(n=rc,B+=10):(n=A,h===0&&D(gg)),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function CM(){let s,g,r,n,f,w,l,z;if(s=B,g=lM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=ht(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=ht(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=ht(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function lM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,12)===nc?(n=nc,B+=12):(n=A,h===0&&D(Mg)),n===A&&(i.substr(B,3)===oc?(n=oc,B+=3):(n=A,h===0&&D(rg))),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function ht(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,10)===Ec?(n=Ec,B+=10):(n=A,h===0&&D(ng)),n===A&&(i.substr(B,3)===ac?(n=ac,B+=3):(n=A,h===0&&D(og))),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function hM(){let s,g,r,n,f,w,l,z;if(s=B,g=FM(),g!==A)if(r=KA(),r!==A){for(n=B,f=[],w=B,l=B,h++,z=Ft(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);w!==A;)f.push(w),w=B,l=B,h++,z=Ft(),h--,z===A?l=void 0:(B=l,l=A),l!==A?(z=rA(),z!==A?(l=[l,z],w=l):(B=w,w=A)):(B=w,w=A);f!==A?n=i.substring(n,B):n=f,n!==A?(f=Ft(),f!==A?(y=s,g=Qe(g,n,f),s=g):(B=s,s=A)):(B=s,s=A)}else B=s,s=A;else B=s,s=A;return s}function FM(){let s,g,r,n,f,w,l,z,S;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,18)===Qc?(n=Qc,B+=18):(n=A,h===0&&D(Eg)),n!==A?(f=pA(),f===A&&(f=null),f!==A?(w=G(),w!==A?(l=XA(),l===A&&(l=null),l!==A?(z=G(),z!==A?(i.charCodeAt(B)===125?(S=Z,B++):(S=A,h===0&&D(K)),S!==A?(y=s,g=fe(n,f,l),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function Ft(){let s,g,r,n,f,w;return s=B,i.charCodeAt(B)===123?(g=v,B++):(g=A,h===0&&D(k)),g!==A?(r=G(),r!==A?(i.substr(B,16)===fc?(n=fc,B+=16):(n=A,h===0&&D(ag)),n!==A?(f=G(),f!==A?(i.charCodeAt(B)===125?(w=Z,B++):(w=A,h===0&&D(K)),w!==A?(y=s,g=Ie(n),s=g):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A)):(B=s,s=A),s}function rA(){let s;return i.length>B?(s=i.charAt(B),B++):(s=A,h===0&&D(Qg)),s}function vA(){let s,g;if(h++,s=[],g=us(),g!==A)for(;g!==A;)s.push(g),g=us();else s=A;return h--,s===A&&(g=A,h===0&&D(fg)),s}function G(){let s,g;for(h++,s=[],g=us();g!==A;)s.push(g),g=us();return h--,s===A&&(g=A,h===0&&D(Ig)),s}function us(){let s;return wg.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(Cg)),s}function jA(){let s,g,r;if(h++,s=B,g=[],Ic.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(wc)),r!==A)for(;r!==A;)g.push(r),Ic.test(i.charAt(B))?(r=i.charAt(B),B++):(r=A,h===0&&D(wc));else g=A;return g!==A?s=i.substring(s,B):s=g,h--,s===A&&(g=A,h===0&&D(lg)),s}function KA(){let s;return hg.test(i.charAt(B))?(s=i.charAt(B),B++):(s=A,h===0&&D(Fg)),s===A&&(s=UM()),s}function UM(){let s,g,r;return s=B,g=mM(),g!==A?(r=YM(),r!==A?(g=[g,r],s=g):(B=s,s=A)):(B=s,s=A),s}function YM(){let s;return i.charCodeAt(B)===10?(s=Ug,B++):(s=A,h===0&&D(Yg)),s}function mM(){let s;return i.charCodeAt(B)===13?(s=mg,B++):(s=A,h===0&&D(Dg)),s}function le(){let s;return i.charCodeAt(B)===92?(s=FA,B++):(s=A,h===0&&D(UA)),s}if(ys=M(),ys!==A&&B===i.length)return ys;throw ys!==A&&B<i.length&&D(bg()),lc(at,wA<i.length?i.charAt(wA):null,wA<i.length?Xe(wA,wA+1):Xe(wA,wA))}var Vn=kn,fB=class{get warnings(){return this.song?.warnings||[]}parse(e,A){let t=Vn(Zr(e),{tracer:new dn,...A});return this.song=new Rn().deserialize(t,{notation:A?.notation??null}),this.song}},Ln=fB;function ps(i){return!Number.isNaN(parseInt(i,10))}function Hs(i){return parseInt(i,10)}var IB=class i{constructor(e,A){this.rule=e,this.metadata=A}evaluate(){if("and"in this.rule&&Array.isArray(this.rule.and))return this.and(this.rule.and);if("or"in this.rule&&Array.isArray(this.rule.or))return this.or(this.rule.or);let[e,A]=Object.entries(this.rule)[0];if(!A)return!1;let t=this.metadata[e];return this.evaluateSingleCondition(t,e,A)}evaluateSingleCondition(e,A,t){let c={all:this.all,contains:this.contains,equals:this.equals,exists:this.exists,first:this.firstPage,greater_than:this.greaterThan,greater_than_equal:this.greaterThanEqual,in:this.in,last:this.lastPage,less_than:this.lessThan,less_than_equal:this.lessThanEqual,like:this.like,not_equals:this.notEquals,not_in:this.notIn},M=Object.keys(c).find(a=>a in t);return M?c[M].call(this,e,A,t):!1}all(e,A,t){return Array.isArray(e)&&Array.isArray(t.all)&&t.all.every(c=>e.includes(c))}and(e){return e.every(A=>new i(A,this.metadata).evaluate())}contains(e,A,t){return typeof e=="string"&&typeof t.contains=="string"&&e.toLowerCase().includes(t.contains.toLowerCase())}equals(e,A,t){return e==t.equals}exists(e,A,t){return!!t.exists&&e!==void 0}firstPage(e,A,t){if(A!=="page")throw new Error("First page condition must be on the page field");return!!t.first&&this.metadata.page===1}greaterThan(e,A,t){let c=t.greater_than;return ps(e)?typeof c=="number"&&Hs(e)>c:!1}greaterThanEqual(e,A,t){let c=t.greater_than_equal;return ps(e)?typeof c=="number"&&Hs(e)>=c:!1}in(e,A,t){let c=t.in;return Array.isArray(c)&&c.includes(e)}lastPage(e,A,t){if(A!=="page")throw new Error("Last page condition must be on the page field");return!!t.last&&this.metadata.page===this.metadata.pages}lessThan(e,A,t){let c=t.less_than;return ps(e)?typeof c=="number"&&Hs(e)<c:!1}lessThanEqual(e,A,t){let c=t.less_than_equal;return ps(e)?typeof c=="number"&&Hs(e)<=c:!1}like(e,A,t){let{like:c}=t;return typeof e=="string"&&typeof c=="string"&&e.toLowerCase().includes(c.toLowerCase())}notEquals(e,A,t){return e!=t.not_equals}notIn(e,A,t){return Array.isArray(t.not_in)&&!t.not_in.includes(e)}or(e){return e.some(A=>new i(A,this.metadata).evaluate())}},Pn=IB,wB=class{constructor(e={}){let A=this.getDefaultConfiguration();this.configuration=Bs(A,e)}configure(e){return this.configuration=Bs(this.configuration,e),this}getDefaultConfiguration(){return An()}},Wi=wB,ss="b|i|u|s|tt|big|small|sub|sup|span|strut|sym",Wn=new RegExp(`<(?:\\/(?:${ss})>|(?:${ss})(?:\\s[^>]*)?\\/?>)`,"g");var Eo=new RegExp(`<(${ss})(\\s[^>]*)?\\/>|<(${ss})(\\s[^>]*)?>|<\\/(${ss})>`,"g");function oi(i){return i.replace(Wn,"")}var CB=class extends Wi{constructor(e={}){super(e)}getDefaultConfiguration(){return en()}},Zn=CB;var lB=class{constructor(e){this.startTime=0,this.renderTime=0,this.x=0,this.y=0,this.currentColumn=1,this.elements=[],this.currentPage=1,this.song=e,this.startTime=performance.now()}initialize(){this.x=this.getMinX(),this.y=this.getMinY(),this.currentColumn=1,this.currentPage=1,this.elements=[],this.initializeBackend()}render(e,A){this.initialize(),this.renderParagraphs(e),this.renderChordDiagrams(),this.renderHeadersAndFooters(),this.finalizeRendering(),this.recordRenderingTime()}getContentBottomY(){return this.getColumnBottomY()}getRenderTime(){return this.renderTime}renderParagraphs(e){e.forEach(A=>{A.units.forEach(t=>{this.renderLines(t)}),A.addSpacing&&(this.y+=this.getParagraphSpacing())})}renderLines(e){e.forEach(A=>{this.renderLineLayout(A)})}renderLineLayout(e){let{items:A,lineHeight:t,line:c}=e;if(this.hasColumnBreak(e)){this.moveToNextColumn();return}this.y+t>this.getColumnBottomY()&&this.moveToNextColumn();let M=this.y,{chordsYOffset:o,lyricsYOffset:a}=this.calculateChordLyricYOffsets(A,M),U=this.x;A.forEach(Y=>{let{item:N,width:R}=Y;this.renderItem(N,c,U,o,a,A,M),U+=R}),this.y+=t,this.x=this.getColumnStartX()}hasColumnBreak(e){let{items:A}=e;return A.length===1&&A[0].item instanceof J&&Zi(A[0].item)}renderItem(e,A,t,c,M,o,a){e instanceof H?this.renderChordLyricsPair(e,A,t,c,M,o):e instanceof J?e.isSectionDelimiter()?this.addSectionLabel(e.label,t,a):gs(e)&&this.addComment(e.value,t,a):e instanceof gA&&this.addTextElement(e.content,t,M,"lyrics")}renderChordLyricsPair(e,A,t,c,M,o){let{chords:a}=e,{lyrics:U}=e;if(a&&(a=this.processChords(a,A)),!this.isLyricsOnly()&&a){let Y=this.calculateChordBaseline(c,o,a);this.addTextElement(a,t,Y,"chord")}U&&U.trim()!==""&&this.addTextElement(U,t,M,"lyrics")}moveToNextColumn(){this.currentColumn+=1,this.currentColumn>this.getColumnCount()&&(this.startNewPage(),this.currentColumn=1),this.x=this.getColumnStartX(),this.y=this.getMinY()}startNewPage(){this.currentPage+=1,this.createNewPage()}calculateChordLyricYOffsets(e,A){let t=e.some(({item:U})=>U instanceof H&&U.chords),c=e.some(({item:U})=>U instanceof H&&U.hasLyrics()),M=A,o=A,a=this.getChordLyricSpacing();return t&&c?(M=A,o=M+this.getMaxChordHeight(e)+a):t&&!c?M=A:!t&&c&&(o=A),{chordsYOffset:M,lyricsYOffset:o}}getMaxChordHeight(e){return e.reduce((A,{chordHeight:t})=>Math.max(A,t||0),0)}processChords(e,A){return OB(e,A,this.song,{renderKey:null,useUnicodeModifier:this.useUnicodeModifiers(),normalizeChords:this.normalizeChords(),decapo:this.getConfiguration().decapo})}addTextElement(e,A,t,c){let M=this.getFontForType(c),{width:o,height:a}=this.measureText(e,M);this.elements.push({x:A,y:t,width:o,height:a,content:e,type:c,style:M,page:this.currentPage,column:this.currentColumn})}addSectionLabel(e,A,t){this.addTextElement(e,A,t,"sectionLabel")}addComment(e,A,t){this.addTextElement(e,A,t,"comment")}getFontForType(e){switch(e){case"chord":return this.getFontConfiguration("chord");case"lyrics":return this.getFontConfiguration("text");case"sectionLabel":return this.getFontConfiguration("sectionLabel");case"comment":return this.getFontConfiguration("comment");default:return this.getFontConfiguration("text")}}getTotalPages(){return this.currentPage}getColumnStartX(){let e=this.getLeftMargin(),A=this.getColumnWidth(),t=this.getColumnSpacing();return e+(this.currentColumn-1)*(A+t)}getColumnBottomY(){return this.getPageHeight()-this.getBottomMargin()-this.getFooterHeight()}getColumnWidth(){let e=this.getPageWidth()-this.getLeftMargin()-this.getRightMargin(),A=this.getColumnCount(),t=this.getColumnSpacing();return(e-(A-1)*t)/A}getMinX(){return this.getLeftMargin()}getMinY(){return this.getTopMargin()+this.getHeaderHeight()}recordRenderingTime(){let e=performance.now();this.renderTime=(e-this.startTime)/1e3}getElementsForPage(e){return this.elements.filter(A=>A.page===e)}getLayout(){return this.getConfiguration().layout}getHeaderConfig(){return this.getLayout().header}getFooterConfig(){return this.getLayout().footer}getPageWidth(){return this.getDocPageSize().width}getPageHeight(){return this.getDocPageSize().height}getLeftMargin(){return this.dimensions.margins.left}getRightMargin(){return this.dimensions.margins.right}getTopMargin(){return this.dimensions.margins.top}getBottomMargin(){return this.dimensions.margins.bottom}getHeaderHeight(){return this.getHeaderConfig()?.height??0}getFooterHeight(){return this.getFooterConfig()?.height??0}getColumnCount(){return this.dimensions.effectiveColumnCount}getSectionsConfig(){let{sections:e}=this.getLayout();if(!e)throw new Error("Configuration must include sections");return e}getColumnSpacing(){return this.getSectionsConfig().global.columnSpacing}getChordLyricSpacing(){return this.getSectionsConfig().global.chordLyricSpacing}getParagraphSpacing(){return this.getSectionsConfig().global.paragraphSpacing||0}useUnicodeModifiers(){return this.getConfiguration().useUnicodeModifiers}normalizeChords(){return this.getConfiguration().normalizeChords}isLyricsOnly(){return!!this.getSectionsConfig()?.base?.display?.lyricsOnly}},Xn=lB;var hB=class{constructor(e,A,t){this.measurer=e,this.config=A,this.song=t}measureLineItems(e,A=!1){let t=[];for(let c=0;c<e.items.length;c+=1){let M=e.items[c],o=e.items[c+1]??null;if(yt(M)){let a=this.processChordLyricsPair(M,o,e,A);t.push(...a)}else if(qn(M))t.push(this.processSoftLineBreak(M));else if(ks(M)){let a=this.processTag(M);t.push(...a)}else t.push({item:null,width:0})}return t}processChordLyricsPair(e,A,t,c=!1){let M=this.handleLyricsOnlyMode(e,A,c);if(M)return M;let o=this.splitChordLyricsPair(e,c);return this.processSplitItems(o,A,t,c)}handleLyricsOnlyMode(e,A,t){if(!t)return null;let c=this.removeHyphens(e.lyrics||"");return this.cleanNextItemHyphens(A,()=>{c=c.trimEnd()}),c===""?[{item:null,width:0}]:null}cleanNextItemHyphens(e,A){if(!e||!yt(e))return;let t=e,c=t.lyrics||"";(c.startsWith(" -")||c.startsWith("-"))&&(A(),t.lyrics=this.removeHyphens(c))}processSplitItems(e,A,t,c){return e.map(M=>this.processSingleSplitItem(M,A,t,c)).filter(M=>M!==null)}processSingleSplitItem(e,A,t,c){return e instanceof gA?this.processSoftLineBreak(e):e instanceof H?this.measureChordLyricsPair(e,A,t,c):null}measureChordLyricsPair(e,A,t,c){let{chords:M,lyrics:o}=this.getAdjustedChordsAndLyrics(e,c);if(c&&o==="")return{item:null,width:0};let a=this.renderChordText(M,t),U=this.calculateMeasurements(a,o,A,c);return{item:new H(M,o),width:U.totalWidth,chordHeight:U.chordHeight}}getAdjustedChordsAndLyrics(e,A){return A?{chords:"",lyrics:this.removeHyphens(e.lyrics||"")}:{chords:e.chords||"",lyrics:e.lyrics||""}}renderChordText(e,A){return OB(e,A,this.song,{renderKey:null,useUnicodeModifier:this.config.useUnicodeModifiers,normalizeChords:this.config.normalizeChords,normalizeChordSuffix:this.config.normalizeChordSuffix,decapo:this.config.decapo})}calculateMeasurements(e,A,t,c){let M=this.config.fonts.chord,o=this.config.fonts.lyrics,a=e?this.measurer.measureTextWidth(e,M):0,U=A?this.measurer.measureTextWidth(A,o):0,Y=this.getAdjustedChordWidth(a,U,e,t,c);return{totalWidth:Math.max(Y,U),chordHeight:e?this.measurer.measureTextHeight(e,M):0}}getAdjustedChordWidth(e,A,t,c,M){let o=c&&yt(c)&&c.chords?.trim()!=="",a=this.getSpaceWidth(this.config.fonts.lyrics);if(!M&&o&&e>=A-a){let Y=this.getChordSpacingAsSpaces();return this.measurer.measureTextWidth(t+Y,this.config.fonts.chord)}return e}processSoftLineBreak(e){let A=this.measurer.measureTextWidth(e.content,this.config.fonts.lyrics);return{item:e,width:A}}processTag(e){let A=[];if(gs(e)||e.isSectionDelimiter()){let t=gs(e)?this.config.fonts.comment:this.config.fonts.sectionLabel,c=this.config.width,M=e.label||e.value||"";this.measurer.splitTextToSize(M,c,t).forEach(a=>{let U=this.measurer.measureTextWidth(a,t);A.push(this.createMeasuredTagItem(e,a,U))})}else A.push({item:e,width:0});return A}createMeasuredTagItem(e,A,t){let c=new J(e.name,A);return e.attributes.__titleSeparator==="true"&&(c._value=" > ",c.attributes.__titleSeparator="true",Object.defineProperty(c,"value",{configurable:!0,get:()=>" > ",set:M=>{c._value=M}})),{item:c,width:t}}splitMeasuredItem(e,A){if(!(e.item instanceof H)||!e.item.lyrics)return[e,null];let{lyrics:t}=e.item,c=this.config.fonts.lyrics,M=this.measurer.splitTextToSize(t,A,c);if(M.length===1)return[e,null];let o=M[0],a=M.slice(1).join(" ");return[{item:new H(e.item.chords,o),width:this.measurer.measureTextWidth(o,c),chordHeight:e.chordHeight},{item:new H("",a),width:this.measurer.measureTextWidth(a,c),chordHeight:0}]}removeHyphens(e){let A=e.replace(/\b(\w+)\s*-\s*(\w+)\b/g,"$1$2");return A=A.replace(/(?:\b(\w+)\s*-\s*$)|(?:-\s*$)|(?:\s+-\s+$)/g,"$1"),/^\s*-\s*$/.test(A)?"":A}splitChordLyricsPair(e,A=!1){let{chords:t,lyrics:c,annotation:M}=e;if(!c||c.trim()==="")return[e];let o=c.split(/,\s*/),a=[];return o.forEach((U,Y)=>{if(Y>0&&Y!==0&&(a.push(new gA(" ")),U.trim()!=="")){let N=A?U.trimStart():U;a.push(new H("",N,""))}if(Y===0&&o.length===1)a.push(new H(t,U,M));else if(Y===0&&o.length>1){let N=U;N+=",",a.push(new H(t,N,M))}}),a}findNextItemWithLyrics(e,A,t){return[...e,...A.slice(t)].find(M=>M.item instanceof H&&M.item.lyrics?.trim())||null}capitalizeFirstWord(e){return e.replace(/^\s*\w/,A=>A.toUpperCase())}getSpaceWidth(e){return this.measurer.measureTextWidth(" ",e)}getChordSpacingAsSpaces(){return" ".repeat(this.config.chordSpacing)}},FB=class{constructor(e){this.config=e}createLineLayout(e,A){let t=this.analyzeLineContent(e),c=this.determineLineType(t),M=Math.max(...e.map(a=>a.chordHeight||0)),o=this.calculateLineHeight(t,M);return{type:c,items:e,lineHeight:o,line:A}}analyzeLineContent(e){return{hasChords:e.some(A=>A.item instanceof H&&A.item.chords),hasLyrics:e.some(A=>A.item instanceof H&&A.item.lyrics?.trim()),hasComments:e.some(A=>A.item instanceof J&&gs(A.item)),hasSectionLabel:e.some(A=>A.item instanceof J&&A.item.isSectionDelimiter()),hasTag:e.some(A=>A.item instanceof J)}}determineLineType(e){return e.hasChords||e.hasLyrics?"ChordLyricsPair":e.hasComments?"Comment":e.hasSectionLabel?"SectionLabel":e.hasTag?"Tag":"Empty"}calculateLineHeight(e,A){let{baseHeight:t,fontConfig:c}=this.getBaseHeightAndFont(e,A),M=c?.lineHeight||1;return t*M}getBaseHeightAndFont(e,A){let t=this.config.linePadding;if(e.hasChords&&e.hasLyrics){let c=this.config.fonts.lyrics;return t+=A+this.config.chordLyricSpacing+c.size,{baseHeight:t,fontConfig:c}}return e.hasChords?{baseHeight:t+A,fontConfig:null}:e.hasLyrics?this.addFontHeight(t,this.config.fonts.lyrics):e.hasComments?this.addFontHeight(t,this.config.fonts.comment):e.hasSectionLabel?this.addFontHeight(t,this.config.fonts.sectionLabel):{baseHeight:t,fontConfig:null}}addFontHeight(e,A){return{baseHeight:e+A.size,fontConfig:A}}createColumnBreakLayout(){return{type:"Tag",line:new Ye,items:[{item:new J("column_break"),width:0}],lineHeight:0}}},UB=class{constructor(e,A){this.itemProcessor=e,this.layoutFactory=A}breakLineIntoLayouts(e,A,t=!1){let c=this.itemProcessor.measureLineItems(e,t),M=this.consolidateConsecutiveSoftBreaks(c);return this.breakContent(M,A,e)}breakContent(e,A,t){if(e.length===0)return[];let{totalWidth:c,softBreakIndices:M}=this.analyzeContent(e);if(this.shouldReturnSingleLayout(c,A))return[this.layoutFactory.createLineLayout(e,t)];if(M.length===0)return this.handleNoSoftBreaks(e,A,t);let{firstChunk:o,secondChunk:a}=this.splitAtBestSoftBreak(e,M,c);return o.length===0?this.breakContent(a,A,t):[this.layoutFactory.createLineLayout(o,t),...this.breakContent(a,A,t)]}handleNoSoftBreaks(e,A,t){let c=this.findBreakIndex(e,A);if(c===-1)return[this.layoutFactory.createLineLayout(e,t)];if(c===0)return this.handleOversizedFirstItem(e,A,t);let M=e.slice(0,c),o=e.slice(c);return this.removeTrailingComma(M),[this.layoutFactory.createLineLayout(M,t),...this.breakContent(o,A,t)]}consolidateConsecutiveSoftBreaks(e){let A=[];for(let t=0;t<e.length;t+=1){let c=e[t];A.push(c),c.item instanceof gA&&(t=this.skipConsecutiveSoftBreaks(e,t))}return A}skipConsecutiveSoftBreaks(e,A){let t=A;for(;t+1<e.length&&e[t+1].item instanceof gA;)t+=1;return t}findBreakIndex(e,A){let t=0;for(let c=0;c<e.length;c+=1){if(t+e[c].width>A)return c;t+=e[c].width}return-1}handleOversizedFirstItem(e,A,t){let[c,M]=this.itemProcessor.splitMeasuredItem(e[0],A),o=M?[M,...e.slice(1)]:e.slice(1);return[this.layoutFactory.createLineLayout([c],t),...this.breakContent(o,A,t)]}analyzeContent(e){let A=[];return{totalWidth:e.reduce((c,M,o)=>(M.item instanceof gA&&A.push(o),c+M.width),0),softBreakIndices:A}}shouldReturnSingleLayout(e,A){return e<=A}splitAtBestSoftBreak(e,A,t){let c=t/2,o=A.map(Y=>({index:Y,widthUpToBreak:this.getWidthUpToIndex(e,Y)})).reduce((Y,N)=>{let R=Math.abs(N.widthUpToBreak-c),W=Math.abs(Y.widthUpToBreak-c);return R===W?N.index>Y.index?N:Y:R<W?N:Y}),a=e.slice(0,o.index),U=e.slice(o.index+1);return this.removeTrailingComma(a),this.capitalizeNextItem(U,U,0),{firstChunk:a,secondChunk:U}}removeTrailingComma(e){let A=e[e.length-1];A?.item instanceof H&&A.item.lyrics?.endsWith(",")&&(A.item.lyrics=A.item.lyrics.slice(0,-1)||"",A.width=this.remeasureLyrics(A))}getWidthUpToIndex(e,A){let t=0;for(let c=0;c<A&&c<e.length;c+=1)t+=e[c].width;return t}remeasureLyrics(e){return e.item instanceof H?this.itemProcessor.measurer.measureTextWidth(e.item.lyrics||"",this.itemProcessor.config.fonts.lyrics):e.width}recalculateChordLyricWidth(e,A=null){if(!(e.item instanceof H))return e.width;let t=e.item,c=this.measureChordAndLyricWidths(t),M=this.adjustChordWidth(t,c,A);return Math.max(M,c.lyricsWidth)}measureChordAndLyricWidths(e){let A=this.itemProcessor.config.fonts.chord,t=this.itemProcessor.config.fonts.lyrics,c=e.chords?this.itemProcessor.measurer.measureTextWidth(e.chords,A):0,M=e.lyrics?this.itemProcessor.measurer.measureTextWidth(e.lyrics,t):0;return{chordWidth:c,lyricsWidth:M}}adjustChordWidth(e,A,t){if(this.shouldSkipChordSpacing(A.chordWidth,t))return A.chordWidth;let c=this.itemProcessor.config.fonts.lyrics,M=this.itemProcessor.measurer.measureTextWidth(" ",c);if(A.chordWidth<A.lyricsWidth-M)return A.chordWidth;let o=this.itemProcessor.config.fonts.chord,a=" ".repeat(this.itemProcessor.config.chordSpacing),U=`${e.chords||""}${a}`;return this.itemProcessor.measurer.measureTextWidth(U,o)}shouldSkipChordSpacing(e,A){return this.itemProcessor.config.displayLyricsOnly||e===0||!A||!(A.item instanceof H)?!0:(A.item.chords||"").trim()===""}capitalizeNextItem(e,A,t){let c=this.itemProcessor.findNextItemWithLyrics(e,A,t);if(c&&c.item instanceof H){let M=c.item.lyrics??"";c.item.lyrics=this.itemProcessor.capitalizeFirstWord(M);let o=A.indexOf(c),a=o>=0&&o<A.length-1?A[o+1]:null;c.width=this.recalculateChordLyricWidth(c,a)}}};function is(){return{type:"Tag",line:new Ye,items:[{item:new J("column_break"),width:0}],lineHeight:0}}function vn(i){return i.reduce((e,A)=>{let t=A.reduce((c,M)=>c+M.lineHeight,0);return e+t},0)}function Kn(i){return i.length===1&&i[0].items.length===1&&i[0].items[0].item instanceof J&&Zi(i[0].items[0].item)}var YB=class{constructor(e){this.chordLyricLineCount=e,this.totalChordLyricPairLines=e}splitParagraph(e,A,t,c){let M=this.flattenLineLayouts(e),o=this.trySplitParagraph(e,M,A,c);return o||(A!==t?[[is()],...e]:e)}trySplitParagraph(e,A,t,c){let M=this.findAcceptableSplits(A),o=this.selectSplit(M,t,c);return o?this.buildSplitResult(e,o.index):null}selectSplit(e,A,t){for(let c=e.length-1;c>=0;c-=1){let M=e[c];if(A+M.heightFirstPart<=t)return M}return null}buildSplitResult(e,A){let{firstPart:t,secondPart:c}=this.partitionLineLayouts(e,A);return[...t,[is()],...c]}flattenLineLayouts(e){let A=[];return e.forEach(t=>{t.forEach(c=>{A.push(c)})}),A}findAcceptableSplits(e){let A=0,t=0;return e.slice(0,-1).reduce((c,M,o)=>{A+=M.lineHeight,t+=M.type==="ChordLyricsPair"?1:0;let a=this.totalChordLyricPairLines-t;return t>=2&&a>=2&&c.push({index:o+1,heightFirstPart:A}),c},[])}partitionLineLayouts(e,A){let t=e.reduce((c,M)=>this.partitionReducer(c,M,A),{firstPart:[],secondPart:[],processed:0});return{firstPart:t.firstPart,secondPart:t.secondPart}}partitionReducer(e,A,t){let c=A.length,M=e.processed+c<=t,o=e.processed>=t;if(M)e.firstPart.push(A);else if(o)e.secondPart.push(A);else{let a=t-e.processed;e.firstPart.push(A.slice(0,a)),e.secondPart.push(A.slice(a))}return e.processed+=c,e}},mB=class{splitParagraph(e,A,t,c){if(e.length===0)return[];let{splitIndex:M,heightFirstPart:o}=this.findSplitMetadata(e);return M===null?this.prependColumnBreakIfNeeded(e,A,t):A+o<=c?this.buildSplitResult(e,M):this.prependColumnBreakIfNeeded(e,A,t)}findSplitMetadata(e){let A=0,t=0;for(let c=0;c<e.length;c+=1){let{height:M,chordCount:o}=this.summarizeLineLayout(e[c]);if(A+=o,t+=M,A>=2)return{splitIndex:c+1,heightFirstPart:t}}return{splitIndex:null,heightFirstPart:t}}summarizeLineLayout(e){return e.reduce((A,t)=>({height:A.height+t.lineHeight,chordCount:A.chordCount+(t.type==="ChordLyricsPair"?1:0)}),{height:0,chordCount:0})}buildSplitResult(e,A){let t=e.slice(0,A),c=e.slice(A);return t.concat([[is()]]).concat(c)}prependColumnBreakIfNeeded(e,A,t){return A===t?e:[[is()]].concat(e)}},DB=class{splitParagraph(e,A,t,c){let M=[];return A!==t&&M.push([is()]),M.concat(e)}},NB=class{splitParagraph(e,A,t,c,M){return this.createStrategy(M).splitParagraph(e,A,t,c)}createStrategy(e){return e<=3?new DB:e===4?new mB:new YB(e)}},jB=class i extends J{constructor(){super("comment"," > "),this.attributes.__titleSeparator="true"}get value(){return" > "}set value(e){super.value=e}clone(){let e=new i;return e.attributes={...this.attributes},e.selector=this.selector,e.isNegated=this.isNegated,e}},On=jB,bB=class{constructor(e,A,t){this.song=e,this.measurer=A,this.config=t,this.sectionCache=new Map,this.processRepeatedSections(),this.itemProcessor=new hB(this.measurer,this.config,this.song),this.layoutFactory=new FB(t),this.lineBreaker=new UB(this.itemProcessor,this.layoutFactory),this.paragraphSplitter=new NB}normalizeSectionLabel(e){return e?e.replace(/\s*\([^)]*(?:x|\d+|repeat|rep)\s*[^)]*\)\s*$/i,"").trim().toLowerCase().replace(/\s+/g,"_"):null}getSectionCacheKey(e){let A=null;return e.label?A=e.label:e.lines.length>0&&(A=this.extractLabelFromFirstItem(e.lines[0].items[0])),this.normalizeSectionLabel(A)??null}extractLabelFromFirstItem(e){if(!e||!ks(e))return null;let A=e;return!gs(A)&&!A.isSectionDelimiter()?null:A.label||A.value||null}processRepeatedSections(){if(!this.config.repeatedSections)return;this.sectionCache.clear();let e=[],A=new Set,{bodyParagraphs:t}=this.song.clone();t.forEach((c,M)=>{this.processParagraph({paragraph:c,index:M,processedParagraphs:e,skipIndices:A,bodyParagraphs:t})}),this.song.renderParagraphs=e,this.sectionCache.clear()}processParagraph(e){let{paragraph:A,index:t,skipIndices:c,processedParagraphs:M}=e;if(this.shouldSkipParagraphIndex(t,c))return;let o=this.getSectionCacheKey(A);if(!o){this.addUncachedParagraph(M,A);return}this.handleParagraphWithCacheKey(o,e)}shouldSkipParagraphIndex(e,A){return A.has(e)}addUncachedParagraph(e,A){e.push(this.cloneParagraph(A))}handleParagraphWithCacheKey(e,A){let t=this.cloneParagraph(A.paragraph);if(this.cacheFirstOccurrence(e,t,A.processedParagraphs))return;let c=this.sectionCache.get(e);c&&this.handleRepeatedSection({cachedParagraph:c,currentParagraph:t,processedParagraphs:A.processedParagraphs,skipIndices:A.skipIndices,startIndex:A.index,bodyParagraphs:A.bodyParagraphs})}cacheFirstOccurrence(e,A,t){return this.sectionCache.has(e)?!1:(this.sectionCache.set(e,A),t.push(A),!0)}handleRepeatedSection(e){switch(this.config.repeatedSections){case"hide":return;case"title_only":this.handleTitleOnlyMode(e);return;case"lyrics_only":e.processedParagraphs.push(e.currentParagraph);return;case"full":this.handleFullMode(e);return;default:e.processedParagraphs.push(e.currentParagraph)}}handleTitleOnlyMode(e){let A=this.collectTitleParagraphs(e);e.processedParagraphs.push(this.resolveTitleOnlyParagraph(A))}collectTitleParagraphs(e){let{currentParagraph:A,skipIndices:t,startIndex:c,bodyParagraphs:M}=e,o=[this.createTitleOnlyParagraph(A)];for(let a=c+1;a<M.length;a+=1){let U=M[a];if(!t.has(a))if(this.shouldIncludeTitle(U))o.push(this.createTitleOnlyParagraph(U)),t.add(a);else break}return o}shouldIncludeTitle(e){let A=this.getSectionCacheKey(e);return!!(A&&this.sectionCache.has(A))}resolveTitleOnlyParagraph(e){return e.length===1?e[0]:this.consolidateTitles(e)}handleFullMode(e){let{cachedParagraph:A,currentParagraph:t,processedParagraphs:c}=e,M=new As;this.buildFullModeLines(t,A).forEach(o=>{M.addLine(o)}),c.push(M)}buildFullModeLines(e,A){if(e.lines.length===0)return A.lines.map(M=>M.clone());let t=e.lines[0],c=t.items[0];return c&&ks(c)?[t.clone(),...A.lines.slice(1).map(M=>M.clone())]:[...e.lines.map(M=>M.clone()),...A.lines.slice(1).map(M=>M.clone())]}consolidateTitles(e){let A=this.cloneParagraph(e[0]);if(A.lines.length===0)return A;let t=A.lines[0];return e.slice(1).forEach(c=>{if(c.lines.length===0)return;let M=c.lines[0];this.addTitleSeparator(t),M.items.forEach(o=>{t.items.push(this.cloneItem(o))})}),A}addTitleSeparator(e){e.items.push(...this.buildTitleSeparatorItems())}buildTitleSeparatorItems(){return[new gA(" "),new On,new gA(" ")]}createTitleOnlyParagraph(e){let A=new As;if(e.lines.length===0)return A;let t=e.lines[0],c=new Ye({type:t.type,items:[]});return t.items.forEach(M=>{ks(M)&&c.addItem(this.cloneItem(M))}),c.items.length===0&&(c.items=[...t.items.map(M=>this.cloneItem(M))]),A.addLine(c),A}cloneParagraph(e){let A=new As;return e.lines.forEach(t=>A.addLine(t.clone())),A}cloneItem(e){return e instanceof gA?new gA(e.content):typeof e.clone=="function"?e.clone():e}computeParagraphLayouts(){let e=[],A={currentY:this.config.minY,currentColumn:1};return this.song.renderParagraphs.forEach(t=>{A=this.processParagraphLayout(t,e,A)}),e}processParagraphLayout(e,A,t){let c=this.computeParagraphLayout(e,this.config.columnWidth,this.config.displayLyricsOnly),M=this.computeLineTypeCounts(c);if(this.shouldSkipParagraphLayout(M))return t;let o=this.adjustParagraphLayouts(c,t,M.chordLyricLineCount),a=this.calculateNextSimulationState(o,t);return this.addParagraphLayout(A,e,o),a}computeLineTypeCounts(e){let A=0,t=0;return e.forEach(c=>{c.forEach(M=>{M.type==="ChordLyricsPair"?A+=1:(M.type==="Comment"||M.type==="SectionLabel")&&(t+=1)})}),{chordLyricLineCount:A,nonLyricLineCount:t}}shouldSkipParagraphLayout(e){return!!this.config.displayLyricsOnly&&e.nonLyricLineCount===1&&e.chordLyricLineCount===0}adjustParagraphLayouts(e,A,t){let c=vn(e);return A.currentY+c<=this.config.columnBottomY?e:this.paragraphSplitter.splitParagraph(e,A.currentY,this.config.minY,this.config.columnBottomY,t)}calculateNextSimulationState(e,A){let{currentY:t,currentColumn:c}=A;return e.forEach(M=>{if(Kn(M))c+=1,c>(this.config.columnCount||1)&&(c=1),t=this.config.minY;else{let o=M.reduce((a,U)=>a+U.lineHeight,0);t+=o}}),t+=this.config.paragraphSpacing,{currentY:t,currentColumn:c}}addParagraphLayout(e,A,t){e.push({units:t,addSpacing:!0,sectionType:A.type})}computeParagraphLayout(e,A,t=!1){let c=[];return e.lines.forEach(M=>{if($n(M)){let o=this.lineBreaker.breakLineIntoLayouts(M,A,t);c.push(o)}}),c}};var ao={[rs]:gt,[Fe]:os,[Ms]:it,[Bt]:ts},Qo={[rs]:kB,[Fe]:ct,[Ms]:SB,[Bt]:Ws};function yt(i){return i instanceof H}function $n(i){return i.items.some(e=>e.isRenderable())}function ks(i){return i instanceof J}function gs(i){return i.name==="comment"}function Zi(i){return i instanceof J&&i.name==="column_break"}function qn(i){return i instanceof gA}function _n(i,e){return(e.delegates[i.type]||he)(i.contents)}function Ao(i){return i.items.some(e=>e instanceof H&&!XB(e.lyrics)||e instanceof J&&e.isRenderable()||Pr(e))}function eo(i,e,A){return i.evaluate(e,A.metadata.separator)}var zB=class extends Wi{format(e,A){return this.song=e,this.metadata=A||e.getMetadata(this.configuration),[this.formatHeader(),this.formatParagraphs()].join("")}formatHeader(){let{title:e,subtitle:A}=this.song,t=e||A?`
`:"";return[this.formatTitle(e),this.formatSubTitle(A),t].join("")}formatParagraphs(){let{bodyParagraphs:e,expandedBodyParagraphs:A}=this.song,{expandChorusDirective:t}=this.configuration,c=t?A:e;return this.song.filterParagraphs(c,this.configuration).map(M=>this.formatParagraph(M,this.metadata)).join(`

`)}formatParagraph(e,A){return e.isLiteral()?[e.label,_n(e,this.configuration)].filter(t=>t).join(`
`):e.lines.filter(t=>t.hasRenderableItems()).map(t=>this.formatLine(t,A)).join(`
`)}formatLine(e,A){return[this.formatLineTop(e,A),this.formatLineBottom(e,A)].filter(c=>!XB(c)).map(c=>(c||"").trimEnd()).join(`
`)}formatTitle(e){return e?`${e.toUpperCase()}
`:""}formatSubTitle(e){return e?`${e}
`:""}formatLineTop(e,A){return Lr(e)?this.formatLineWithFormatter(e,this.formatItemTop,A):null}chordLyricsPairLength(e,A){let t=e.annotation||this.renderChords(e,A),{lyrics:c}=e,M=(t||"").length,o=oi(c||"").length;return M>=o?M+1:Math.max(M,o)}renderChords(e,A){return OB(e.chord??e.chords,A,this.song,{renderKey:this.configuration.key,useUnicodeModifier:this.configuration.useUnicodeModifiers,normalizeChords:this.configuration.normalizeChords,normalizeChordSuffix:this.configuration.normalizeChordSuffix,decapo:this.configuration.decapo})}formatItemTop(e,A,t){if(e instanceof J&&e.isRenderable())return e.label;if(e instanceof H){let c=e.annotation||this.renderChords(e,t);return Kc(c,this.chordLyricsPairLength(e,t))}return""}formatLineBottom(e,A){return Ao(e)?this.formatLineWithFormatter(e,this.formatItemBottom,A):""}formatLineWithFormatter(e,A,t){return e.items.map(c=>A.call(this,c,t,e)).join("")}formatItemBottom(e,A,t){return e instanceof J&&e.isRenderable()?e.label:e instanceof H?Kc(oi(e.lyrics||""),this.chordLyricsPairLength(e,t)):"evaluate"in e?eo(e,A,this.configuration):""}constructor(...e){super(...e),this.song=new cs,this.metadata=new st}},so=zB,TB=class{constructor(e,A){this.backend=e,this.context=A}renderLayout(e,A){let{height:t}=e,{height:c}=this.backend.pageSize,M=A==="header"?this.context.margins.top:c-t-this.context.margins.bottom;e.content.forEach(o=>{let a=o;this.shouldRenderContent(a)&&(a.type==="text"?this.renderTextItem(a,M):a.type==="image"?this.renderImage(a,M):a.type==="line"&&this.renderLine(a,M))})}shouldRenderContent(e){if(!e.condition)return!0;let{metadata:A,extraMetadata:t}=this.context,c=new Proxy({},{get:(M,o)=>{let a=t?.[o]??A.get(o);if((o==="page"||o==="pages"||o==="renderTime")&&typeof a=="string"){let U=Number(a);if(!Number.isNaN(U))return U}return a}});return new Pn(e.condition,c).evaluate()}renderTextItem(e,A){let{value:t,template:c="",style:M,position:o}=e,a=this.context.metadata.merge(this.context.extraMetadata),U=t||this.evaluateTemplate(c,a);if(!U)return;this.backend.setFontStyle(M);let Y=o.width||this.getAvailableWidth(),N=A+o.y;o.clip?this.renderClippedText(U,e,Y,N,M):this.renderMultilineText(U,e,Y,N,M)}renderClippedText(e,A,t,c,M){let{position:o}=A,a=o.ellipsis?this.clipTextWithEllipsis(e,t,M):this.clipText(e,t,M),U=this.backend.getTextWidth(a,M),Y=o.width??U;this.backend.setTextItem?.(A);let N=this.calculateX(o.x,Y,o.offsetX);this.backend.text(a,N,c)}clipTextWithEllipsis(e,A,t){let c=e;for(;this.backend.getTextWidth(`${c}...`,t)>A&&c.length>0;)c=c.slice(0,-1);return c!==e?`${c}...`:e}clipText(e,A,t){let c=e;for(;this.backend.getTextWidth(c,t)>A&&c.length>0;)c=c.slice(0,-1);return c}renderMultilineText(e,A,t,c,M){let{position:o}=A,a=this.backend.splitTextToSize(e,t,M),U=c;a.forEach(Y=>{let N=this.backend.getTextWidth(Y,M),R=o.width??N;this.backend.setTextItem?.(A);let W=this.calculateX(o.x,R,o.offsetX);this.backend.text(Y,W,U),U+=M.size*(M.lineHeight??1.2)})}renderImage(e,A){let{src:t,position:c,size:M,alias:o,compression:a,rotation:U}=e,Y=this.calculateX(c.x,M.width,c.offsetX),N=A+c.y,R=t.split(".").pop()?.toUpperCase();this.backend.addImage(t,R,Y,N,M.width,M.height,o,a,U)}renderLine(e,A){let{style:t,position:c}=e;this.backend.setLineStyle(t);let M=this.context.margins.left+(c.x||0),o=A+c.y,a=c.width==="auto"?this.getAvailableWidth():c.width;this.backend.line(M,o,M+a,o+(c.height||0)),this.backend.resetDash()}evaluateTemplate(e,A){try{let t=new Ln().parse(e);return new so().format(t,A)}catch(t){throw new Error(`Error evaluating template

${e}

: ${t.message}`,{cause:t})}}calculateX(e,A=0,t=0){switch(e){case"center":return this.backend.pageSize.width/2-A/2+t;case"right":return this.backend.pageSize.width-this.context.margins.right-A+t;default:return typeof e=="number"?this.context.margins.left+e+t:this.context.margins.left+t}}getAvailableWidth(){return this.backend.pageSize.width-this.context.margins.left-this.context.margins.right}},to=TB,yB=class extends Xn{constructor(e,A,t){super(e),this._dimensions=null,this._dimensionCacheKey=null,this.doc=rn.setup(A),this.configuration=t}getFontConfiguration(e){return this.configuration.fonts[e]}getDocumentMetadata(){return{pageWidth:this.getPageWidth(),pageHeight:this.getPageHeight(),marginLeft:this.getLeftMargin(),marginRight:this.getRightMargin(),marginTop:this.getTopMargin(),marginBottom:this.getBottomMargin(),columnWidth:this.getColumnWidth(),columnCount:this.getColumnCount(),currentPage:this.doc.currentPage,totalPages:this.doc.totalPages,renderTime:this.renderTime,dimensions:this.dimensions}}save(e){this.doc.save(e)}async generatePDF(){return this.doc.output()}getDoc(){return this.doc}initializeBackend(){}createNewPage(){this.doc.newPage()}renderChordDiagrams(){let{chordDiagrams:e}=this.configuration.layout;new fn(this.createDiagramContext(),this.createDiagramConfig(e)).render()}createDiagramContext(){return{song:this.song,doc:this.doc,getMinY:()=>this.getMinY(),getColumnStartX:()=>this.getColumnStartX(),getColumnBottomY:()=>this.getColumnBottomY(),moveToNextColumn:()=>this.moveToNextColumn(),dimensions:this.dimensions,getX:()=>this.x,getY:()=>this.y,getCurrentColumn:()=>this.currentColumn,getCurrentPage:()=>this.currentPage,setPosition:(e,A,t,c)=>{this.x=e,this.y=A,this.currentColumn=t,this.currentPage=c}}}createDiagramConfig(e){return{enabled:e.enabled,renderingConfig:e.renderingConfig,fonts:e.fonts,overrides:e.overrides}}renderHeadersAndFooters(){let e=this.createLayoutRenderer();this.getHeaderConfig()&&this.doc.eachPage(()=>{e.renderLayout(this.getHeaderConfig(),"header")}),this.getFooterConfig()&&this.doc.eachPage(()=>{e.renderLayout(this.getFooterConfig(),"footer")})}createLayoutRenderer(){let e=this.createLayoutBackend();return new to(e,{metadata:this.song.metadata,margins:this.dimensions.margins,extraMetadata:this.getExtraMetadata(this.doc.currentPage,this.doc.totalPages)})}createLayoutBackend(){return{pageSize:this.doc.pageSize,currentPage:this.doc.currentPage,totalPages:this.doc.totalPages,text:(e,A,t)=>this.doc.text(e,A,t),getTextWidth:e=>this.doc.getTextWidth(e),splitTextToSize:(e,A)=>this.doc.splitTextToSize(e,A),setFontStyle:e=>this.doc.setFontStyle(e),addImage:(e,A,t,c,M,o,a,U,Y)=>{this.doc.addImage(e,A,t,c,M,o,a,U,Y)},line:(e,A,t,c)=>this.doc.line(e,A,t,c),setLineStyle:e=>this.doc.setLineStyle(e),resetDash:()=>this.doc.resetDash(),setDrawColor:e=>this.doc.setDrawColor(e),setLineWidth:e=>this.doc.setLineWidth(e)}}measureText(e,A){let t=this.doc.getTextDimensions(e,A);return{width:t.w,height:t.h}}calculateChordBaseline(e,A,t){let c=this.getFontConfiguration("chord"),M=this.doc.getTextDimensions(t,c);return e+this.getMaxChordHeight(A)-M.h}finalizeRendering(){let e=Math.max(this.currentPage,this.doc.totalPages);for(;this.doc.totalPages<e;)this.doc.newPage();for(let A=1;A<=e;A+=1){let t=this.getElementsForPage(A);t.length>0&&(this.doc.setPage(A),t.forEach(c=>{this.drawElement(c)}))}}getConfiguration(){return this.configuration}get dimensions(){let e=this.generateDimensionCacheKey();return(this._dimensionCacheKey!==e||this._dimensions===null)&&(this._dimensions=this.buildDimensions(),this._dimensionCacheKey=e),this._dimensions}getDocPageSize(){return this.doc.pageSize}drawElement(e){switch(e.style&&this.doc.setFontStyle(e.style),e.type){case"chord":case"lyrics":case"sectionLabel":case"comment":this.doc.text(e.content,e.x,e.y),this.drawUnderlineIfNeeded(e);break;default:console.warn(`Unknown element type: ${e.type}`);break}}drawUnderlineIfNeeded(e){let A=e.content?.trim()===">";if(e.style?.underline&&!A){let{w:t}=this.doc.getTextDimensions(e.content);this.doc.setDrawColor(0),this.doc.setLineWidth(1.25),this.doc.line(e.x,e.y+3,e.x+t,e.y+3)}}buildDimensions(){let{width:e,height:A}=this.doc.pageSize,{columnCount:t,columnSpacing:c,minColumnWidth:M,maxColumnWidth:o}=this.configuration.layout.sections.global;return new RM(e,A,this.configuration.layout,{columnCount:t,columnSpacing:c,minColumnWidth:M,maxColumnWidth:o})}generateDimensionCacheKey(){let{width:e,height:A}=this.doc.pageSize,{layout:t}=this.configuration,{global:c}=t.sections;return[e,A,t.global.margins.left,t.global.margins.right,t.global.margins.top,t.global.margins.bottom,t.header.height,c.columnCount,c.columnSpacing,c.minColumnWidth||0,c.maxColumnWidth||0].join("-")}getExtraMetadata(e,A){let t={page:e.toString(),pages:A.toString(),renderTime:this.renderTime.toString()},c=this.song.metadata.getSingle("capo"),M=this.song.metadata.getSingle("key");if(c&&M){let o=parseInt(c,10);t.capoKey=Fn(M)[o]}return t}},Bo=yB,RB=class extends Zn{getDefaultConfiguration(){return sn()}format(e,A=pc){this.song=e,this.renderer=new Bo(e,A,this.configuration),this.renderer.initialize();let c=this.createLayoutEngine().computeParagraphLayouts();this.renderer.render(c)}createLayoutEngine(){if(!this.renderer)throw new Error("Renderer not initialized");let e=this.renderer.getDocumentMetadata(),{dimensions:A}=e,t={width:A.columnWidth,fonts:{chord:this.configuration.fonts.chord,lyrics:this.configuration.fonts.text,comment:this.configuration.fonts.comment,sectionLabel:this.configuration.fonts.sectionLabel},chordSpacing:this.configuration.layout.sections.global.chordSpacing,chordLyricSpacing:this.configuration.layout.sections.global.chordLyricSpacing,linePadding:this.configuration.layout.sections.global.linePadding,useUnicodeModifiers:this.configuration.useUnicodeModifiers,normalizeChords:this.configuration.normalizeChords,normalizeChordSuffix:this.configuration.normalizeChordSuffix,minY:A.minY,columnWidth:A.columnWidth,columnSpacing:this.configuration.layout.sections.global.columnSpacing,minColumnWidth:this.configuration.layout.sections.global.minColumnWidth,maxColumnWidth:this.configuration.layout.sections.global.maxColumnWidth,paragraphSpacing:this.configuration.layout.sections.global.paragraphSpacing||0,columnBottomY:this.renderer.getContentBottomY(),displayLyricsOnly:!!this.configuration.layout.sections?.base?.display?.lyricsOnly,decapo:this.configuration.decapo,repeatedSections:this.configuration.layout.sections?.base?.display?.repeatedSections};return this.configuration.layout.sections.global.columnCount&&(t.columnCount=this.configuration.layout.sections.global.columnCount),new bB(this.song,new Vs(this.renderer.getDoc()),t)}save(){this.renderer&&this.renderer.save(`${this.song.title||"untitled"}.pdf`)}async generatePDF(){if(this.renderer)return this.renderer.generatePDF();throw new Error("Renderer not initialized")}getDocumentWrapper(){if(!this.renderer)throw new Error("Renderer not initialized");return this.renderer.doc}constructor(...e){super(...e),this.song=new cs,this.renderer=null}},co=RB;return TM(io);})();
